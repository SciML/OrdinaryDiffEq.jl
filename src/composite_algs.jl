...
function (AS::AutoSwitchCache)(integrator)
    ...
    AS.count =
        is_stiff(integrator, AS.nonstiffalg, AS.nonstifftol, AS.stifftol, AS.is_stiffalg) ?
        AS.count < 0 ? 1 : AS.count + 1 : AS.count > 0 ? -1 : AS.count - 1
