...
@inline function perform_step!(integrator,cache::BS3Cache,f=integrator.f)
  ...
  a1 = dt*a21
  for i in uidx
    tmp[i] = @muladd uprev[i]+a1*k1[i]
  end
  f(@muladd(t+c1*dt),tmp,k2)
  a2 = dt*a32
  for i in uidx
    tmp[i] = @muladd uprev[i]+a2*k2[i]
  end
  f(@muladd(t+c2*dt),tmp,k3)
  for i in uidx
    u[i] = @muladd uprev[i]+dt*(a41*k1[i]+a42*k2[i]+a43*k3[i])
  end
  f(t+dt,u,k4)
  if integrator.opts.adaptive
    for i in uidx
...
@inline function perform_step!(integrator,cache::BS5Cache,f=integrator.f)
  ...
  for i in uidx
    tmp[i] = @muladd uprev[i]+dt*(a71*k1[i]+a72*k2[i]+a73*k3[i]+a74*k4[i]+a75*k5[i]+a76*k6[i])
  end
  f(t+dt,tmp,k7)
  for i in uidx
    u[i] = @muladd uprev[i]+dt*(a81*k1[i]+a83*k3[i]+a84*k4[i]+a85*k5[i]+a86*k6[i]+a87*k7[i])
  end
  f(t+dt,u,k8)
  if integrator.opts.adaptive
    for i in uidx
      uhat[i]   = dt*@muladd(bhat1*k1[i] + bhat3*k3[i] + bhat4*k4[i] + bhat5*k5[i] + bhat6*k6[i])
      utilde[i] = @muladd uprev[i] + dt*(btilde1*k1[i] + btilde2*k2[i] + btilde3*k3[i] + btilde4*k4[i] + btilde5*k5[i] + btilde6*k6[i] + btilde7*k7[i] + btilde8*k8[i])
      atmp[i] = ((uhat[i])./@muladd(integrator.opts.abstol+max(abs(uprev[i]),abs(u[i])).*integrator.opts.reltol))
      atmptilde[i] = ((utilde[i]-u[i])./@muladd(integrator.opts.abstol+max(abs(uprev[i]),abs(u[i])).*integrator.opts.reltol))
    end
...
@inline function perform_step!(integrator,cache::DP5Cache,f=integrator.f)
  ...
  for i in uidx
    ...
    integrator.k[4][i] = @muladd(d1*k1[i]+d3*k3[i]+d4*k4[i]+d5*k5[i]+d6*k6[i]+d7*k7[i])
  end
  @pack integrator = t,dt,u,k
end
