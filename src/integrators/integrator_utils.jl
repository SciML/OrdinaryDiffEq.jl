...
function modify_dt_for_tstops!(integrator)
    if has_tstop(integrator)
        ...
        if integrator.opts.adaptive
            integrator.dt =
                integrator.tdir * min(abs(integrator.dt), abs(tdir_tstop - tdir_t)) # step! to the end
        ...
        elseif integrator.dtchangeable && !integrator.force_stepfail
            ...
            integrator.dt =
                integrator.tdir * min(abs(integrator.dtcache), abs(tdir_tstop - tdir_t)) # step! to the end
        end
    end
end
        ...
        if curt != integrator.t # If <t, interpolate
            ...
            if integrator.opts.save_idxs === nothing
                ...
                copyat_or_push!(
                    integrator.sol.u,
                    integrator.saveiter,
                    integrator.u[integrator.opts.save_idxs],
                    Val{false},
                )
            end
...
function _loopfooter!(integrator)
    ...
    elseif integrator.opts.adaptive
        ...
        if integrator.accept_step # Accept
            ...
            dtnew =
                DiffEqBase.value(step_accept_controller!(integrator, integrator.alg, q)) *
...
function calc_dt_propose!(integrator, dtnew)
    if (typeof(integrator.alg) <: Union{ROCK2,ROCK4,SERK2,ESERK4,ESERK5}) &&
       integrator.opts.adaptive &&
       (integrator.iter >= 1)
        (integrator.alg isa ROCK2) && (
            dtnew = min(
                dtnew,
                typeof(dtnew)((
                    ((min(integrator.alg.max_stages, 200)^2.0) * 0.811 - 1.5) /
                    integrator.eigen_est
                )),
            )
        )
        (integrator.alg isa ROCK4) && (
            dtnew = min(
                dtnew,
                typeof(dtnew)((
                    ((min(integrator.alg.max_stages, 152)^2.0) * 0.353 - 3) /
                    integrator.eigen_est
                )),
            )
        )
        (integrator.alg isa SERK2) && (
            dtnew = min(
                dtnew,
        ...
        elseif tdir_t > tdir_tstop
            if !integrator.dtchangeable
                DiffEqBase.change_t_via_interpolation!(
                    integrator,
                    integrator.tdir * pop_tstop!(integrator),
                    Val{true},
                )
                integrator.just_hit_tstop = true
            else
...
DiffEqBase.nlsolve_f(f, alg::OrdinaryDiffEqAlgorithm) =
    f isa SplitFunction && issplit(alg) ? f.f1 : f
...
(integrator::ODEIntegrator)(
    ...
    ::Type{deriv} = Val{0};
    idxs = nothing,
) where {deriv} = current_interpolant!(val, t, integrator, idxs, deriv)
