...
function _change_t_via_interpolation!(
    integrator,
    t,
    modify_save_endpoint::Type{Val{T}},
) where {T}
...
) where {T} = _change_t_via_interpolation!(integrator, t, modify_save_endpoint)
...
get_proposed_dt(integrator::ODEIntegrator) =
    ifelse(integrator.opts.adaptive, integrator.dtpropose, integrator.dtcache)
set_proposed_dt!(integrator::ODEIntegrator, dt::Number) =
...
@inline function DiffEqBase.get_du!(out, integrator::ODEIntegrator)
    ...
    if typeof(integrator.cache) <: FunctionMapCache
        ...
        return if isdefined(integrator, :fsallast) && !(
            ...
            Union{Rosenbrock23,Rosenbrock32,Rodas4,Rodas4P,Rodas4P2,Rodas5,Rodas5P}
        )
...
@inline DiffEqBase.get_tmp_cache(integrator, alg::OrdinaryDiffEqAlgorithm, cache) =
    (cache.tmp,)
@inline DiffEqBase.get_tmp_cache(integrator, alg::Union{RadauIIA3,RadauIIA5}, cache) =
...
@inline DiffEqBase.get_tmp_cache(integrator, alg::OrdinaryDiffEqNewtonAlgorithm, cache) =
...
@inline DiffEqBase.get_tmp_cache(
    integrator,
    alg::Union{SSPRK22,SSPRK33,SSPRK53_2N1,SSPRK53_2N2,SSPRK43,SSPRK432,SSPRK932},
    cache,
) = (cache.k,)
@inline DiffEqBase.get_tmp_cache(
    integrator,
...
function DiffEqBase.add_saveat!(integrator::ODEIntegrator, t)
    integrator.tdir * (t - integrator.t) < zero(integrator.t) && error(
        "Tried to add a saveat that is behind the current time. This is strictly forbidden",
    )
...
resize_non_user_cache!(integrator::ODEIntegrator, i::Int) =
    resize_non_user_cache!(integrator, integrator.cache, i)
deleteat_non_user_cache!(integrator::ODEIntegrator, i) =
    deleteat_non_user_cache!(integrator, integrator.cache, i)
addat_non_user_cache!(integrator::ODEIntegrator, i) =
