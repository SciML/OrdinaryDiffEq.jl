...
"""
struct IController <: AbstractController end
...
```
"""
struct PredictiveController <: AbstractController end
...
struct DummyController <: AbstractController end
...
function choose_order!(
    ...
    if k < max_order &&
       integrator.cache.nconsteps >= integrator.cache.order + 2 &&
       (
           (k == 1 && terk > terkp1) ||
        ...
        while !(terkm2 > terkm1 > terk > terkp1) && k > 2
            ...
            terkm1 = terkm2
            fd_weights =
                calc_finite_difference_weights(ts_tmp, t + dt, k - 2, Val(max_order))
       ...
       (
        ...
        while !(terkm2 > terkm1 > terk > terkp1) && k > 2
            ...
            if typeof(u) <: Number
                ...
                vc = _vec(terk_tmp)
                for i = 2:k-2
                    @.. broadcast = false @views vc +=
                        fd_weights[i, k-2] * u_history[:, i-1]
                end
                @.. broadcast = false terk_tmp *= abs(dt^(k - 2))
            end
...
function stepsize_controller!(
    integrator,
    alg::Union{FBDF{max_order},DFBDF{max_order}},
) where {max_order}
    @unpack cache = integrator
...
@inline function stepsize_controller!(
    integrator,
    alg::Union{ExtrapolationMidpointDeuflhard,ImplicitDeuflhardExtrapolation},
)
    # Dummy function
...
function stepsize_controller_internal!(
    ...
    if iszero(integrator.EEst)
        ...
        integrator.opts.gamma =
            DiffEqBase.fastpow(typeof(integrator.opts.gamma)(1 // 4), controller.beta1)
...
function stepsize_predictor!(
    ...
    if iszero(integrator.EEst)
        ...
        # Compute new stepsize scaling
        qtmp =
            EEst * DiffEqBase.fastpow(
                DiffEqBase.fastpow(tol, (1.0 - s_curr / s_new)),
                controller.beta1,
            ) / integrator.opts.gamma
        @fastmath q = max(inv(integrator.opts.qmax), min(inv(integrator.opts.qmin), qtmp))
    end
...
function step_reject_controller!(
    ...
    dt_red =
        integrator.dt / integrator.cache.Q[integrator.cache.n_old-integrator.alg.n_min+1]
    dtmin = timedepentdtmin(integrator)
    ...
    if typeof(alg) <: Union{
        ...
        if iszero(integrator.EEst)
            ...
            if typeof(alg) <: ImplicitHairerWannerExtrapolation
                controller.beta1 =
                    typeof(controller.beta1)(1 // (2integrator.cache.n_curr + 1))
            elseif typeof(alg) <: ImplicitEulerExtrapolation
                controller.beta1 = typeof(controller.beta1)(1 // (integrator.cache.n_curr))
            else
                controller.beta1 =
                    typeof(controller.beta1)(1 // (integrator.cache.n_curr - 1))
            end
            integrator.opts.gamma = DiffEqBase.fastpow(
                typeof(integrator.opts.gamma)(65 // 100),
                controller.beta1,
            )
            # Compute new stepsize scaling
            qtmp =
                DiffEqBase.fastpow(integrator.EEst, controller.beta1) /
                (integrator.opts.gamma)
            @fastmath q =
                max(inv(integrator.opts.qmax), min(inv(integrator.opts.qmin), qtmp))
        end
