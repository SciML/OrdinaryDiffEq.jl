...
@inline function perform_step!(integrator::ODEIntegrator,cache::Feagin12ConstantCache)
  ...
  integrator.fsallast = k
  if integrator.opts.adaptive
    integrator.EEst = integrator.opts.internalnorm((dt*(k2 - k24) * adaptiveConst)./(integrator.opts.abstol+max(abs(uprev),abs(u))*integrator.opts.reltol))
  end
  @pack integrator = t,dt,u,k
end
...
@inline function perform_step!(integrator::ODEIntegrator,cache::Feagin14ConstantCache)
  ...
  u = uprev + tmp
  if integrator.opts.adaptive
    integrator.EEst = integrator.opts.internalnorm((dt*(k2 - k34) * adaptiveConst)./(integrator.opts.abstol+max(abs(uprev),abs(u))*integrator.opts.reltol))
  end
  k = f(t+dt,u) # For the interpolation, needs k at the updated point
  integrator.fsallast = k
  @pack integrator = t,dt,u,k
