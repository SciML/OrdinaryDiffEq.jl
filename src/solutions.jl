...
function ODESolution{uType,tType,isinplace}(u,
        prob::AbstractODEProblem{uType,tType,Val{isinplace}},
        alg;timeseries=[],timeseries_analytic=[],
  ...
  if !isempty(timeseries_analytic)
    errors[:final] = mean(abs.(timeseries[end]-timeseries_analytic[end]))
  ...
  return(ODESolution(timeseries,timeseries_analytic,errors,t,k,prob,alg,interp,dense))
end
