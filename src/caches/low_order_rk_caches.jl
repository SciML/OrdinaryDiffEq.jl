...
struct LLDRK46ConstantCache{T,T2} <: OrdinaryDiffEqConstantCache
  α1::T
  α2::T
  α3::T
  α4::T
...
function alg_cache(alg::LLDRK46,u,rate_prototype,uEltypeNoUnits,uBottomEltypeNoUnits,tTypeNoUnits,uprev,uprev2,f,t,dt,reltol,p,calck,::Type{Val{true}})
  tmp = similar(u)
  k = zero(rate_prototype)
  fsalfirst = zero(rate_prototype)
  tab = RK46NLConstantCache(real(uBottomEltypeNoUnits), real(tTypeNoUnits))
  LLDRK46Cache(u,uprev,k,tmp,fsalfirst,tab)
end

function alg_cache(alg::LLDRK46,u,rate_prototype,uEltypeNoUnits,uBottomEltypeNoUnits,tTypeNoUnits,uprev,uprev2,f,t,dt,reltol,p,calck,::Type{Val{false}})
  LLDRK46ConstantCache(real(uBottomEltypeNoUnits), real(tTypeNoUnits))
end
