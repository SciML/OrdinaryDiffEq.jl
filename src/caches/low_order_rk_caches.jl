...
alg_cache(
    ...
    calck,
    ::Val{false},
) where {uEltypeNoUnits,uBottomEltypeNoUnits,tTypeNoUnits} = SplitEulerConstantCache()
...
alg_cache(
    ...
    calck,
    ::Val{false},
) where {uEltypeNoUnits,uBottomEltypeNoUnits,tTypeNoUnits} = EulerConstantCache()
...
function alg_cache(
    ...
    HeunCache(u, uprev, zero(u), atmp, zero(rate_prototype), zero(rate_prototype))
end
...
function alg_cache(
    ...
    atmp = similar(u, uEltypeNoUnits)
    recursivefill!(atmp, false)
    RalstonCache(u, uprev, zero(u), atmp, zero(rate_prototype), zero(rate_prototype))
end
...
alg_cache(
    ...
    calck,
    ::Val{false},
) where {uEltypeNoUnits,uBottomEltypeNoUnits,tTypeNoUnits} = HeunConstantCache()
...
alg_cache(
    ...
    calck,
    ::Val{false},
) where {uEltypeNoUnits,uBottomEltypeNoUnits,tTypeNoUnits} = RalstonConstantCache()
...
alg_cache(
    ...
    calck,
    ::Val{false},
) where {uEltypeNoUnits,uBottomEltypeNoUnits,tTypeNoUnits} = MidpointConstantCache()
...
alg_cache(
    ...
    calck,
    ::Val{false},
) where {uEltypeNoUnits,uBottomEltypeNoUnits,tTypeNoUnits} = RK4ConstantCache()
...
@cache struct OwrenZen3Cache{uType,rateType,uNoUnitsType,TabType} <:
              OrdinaryDiffEqMutableCache
    u::uType
    uprev::uType
    k1::rateType
...
@cache struct OwrenZen4Cache{uType,rateType,uNoUnitsType,TabType} <:
              OrdinaryDiffEqMutableCache
    u::uType
    uprev::uType
    k1::rateType
...
@cache struct OwrenZen5Cache{uType,rateType,uNoUnitsType,TabType} <:
              OrdinaryDiffEqMutableCache
    u::uType
    uprev::uType
    k1::rateType
...
function alg_cache(
    ...
    KYK2014DGSSPRK_3S2_ConstantCache(
        constvalue(uBottomEltypeNoUnits),
        constvalue(tTypeNoUnits),
    )
end
