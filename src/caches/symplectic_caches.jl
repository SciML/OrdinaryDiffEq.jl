...
alg_cache(
    ...
    calck,
    ::Val{false},
) where {uEltypeNoUnits,uBottomEltypeNoUnits,tTypeNoUnits} = SymplecticEulerConstantCache()
...
function alg_cache(
    ...
    k = zero(rate_prototype)
    fsalfirst = zero(rate_prototype)
    tab = VerletLeapfrogConstantCache(
...
alg_cache(
    ...
    PseudoVerletLeapfrogConstantCache(
        constvalue(uBottomEltypeNoUnits),
        constvalue(tTypeNoUnits),
    )
