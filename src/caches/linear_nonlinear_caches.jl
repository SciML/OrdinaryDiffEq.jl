...
for (Alg, Cache) in [
    ...
    @eval function alg_cache(
        ...
        if alg.krylov
            ...
            A =
                size(f.f1.f) == () ? convert(Number, f.f1.f) :
...
function alg_cache(
    ...
    rtmp, G, du1 = (zero(rate_prototype) for i = 1:3) # rateType caches
    plist = (1,)
    uf, jac_config, J, phihA, KsCache =
        alg_cache_expRK(alg, u, uEltypeNoUnits, uprev, f, t, dt, p, du1, tmp, dz, plist) # other caches
...
function alg_cache_exprb(
    ...
    if f isa SplitFunction
        error(
            "Algorithm $alg cannnot be used for split problems. Consider reformat to a regular `ODEProblem`",
        )
    end
    n = length(u)
    T = eltype(u)
...
function alg_cache(
    ...
    rtmp, F2, du1 = (zero(rate_prototype) for i = 1:3) # rateType caches
    plist = (3, 3)
    uf, jac_config, J, KsCache =
        alg_cache_exprb(alg, u, uEltypeNoUnits, uprev, f, t, p, du1, tmp, dz, plist) # other caches
