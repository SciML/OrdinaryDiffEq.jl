...
@cache mutable struct SHLDDRK_2NCache{uType, rateType, TabType} <:
                      OrdinaryDiffEqMutableCache
    u::uType
    uprev::uType
    k::rateType
...
function SHLDDRK_2NConstantCache(T1, T2)
    ...
    c52 = T2(0.8034472)
    c62 = T2(0.9184166)
    SHLDDRK_2NConstantCache(α21, α31, α41, α51, β11, β21, β31, β41, β51, c21, c31, c41, c51,
...
struct SSPRK73ConstantCache{T, T2} <: OrdinaryDiffEqConstantCache
    ...
    function SSPRK73ConstantCache(T, T2)
        ...
        new{T, T2}(α40, α43, α50, α51, α54, α73, α76, β10, β21, β32, β43, β54, β65, β76, c1,
                   c2, c3, c4, c5, c6)
    end
end
...
@cache mutable struct SSPRKMSVS32Cache{uType, rateType, dtArrayType, dtType, StageLimiter,
                                       StepLimiter} <: OrdinaryDiffEqMutableCache
    u::uType
    uprev::uType
    fsalfirst::rateType
    u_2::uType
...
function alg_cache(alg::SSPRKMSVS32, u, rate_prototype, ::Type{uEltypeNoUnits},
    ...
    SSPRKMSVS32Cache(u, uprev, fsalfirst, u_2, u_1, k, tmp, dts, dtf, μ, 0.5,
                     alg.stage_limiter!, alg.step_limiter!, 1)
end
