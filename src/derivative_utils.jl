...
function calc_tderivative!(
    integrator::ODEIntegrator{algType,IIP,<:Array},
    cache,
    dtd1,
    repeat_step,
) where {algType,IIP}
...
"""
mutable struct WOperator{IIP,T,MType,GType,JType,F,C,JV} <:
    ...
    function WOperator{IIP}(
        ...
        if J isa Union{Number,DiffEqScalar}
            ...
            if AJ isa AbstractMatrix
                mm =
                    mass_matrix isa DiffEqArrayOperator ?
...
@noinline _throwWMerror(W, mass_matrix) =
    throw(DimensionMismatch("W: $(axes(W)), mass matrix: $(axes(mass_matrix))"))
@noinline _throwJMerror(J, mass_matrix) =
...
function calc_W!(
    W,
    integrator,
    nlsolver::Union{Nothing,AbstractNLSolver},
    cache,
    dtgamma,
    repeat_step,
    W_transform = false,
    ...
    if W_transform && DiffEqBase.has_Wfact_t(f)
        ...
        is_compos && (
            integrator.eigen_est =
                constvalue(opnorm(LowerTriangular(W), Inf)) + inv(dtgamma)
    ...
    if W isa WOperator
        ...
        set_gamma!(W, dtgamma)
        if W.J !== nothing &&
           !(W.J isa SparseDiffTools.JacVec) &&
...
@noinline function calc_W(integrator, nlsolver, dtgamma, repeat_step, W_transform = false)
    ...
    elseif DiffEqBase.has_jac(f)
        ...
        if typeof(J) <: StaticArray &&
           ...
           Union{Rosenbrock23,Rodas4,Rodas4P,Rodas4P2,Rodas5,Rodas5P}
            W = W_transform ? J - mass_matrix * inv(dtgamma) : dtgamma * J - mass_matrix
        else
        ...
        if isdae
            ...
            W_full =
                W_transform ? J - mass_matrix * inv(dtgamma) : dtgamma * J - mass_matrix
    ...
    is_compos && (
        integrator.eigen_est =
            isarray ? constvalue(opnorm(J, Inf)) : integrator.opts.internalnorm(J, t)
    )
    return W
end

function calc_rosenbrock_differentiation!(
    integrator,
    cache,
    dtd1,
    dtgamma,
...
function update_W!(
    ...
    integrator::SciMLBase.DEIntegrator{<:Any,true},
    cache,
    dtgamma,
    repeat_step::Bool,
    newJW = nothing,
...
function update_W!(
    nlsolver::AbstractNLSolver,
    integrator::SciMLBase.DEIntegrator{<:Any,false},
    cache,
    dtgamma,
    repeat_step::Bool,
    newJW = nothing,
...
function build_J_W(
    ...
    elseif (
        ...
        jacvec = SparseDiffTools.JacVec(
            UJacobianWrapper(_f, t, p),
            copy(u),
            OrdinaryDiffEqTag(),
    ...
    elseif islin || (!IIP && DiffEqBase.has_jac(f))
        ...
        elseif IIP
            ...
            if len !== nothing &&
               typeof(alg) <: Union{Rosenbrock23,Rodas4,Rodas4P,Rodas4P2,Rodas5,Rodas5P}
