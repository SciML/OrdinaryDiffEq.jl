...
function DiffEqBase.__init(prob::DiffEqBase.AbstractDAEProblem,
                           alg::DAEAlgorithm,
  ...
  if (((!(typeof(alg) <: DAEAdaptiveAlgorithm)) || !adaptive) && dt == tType(0) && isempty(tstops)) && !(typeof(alg) <: Union{FunctionMap,LinearExponential})
  ...
  integrator = DAEIntegrator{typeof(alg),isinplace(prob),uType,tType,typeof(p),typeof(eigen_est),
...
function DiffEqBase.solve!(integrator::DAEIntegrator)
