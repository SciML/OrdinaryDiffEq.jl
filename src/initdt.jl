@muladd function ode_determine_initdt(
    ...
    if prob.f.mass_matrix != I && (
        !(typeof(prob.f) <: DynamicalODEFunction) ||
    ...
    dt₀ = OrdinaryDiffEq.ArrayInterface.IfElse.ifelse(
        (d₀ < 1 // 10^(5)) | (d₁ < 1 // 10^(5)),
        smalldt,
        convert(_tType, oneunit_tType * (d₀ / d₁) / 100),
    )
...
@muladd function ode_determine_initdt(
    ...
    if Base.promote_op(/, typeof(u0), typeof(oneunit(t))) !== typeof(f₀)
        throw(
            TypeNotConstantError(
                Base.promote_op(/, typeof(u0), typeof(oneunit(t))),
                typeof(f₀),
            ),
        )
    end
