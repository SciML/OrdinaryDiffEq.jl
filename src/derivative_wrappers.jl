...
function derivative(f, x::Union{Number,AbstractArray{<:Number}}, integrator)
    local d
    tmp = length(x) # We calculate derivtive for all elements in gradient
    alg = unwrap_alg(integrator, true)
    if alg_autodiff(alg)
        ...
        d = FiniteDiff.finite_difference_derivative(
            f,
            x,
            alg_difftype(alg),
            dir = diffdir(integrator),
...
jacobian_finitediff!(J, f, x, jac_config, integrator) = (
    FiniteDiff.finite_difference_jacobian!(J, f, x, jac_config, dir = diffdir(integrator));
...
function DiffEqBase.build_jac_config(
    alg,
    f::F1,
    uf::F2,
    du1,
    ...
    tmp,
    du2,
    ::Val{transform} = Val(true),
) where {transform,F1,F2}
    ...
    if !DiffEqBase.has_jac(f) && # No Jacobian if has analytical solution
       ...
       (
           (
               concrete_jac(alg) === nothing && (
                   !haslinsolve || (
                       haslinsolve && # No Jacobian if linsolve doesn't want it
                       (
       ...
       ) # Jacobian if explicitly asked for
        ...
        if alg_autodiff(alg)
            ...
            jac_config = ForwardColorJacCache(
                uf,
                uprev,
                _chunksize;
                colorvec = colorvec,
            ...
            if alg_difftype(alg) !== Val{:complex}
                ...
                jac_config = FiniteDiff.JacobianCache(
                    Complex{eltype(tmp)}.(tmp),
                    Complex{eltype(du1)}.(du1),
                    nothing,
                    alg_difftype(alg),
                    eltype(u),
                    colorvec = colorvec,
...
function DiffEqBase.resize_jac_config!(jac_config::SparseDiffTools.ForwardColorJacCache, i)
    ...
    ps =
        SparseDiffTools.adapt.(
            DiffEqBase.parameterless_type(jac_config.dx),
            SparseDiffTools.generate_chunked_partials(
                jac_config.dx,
    ...
    if !DiffEqBase.has_tgrad(f)
        if alg_autodiff(alg)
            ...
            if du1 isa Array
                ...
                grad_config = ArrayInterface.restructure(
                    du1,
                    Dual{
                        T,
                        eltype(du1),
                        1,
                    }.(du1, (ForwardDiff.Partials((one(eltype(du1)),)),)) .* false,
                )
            end
        else
