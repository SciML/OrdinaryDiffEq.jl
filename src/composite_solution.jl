struct ODECompositeSolution{T, N, uType, uType2, EType, tType, rateType, P, A, IType, DE} <:
...
function DiffEqBase.build_solution(prob::Union{DiffEqBase.AbstractODEProblem,
    ...
    if DiffEqBase.has_analytic(f)
        ...
        sol = ODECompositeSolution{T, N, typeof(u), typeof(u_analytic), typeof(errors),
                                   typeof(t), typeof(k),
                                   typeof(prob), typeof(alg), typeof(interp),
                                   typeof(destats)}(u, u_analytic, errors, t, k, prob, alg,
                                                    interp, alg_choice, dense, 0, destats,
                                                    retcode)
        if calculate_error
        ...
        return ODECompositeSolution{T, N, typeof(u), typeof(nothing), typeof(nothing),
                                    typeof(t), typeof(k),
                                    typeof(prob), typeof(alg), typeof(interp),
                                    typeof(destats)}(u, nothing, nothing, t, k, prob, alg,
                                                     interp, alg_choice, dense, 0, destats,
                                                     retcode)
    end
end

function DiffEqBase.solution_new_retcode(sol::ODECompositeSolution{T, N, uType, uType2,
                                                                   EType, tType, rateType,
                                                                   P, A, IType, DE},
                                         retcode) where {T, N, uType, uType2, EType, tType,
                                                         rateType, P, A, IType, DE}
    ODECompositeSolution{T, N, uType, uType2, EType, tType, rateType, P, A, IType, DE}(sol.u,
                                                                                       sol.u_analytic,
                                                                                       sol.errors,
                                                                                       sol.t,
                                                                                       sol.k,
...
function DiffEqBase.sensitivity_solution(sol::ODECompositeSolution, u, t)
    ...
    ODECompositeSolution{T, N, typeof(u), typeof(sol.u_analytic), typeof(sol.errors),
                         ...
                         typeof(interp), typeof(sol.destats)}(u, sol.u_analytic, sol.errors,
                                                              t, nothing, sol.prob,
                                                              sol.alg, interp,
                                                              sol.alg_choice,
