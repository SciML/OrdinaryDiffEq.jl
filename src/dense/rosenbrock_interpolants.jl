...
@muladd function _ode_interpolant(
    ...
    @.. broadcast = false (
        k[1][idxs] + Θ * (-2 * k[1][idxs] + 2 * k[2][idxs] - 3 * k[2][idxs] * Θ) -
        y₀[idxs] + y₁[idxs]
    ) / dt
end
...
@muladd function _ode_interpolant(
    ...
    @.. broadcast = false Θ1 * y₀[idxs] +
                          Θ *
                          (y₁[idxs] + Θ1 * (k[1][idxs] + Θ * (k[2][idxs] + Θ * k[3][idxs])))
end
...
@muladd function _ode_interpolant(
    ...
    @inbounds (
        k[1] + Θ * (-2 * k[1] + 2 * k[2] + Θ * (-3 * k[2] + 3 * k[3] - 4 * Θ * k[3])) - y₀ +
        y₁
    ) / dt
end
...
@muladd function _ode_interpolant!(
    ...
    @.. broadcast = false out =
        (
            k[1] + Θ * (-2 * k[1] + 2 * k[2] + Θ * (-3 * k[2] + 3 * k[3] - 4 * Θ * k[3])) -
            y₀ + y₁
        ) / dt
    out
end
