...
@inline DiffEqBase.addsteps!(integrator::ODEIntegrator, args...) =
...
@generated function composite_ode_interpolant(
    ...
    push!(
        ...
        quote
            throw(
                "Cache $current is not available. There are only $(length(caches)) caches.",
            )
        end,
    )
    return expr
    ...
    @inbounds for j in idx
        ...
        elseif !id.dense
            if eltype(timeseries) <: AbstractArray
                ...
                vals[j] =
                    linear_interpolant(Θ, dt, timeseries[i₋], timeseries[i₊], idxs, deriv)
            end
        elseif typeof(cache) <: CompositeCache
            DiffEqBase.addsteps!(
                ks[i₊],
...
function _ode_interpolant(Θ, dt, y₀, y₁, k, cache, idxs, T::Type{Val{TI}}) where {TI}
    hermite_interpolant(
        ...
        y₁,
        k,
        Val{typeof(cache)<:OrdinaryDiffEqMutableCache},
        idxs,
        T,
    )
end
...
@muladd function hermite_interpolant(
    ...
    #@.. broadcast=false (1-Θ)*y₀+Θ*y₁+Θ*(Θ-1)*((1-2Θ)*(y₁-y₀)+(Θ-1)*dt*k[1] + Θ*dt*k[2])
    @inbounds @.. broadcast = false (1 - Θ) * y₀ +
                                    Θ * y₁ +
                                    Θ *
                                    (Θ - 1) *
                                    (
...
@muladd function hermite_interpolant!(out, Θ, dt, y₀, y₁, k, idxs, T::Type{Val{0}}) # Default interpolant is Hermite
    @views @.. broadcast = false out =
        ...
        Θ *
        (Θ - 1) *
        ((1 - 2Θ) * (y₁[idxs] - y₀[idxs]) + (Θ - 1) * dt * k[1][idxs] + Θ * dt * k[2][idxs])
end
...
@muladd function hermite_interpolant(Θ, dt, y₀, y₁, k, cache, idxs, T::Type{Val{2}}) # Default interpolant is Hermite
    ...
    @views out =
        (
            -4 * dt * k[1][idxs] - 2 * dt * k[2][idxs] - 6 * y₀[idxs] +
            Θ *
            (6 * dt * k[1][idxs] + 6 * dt * k[2][idxs] + 12 * y₀[idxs] - 12 * y₁[idxs]) +
            6 * y₁[idxs]
        ) / (dt * dt)
    out
end
...
@muladd function hermite_interpolant(
    ...
    #@.. broadcast=false (6*dt*k[1] + 6*dt*k[2] + 12*y₀ - 12*y₁)/(dt*dt*dt)
    @inbounds @.. broadcast = false (6 * dt * k[1] + 6 * dt * k[2] + 12 * y₀ - 12 * y₁) /
                                    (dt * dt * dt)
end
...
@muladd function hermite_interpolant!(
    ...
    @inbounds @simd ivdep for i in eachindex(out)
        out[i] =
            (6 * dt * k[1][i] + 6 * dt * k[2][i] + 12 * y₀[i] - 12 * y₁[i]) / (dt * dt * dt)
    end
    out
end
