...
function DiffEqBase.addsteps!(
    ...
    if length(k) < 2 || always_calc_begin
        ...
        linres =
            dolinsolve(cache, linres.cache; b = _vec(linsolve_tmp), reltol = cache.reltol)
        vecu = _vec(linres.u)
        veck2 = _vec(k₂)
...
function DiffEqBase.addsteps!(
    ...
    if length(k) < 2 || always_calc_begin
        ...
        @.. broadcast = false veck5 = -vecu
        @.. broadcast = false tmp =
            uprev + a61 * k1 + a62 * k2 + a63 * k3 + a64 * k4 + a65 * k5
        f(du, tmp, p, t + dt)

        if mass_matrix === I
            @.. broadcast = false linsolve_tmp =
                du + (dtC62 * k2 + dtC64 * k4 + dtC61 * k1 + dtC63 * k3 + dtC65 * k5)
        else
            @.. broadcast = false du1 =
                dtC62 * k2 + dtC64 * k4 + dtC61 * k1 + dtC63 * k3 + dtC65 * k5
            mul!(du2, mass_matrix, du1)
            @.. broadcast = false linsolve_tmp = du + du2
        end
        ...
        @.. broadcast = false tmp =
            uprev + a61 * k1 + a62 * k2 + a63 * k3 + a64 * k4 + a65 * k5 + k6
        f(du, tmp, p, t + dt)
        ...
        if mass_matrix === I
            ...
            @.. broadcast = false du1 =
                dtC72 * k2 + dtC74 * k4 + dtC71 * k1 + dtC73 * k3 + dtC75 * k5 + dtC76 * k6
            mul!(du2, mass_matrix, du1)
            @.. broadcast = false linsolve_tmp = du + du2
        end
        ...
        @.. broadcast = false tmp =
            uprev + a71 * k1 + a72 * k2 + a73 * k3 + a74 * k4 + a75 * k5 + a76 * k6 + k7
        f(du, tmp, p, t + dt)
