...
@muladd function _ode_interpolant(
    ...
    @dp5pre0
    @inbounds @.. broadcast = false y₀ +
                                    dt * (k[1] * b10 + k[2] * b20 + k[3] * b30 + k[4] * b40)
end
...
@muladd function _ode_interpolant(
    ...
    y₁,
    k,
    cache::Union{DP5ConstantCache,DP5Cache},
    idxs,
    T::Type{Val{0}},
)
    @dp5pre0
    @views @.. broadcast = false y₀[idxs] +
                                 dt * (
        k[1][idxs] * b10 + k[2][idxs] * b20 + k[3][idxs] * b30 + k[4][idxs] * b40
    )
end
...
@muladd function _ode_interpolant!(
    ...
    @views @.. broadcast = false out =
        y₀[idxs] +
        dt * (k[1][idxs] * b10 + k[2][idxs] * b20 + k[3][idxs] * b30 + k[4][idxs] * b40)
    out
end
...
@muladd function _ode_interpolant(
    ...
    @dp5pre1
    @views @.. broadcast = false k[1][idxs] +
                                 k[2][idxs] * b20diff +
                                 k[3][idxs] * b30diff +
                                 k[4][idxs] * b40diff
end
...
@muladd function _ode_interpolant!(
    ...
    @dp5pre1
    @views @.. broadcast = false out =
        k[1][idxs] + k[2][idxs] * b20diff + k[3][idxs] * b30diff + k[4][idxs] * b40diff
    out
end
...
@muladd function _ode_interpolant(
    ...
    @dp5pre2
    @inbounds @.. broadcast = false (k[2] * b20diff2 + k[3] * b30diff2 + k[4] * b40diff2) *
                                    invdt
end
...
@muladd function _ode_interpolant(
    ...
    @dp5pre2
    @views @.. broadcast = false (
        k[2][idxs] * b20diff2 + k[3][idxs] * b30diff2 + k[4][idxs] * b40diff2
    ) * invdt
end
...
@muladd function _ode_interpolant!(
    ...
    @dp5pre2
    @views @.. broadcast = false out =
        (k[2][idxs] * b20diff2 + k[3][idxs] * b30diff2 + k[4][idxs] * b40diff2) * invdt
    out
end
...
@muladd function _ode_interpolant!(
    ...
    @dp5pre3
    @views @.. broadcast = false out =
        (k[3][idxs] * b30diff3 + k[4][idxs] * b40diff3) * invdt2
    out
end
...
@muladd function _ode_interpolant!(
    ...
    @ssprkpre1
    @views @.. broadcast = false out =
        (y₁[idxs] - y₀[idxs]) * c10diffinvdt + k[1][idxs] * b10diff
    out
end
...
@muladd function _ode_interpolant(
    ...
    @ssprkpre2
    @views @.. broadcast = false (y₁[idxs] - y₀[idxs]) * c10diff2invdt2 +
...
@muladd function _ode_interpolant!(
    ...
    @ssprkpre2
    @views @.. broadcast = false out =
        (y₁[idxs] - y₀[idxs]) * c10diff2invdt2 + k[1][idxs] * b10diff2invdt
    out
end
...
@muladd function _ode_interpolant(
    ...
    @tsit5pre0
    return @inbounds @.. broadcast = false y₀ +
                                           dt * (
        k[1] * b1Θ +
        k[2] * b2Θ +
        k[3] * b3Θ +
...
@muladd function _ode_interpolant(
    ...
    @tsit5pre1
    return @inbounds @.. broadcast = false k[1] * b1Θdiff +
                                           k[2] * b2Θdiff +
                                           k[3] * b3Θdiff +
                                           k[4] * b4Θdiff +
                                           k[5] * b5Θdiff +
...
@muladd function _ode_interpolant(
    ...
    @views @.. broadcast = false y₀[idxs] +
                                 dt * (
        k[1][idxs] * b1Θ + k[2][idxs] * b2Θ + k[3][idxs] * b3Θ + k[4][idxs] * b4Θ
    )
end
...
@muladd function _ode_interpolant!(
    ...
    @views @.. broadcast = false out =
        y₀[idxs] +
        dt * (k[1][idxs] * b1Θ + k[2][idxs] * b2Θ + k[3][idxs] * b3Θ + k[4][idxs] * b4Θ)
    out
end
...
@muladd function _ode_interpolant(
    ...
    @owrenzen3pre1
    @inbounds @.. broadcast = false k[1] * b1Θdiff +
                                    k[2] * b2Θdiff +
                                    k[3] * b3Θdiff +
                                    k[4] * b4Θdiff
end
...
@muladd function _ode_interpolant(
    ...
    @owrenzen3pre1
    @views @.. broadcast = false k[1][idxs] * b1Θdiff +
                                 k[2][idxs] * b2Θdiff +
                                 k[3][idxs] * b3Θdiff +
                                 k[4][idxs] * b4Θdiff
end
...
@muladd function _ode_interpolant(
    ...
    @inbounds @.. broadcast = false (
        k[1] * b1Θdiff2 + k[2] * b2Θdiff2 + k[3] * b3Θdiff2 + k[4] * b4Θdiff2
    ) * invdt
end
...
@muladd function _ode_interpolant(
    ...
    @owrenzen3pre3
    @inbounds @.. broadcast = false (
        k[1] * b1Θdiff3 + k[2] * b2Θdiff3 + k[3] * b3Θdiff3 + k[4] * b4Θdiff3
    ) * invdt2
end
...
@def owrenzen4unpack begin
    if typeof(cache) <: OrdinaryDiffEqMutableCache
        @unpack r14, r13, r12, r34, r33, r32, r44, r43, r42, r54, r53, r52, r64, r63, r62 =
            cache.tab
    else
        @unpack r14, r13, r12, r34, r33, r32, r44, r43, r42, r54, r53, r52, r64, r63, r62 =
            cache
...
@muladd function _ode_interpolant!(
    ...
    @owrenzen4pre0
    @inbounds @.. broadcast = false out =
        y₀ + dt * (k[1] * b1Θ + k[3] * b3Θ + k[4] * b4Θ + k[5] * b5Θ + k[6] * b6Θ)
    #@inbounds for i in eachindex(out)
...
@muladd function _ode_interpolant(
    ...
    y₁,
    k,
    cache::Union{BS5ConstantCache,BS5Cache},
    idxs,
    T::Type{Val{0}},
)
    @bs5pre0
           ...
           (
               ...
               Θ1 * (
                   ...
                   Θ * (
                       k[3][idxs] +
                       Θ1 *
                       (k[4][idxs] + Θ * (k[5][idxs] + Θ1 * (k[6][idxs] + Θ * k[7][idxs])))
                   )
               )
           )
end
...
@muladd function _ode_interpolant!(
    ...
    @inbounds @.. broadcast = false out =
        ...
        (
            k[1] +
            Θ1 * (k[2] + Θ * (k[3] + Θ1 * (k[4] + Θ * (k[5] + Θ1 * (k[6] + Θ * k[7])))))
        )
    #@inbounds for i in eachindex(out)
...
@muladd function _ode_interpolant(
    ...
    b3diff = @.. broadcast = false -3 * k[3][idxs] - 6 * k[4][idxs] +
                          3 * k[5][idxs] +
                          3 * k[6][idxs]
    b4diff = @.. broadcast = false 4 * k[4][idxs] - 8 * k[5][idxs] - 12 * k[6][idxs] +
                          4 * k[7][idxs]
            ...
            Θ * (
                ...
                Θ * (
                    ...
                    Θ * (
                        ...
                        Θ * (-6 * k[6][idxs] + 18 * k[7][idxs] - 7 * k[7][idxs] * Θ)
                    )
                )
            )
        )
...
@muladd function _ode_interpolant(
    ...
    return ArrayPartition(
        duprev + dt * Θ * (bp1Θ * k1 + bp3Θ * k3 + bp4Θ * k4 + bp5Θ * k5 + bp6Θ * k6),
        uprev +
        dt * Θ * (duprev + dt * Θ * (b1Θ * k1 + b3Θ * k3 + b4Θ * k4 + b5Θ * k5 + b6Θ * k6)),
    )
end
...
@muladd function _ode_interpolant!(
    ...
    @dprkn6pre0
    @inbounds @.. broadcast = false out.x[2] =
        uprev +
        dt * Θ * (duprev + dt * Θ * (b1Θ * k1 + b3Θ * k3 + b4Θ * k4 + b5Θ * k5 + b6Θ * k6))
    @inbounds @.. broadcast = false out.x[1] =
        duprev + dt * Θ * (bp1Θ * k1 + bp3Θ * k3 + bp4Θ * k4 + bp5Θ * k5 + bp6Θ * k6)
