...
"""
"""
@def tsit5testunpack begin
    var"#T#" = constvalue(recursive_unitless_bottom_eltype(y₁))
    r11, r12, r13, r14, r22, r23, r24, r32, r33, r34, r42, r43, r44, r52, r53, r54, r62, r63, r64, r72, r73, r74 = Tsit5TestConstantCacheActual(var"#T#")
end

@def tsit5testpre0 begin
    @tsit5testunpack
    Θ² = Θ * Θ
    b1Θ = Θ * @evalpoly(Θ, r11, r12, r13, r14)
    b2Θ = Θ² * @evalpoly(Θ, r22, r23, r24)
...
@muladd function _ode_interpolant(Θ, dt, y₀, y₁, k, cache::Tsit5TestConstantCache,
                                  idxs::Nothing, T::Type{Val{0}})
    @tsit5testpre0
    #@.. broadcast=false y₀ + dt*(k[1]*b1Θ + k[2]*b2Θ + k[3]*b3Θ + k[4]*b4Θ + k[5]*b5Θ + k[6]*b6Θ + k[7]*b7Θ)
    return @inbounds y₀ +
...
@muladd function _ode_interpolant(Θ, dt, y₀, y₁, k, cache::Tsit5TestCache, idxs::Nothing,
                                  T::Type{Val{0}})
    @tsit5testpre0
    return @inbounds @.. broadcast=false y₀+dt * (k[1] * b1Θ + k[2] * b2Θ + k[3] * b3Θ +
                                             k[4] * b4Θ +
...
@muladd function _ode_interpolant(Θ, dt, y₀, y₁, k,
                                  cache::Union{Tsit5TestConstantCache, Tsit5TestCache}, idxs,
                                  T::Type{Val{0}})
    @tsit5testpre0
    return y₀[idxs] +
           dt * (k[1][idxs] * b1Θ + k[2][idxs] * b2Θ + k[3][idxs] * b3Θ +
...
@muladd function _ode_interpolant!(out, Θ, dt, y₀, y₁, k,
                                   cache::Union{Tsit5TestConstantCache, Tsit5TestCache},
                                   idxs::Nothing, T::Type{Val{0}})
    @tsit5testpre0
    @inbounds @.. broadcast=false out=y₀ +
                                      dt *
