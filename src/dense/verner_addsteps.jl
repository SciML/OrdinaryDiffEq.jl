...
@muladd function DiffEqBase.addsteps!(k,t,uprev,u,dt,f,p,cache::Vern9Cache,always_calc_begin = false,allow_calc_end = true,force_calc_end = false)
  if length(k) < 16 || always_calc_begin
  ...
  if (allow_calc_end && length(k)< 26) || force_calc_end # Have not added the extra stages yet
    rtmp = similar(cache.k1)
    @unpack tmp = cache
...
@muladd function DiffEqBase.addsteps!(k,t,uprev,u,dt,f,p,cache::Vern9ConstantCache,always_calc_begin = false,allow_calc_end = true,force_calc_end = false)
  if (length(k) < 10 || always_calc_begin) || ((allow_calc_end && length(k)< 20) || force_calc_end)
  end
  if length(k) < 10 || always_calc_begin
