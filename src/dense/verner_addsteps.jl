...
@muladd function _ode_addsteps!(k, t, uprev, u, dt, f, p, cache::Vern7Cache{<:Array},
    ...
    if length(k) < 10 || always_calc_begin
        ...
        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a061 * k1[i] + a063 * k3[i] + a064 * k4[i] + a065 * k5[i])
        end
        f(k6, tmp, p, t + c6 * dt)

        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a071 * k1[i] + a073 * k3[i] + a074 * k4[i] + a075 * k5[i] +
                      a076 * k6[i])
        end
        f(k7, tmp, p, t + c7 * dt)
        ...
        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a081 * k1[i] + a083 * k3[i] + a084 * k4[i] + a085 * k5[i] +
                      a086 * k6[i] + a087 * k7[i])
        end
        f(k8, tmp, p, t + c8 * dt)
        ...
        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a091 * k1[i] + a093 * k3[i] + a094 * k4[i] + a095 * k5[i] +
                      a096 * k6[i] + a097 * k7[i] + a098 * k8[i])
        end
        f(k9, tmp, p, t + dt)

        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a101 * k1[i] + a103 * k3[i] + a104 * k4[i] + a105 * k5[i] +
                      a106 * k6[i] + a107 * k7[i])
        end
        f(k10, tmp, p, t + dt)
...
@muladd function _ode_addsteps!(k, t, uprev, u, dt, f, p, cache::Vern8Cache,
    ...
    if length(k) < 13 || always_calc_begin
        ...
        f(k7, tmp, p, t + c7 * dt)
        @.. broadcast=false tmp=uprev +
                                dt * (a0801 * k1 + a0804 * k4 + a0805 * k5 + a0806 * k6 +
                                 a0807 * k7)
        f(k8, tmp, p, t + c8 * dt)
        @.. broadcast=false tmp=uprev +
...
@muladd function _ode_addsteps!(k, t, uprev, u, dt, f, p, cache::Vern9Cache,
    ...
    if length(k) < 10 || always_calc_begin
        ...
        f(k9, tmp, p, t + c8 * dt)
        @.. broadcast=false tmp=uprev +
                                dt * (a1001 * k1 + a1006 * k6 + a1007 * k7 + a1008 * k8 +
                                 a1009 * k9)
        f(k10, tmp, p, t + c9 * dt)
        @.. broadcast=false tmp=uprev +
        ...
        f(k15, tmp, p, t + dt)
        @.. broadcast=false tmp=uprev +
                                dt * (a1601 * k1 + a1606 * k6 + a1607 * k7 + a1608 * k8 +
...
@muladd function _ode_addsteps!(k, t, uprev, u, dt, f, p, cache::Vern6ConstantCache,
    ...
    if length(k) < 9 || always_calc_begin
        ...
        copyat_or_push!(k, 5,
                        f(uprev + dt * (a51 * k[1] + a53 * k[3] + a54 * k[4]), p,
                          t + c4 * dt))
        copyat_or_push!(k, 6,
                        f(uprev + dt * (a61 * k[1] + a63 * k[3] + a64 * k[4] + a65 * k[5]),
                          p, t + c5 * dt))
        copyat_or_push!(k, 7,
                        f(uprev +
                          dt *
...
@muladd function _ode_addsteps!(k, t, uprev, u, dt, f, p, cache::Vern7ConstantCache,
    ...
    if length(k) < 10 || always_calc_begin
        ...
        copyat_or_push!(k, 5,
                        f(uprev + dt * (a051 * k[1] + a053 * k[3] + a054 * k[4]), p,
                          t + c5 * dt))
        copyat_or_push!(k, 6,
                        f(uprev +
        ...
        copyat_or_push!(k, 7,
                        f(uprev +
                          dt * (a071 * k[1] + a073 * k[3] + a074 * k[4] + a075 * k[5] +
                           a076 * k[6]), p, t + c7 * dt))
        copyat_or_push!(k, 8,
                        f(uprev +
                          dt * (a081 * k[1] + a083 * k[3] + a084 * k[4] + a085 * k[5] +
                           a086 * k[6] + a087 * k[7]), p, t + c8 * dt))
        copyat_or_push!(k, 9,
                        f(uprev +
                          dt * (a091 * k[1] + a093 * k[3] + a094 * k[4] + a095 * k[5] +
                           a096 * k[6] + a097 * k[7] + a098 * k[8]), p, t + dt))
        copyat_or_push!(k, 10,
                        f(uprev +
                          dt * (a101 * k[1] + a103 * k[3] + a104 * k[4] + a105 * k[5] +
...
@muladd function _ode_addsteps!(k, t, uprev, u, dt, f, p, cache::Vern8ConstantCache,
    ...
    if length(k) < 13 || always_calc_begin
        ...
        copyat_or_push!(k, 5,
                        f(uprev + dt * (a0501 * k[1] + a0503 * k[3] + a0504 * k[4]), p,
                          t + c5 * dt))
        copyat_or_push!(k, 6,
                        f(uprev + dt * (a0601 * k[1] + a0604 * k[4] + a0605 * k[5]), p,
                          t + c6 * dt))
        copyat_or_push!(k, 7,
                        f(uprev +
        ...
        copyat_or_push!(k, 13,
                        f(uprev +
                          dt *
                          (a1301 * k[1] + a1304 * k[4] + a1305 * k[5] + a1306 * k[6] +
                           a1307 * k[7] + a1308 * k[8] + a1309 * k[9] + a1310 * k[10]), p,
                          t + dt))
    end
    if (allow_calc_end && length(k) < 21) || force_calc_end # Have not added the extra stages yet
        (;c14, a1401, a1406, a1407, a1408, a1409, a1410, a1411, a1412, c15, a1501, a1506, a1507, a1508, a1509, a1510, a1511, a1512, a1514, c16, a1601, a1606, a1607, a1608, a1609, a1610, a1611, a1612, a1614, a1615, c17, a1701, a1706, a1707, a1708, a1709, a1710, a1711, a1712, a1714, a1715, a1716, c18, a1801, a1806, a1807, a1808, a1809, a1810, a1811, a1812, a1814, a1815, a1816, a1817, c19, a1901, a1906, a1907, a1908, a1909, a1910, a1911, a1912, a1914, a1915, a1916, a1917, c20, a2001, a2006, a2007, a2008, a2009, a2010, a2011, a2012, a2014, a2015, a2016, a2017, c21, a2101, a2106, a2107, a2108, a2109, a2110, a2111, a2112, a2114, a2115, a2116, a2117) = cache.tab.extra
    ...
    if length(k) < 10 || always_calc_begin
        ...
        copyat_or_push!(k, 7,
                        f(uprev +
                          ...
                          p, t + c6 * dt))
        copyat_or_push!(k, 2,
                        f(uprev + dt * (a0801 * k[1] + a0806 * k[6] + a0807 * k[7]), p,
                          t + c7 * dt))
        copyat_or_push!(k, 3,
                        f(uprev +
        ...
        copyat_or_push!(k, 5,
                        f(uprev +
                          ...
                          (a1101 * k[1] + a1106 * k[6] + a1107 * k[7] + a1108 * k[2] +
                           a1109 * k[3] + a1110 * k[4]), p, t + c10 * dt))
        ...
        copyat_or_push!(k, 6,
                        f(uprev +
                          ...
                          (a1201 * k[1] + a1206 * temp6 + a1207 * temp7 + a1208 * k[2] +
                           a1209 * k[3] + a1210 * k[4] + a1211 * k[5]), p, t + c11 * dt))
        copyat_or_push!(k, 7,
                        f(uprev +
                          dt *
                          (a1301 * k[1] + a1306 * temp6 + a1307 * temp7 + a1308 * k[2] +
                           a1309 * k[3] + a1310 * k[4] + a1311 * k[5] + a1312 * k[6]), p,
                          t + c12 * dt))
        copyat_or_push!(k, 8,
                        f(uprev +
                          dt *
