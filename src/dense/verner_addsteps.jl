@muladd function DiffEqBase.addsteps!(
    k,
    t,
    uprev,
    u,
    ...
    if length(k) < 9 || always_calc_begin
        ...
        f(k8, tmp, p, t + dt)
        @.. broadcast = false tmp =
            uprev + dt * (a91 * k1 + a94 * k4 + a95 * k5 + a96 * k6 + a97 * k7 + a98 * k8)
        f(k9, tmp, p, t + dt)
...
@muladd function DiffEqBase.addsteps!(
    ...
    if length(k) < 10 || always_calc_begin
        ...
        f(k5, tmp, p, t + c5 * dt)
        @.. broadcast = false tmp =
            uprev + dt * (a061 * k1 + a063 * k3 + a064 * k4 + a065 * k5)
        f(k6, tmp, p, t + c6 * dt)
        @.. broadcast = false tmp =
...
@muladd function DiffEqBase.addsteps!(
    ...
    if length(k) < 10 || always_calc_begin
        ...
        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] =
                uprev[i] + dt * (a061 * k1[i] + a063 * k3[i] + a064 * k4[i] + a065 * k5[i])
        end
        f(k6, tmp, p, t + c6 * dt)
...
@muladd function DiffEqBase.addsteps!(
    ...
    if length(k) < 13 || always_calc_begin
        ...
        f(k6, tmp, p, t + c6 * dt)
        @.. broadcast = false tmp =
            uprev + dt * (a0701 * k1 + a0704 * k4 + a0705 * k5 + a0706 * k6)
        f(k7, tmp, p, t + c7 * dt)
        @.. broadcast = false tmp =
            uprev + dt * (a0801 * k1 + a0804 * k4 + a0805 * k5 + a0806 * k6 + a0807 * k7)
        f(k8, tmp, p, t + c8 * dt)
        @.. broadcast = false tmp =
            uprev +
            dt *
...
@muladd function DiffEqBase.addsteps!(
    ...
    if length(k) < 10 || always_calc_begin
        ...
        a1612,
        a1613 = cache.tab
        @unpack k1, k2, k3, k4, k5, k6, k7, k8, k9, k10, k11, k12, k13, k14, k15, k16, tmp =
            cache
        uidx = eachindex(uprev)
        f(k1, uprev, p, t)
        @.. broadcast = false tmp = uprev + dt * (a0201 * k1)
        ...
        f(k8, tmp, p, t + c7 * dt)
        @.. broadcast = false tmp =
            uprev + dt * (a0901 * k1 + a0906 * k6 + a0907 * k7 + a0908 * k8)
        f(k9, tmp, p, t + c8 * dt)
        @.. broadcast = false tmp =
...
@muladd function DiffEqBase.addsteps!(
    ...
    if length(k) < 9 || always_calc_begin
        ...
        copyat_or_push!(
            k,
            5,
            f(uprev + dt * (a51 * k[1] + a53 * k[3] + a54 * k[4]), p, t + c4 * dt),
        )
        copyat_or_push!(
            k,
            6,
        ...
        copyat_or_push!(
            ...
            f(
                uprev +
                dt * (a71 * k[1] + a73 * k[3] + a74 * k[4] + a75 * k[5] + a76 * k[6]),
                p,
                t + c6 * dt,
            ),
        )
...
@muladd function DiffEqBase.addsteps!(
    ...
    if length(k) < 10 || always_calc_begin
        ...
        copyat_or_push!(
            k,
            5,
            f(uprev + dt * (a051 * k[1] + a053 * k[3] + a054 * k[4]), p, t + c5 * dt),
        )
        copyat_or_push!(
            k,
            6,
            f(
                uprev + dt * (a061 * k[1] + a063 * k[3] + a064 * k[4] + a065 * k[5]),
                p,
                t + c6 * dt,
            ),
        )
        copyat_or_push!(
            ...
            f(
                uprev +
                dt * (a071 * k[1] + a073 * k[3] + a074 * k[4] + a075 * k[5] + a076 * k[6]),
                p,
                t + c7 * dt,
            ),
        )
...
@muladd function DiffEqBase.addsteps!(
    ...
    if length(k) < 13 || always_calc_begin
        ...
        copyat_or_push!(
            k,
            5,
            f(uprev + dt * (a0501 * k[1] + a0503 * k[3] + a0504 * k[4]), p, t + c5 * dt),
        )
        copyat_or_push!(
            k,
            6,
            f(uprev + dt * (a0601 * k[1] + a0604 * k[4] + a0605 * k[5]), p, t + c6 * dt),
        )
        copyat_or_push!(
            k,
            7,
            f(
                uprev + dt * (a0701 * k[1] + a0704 * k[4] + a0705 * k[5] + a0706 * k[6]),
                p,
                t + c7 * dt,
            ),
        )
        copyat_or_push!(
            ...
            f(
                uprev +
                dt *
                (a0801 * k[1] + a0804 * k[4] + a0805 * k[5] + a0806 * k[6] + a0807 * k[7]),
                p,
                t + c8 * dt,
            ),
        )
...
@muladd function DiffEqBase.addsteps!(
    ...
    if length(k) < 10 || always_calc_begin
        ...
        copyat_or_push!(
            k,
            5,
            f(uprev + dt * (a0501 * k[1] + a0503 * k[3] + a0504 * k[4]), p, t + c4 * dt),
        )
        copyat_or_push!(
            k,
            6,
            f(uprev + dt * (a0601 * k[1] + a0604 * k[4] + a0605 * k[5]), p, t + c5 * dt),
        )
        copyat_or_push!(
            k,
            7,
        ...
        copyat_or_push!(
            k,
            2,
            f(uprev + dt * (a0801 * k[1] + a0806 * k[6] + a0807 * k[7]), p, t + c7 * dt),
        )
        copyat_or_push!(
            k,
            3,
            f(
                uprev + dt * (a0901 * k[1] + a0906 * k[6] + a0907 * k[7] + a0908 * k[2]),
                p,
                t + c8 * dt,
            ),
        )
        copyat_or_push!(
            ...
            f(
                uprev +
                dt *
                (a1001 * k[1] + a1006 * k[6] + a1007 * k[7] + a1008 * k[2] + a1009 * k[3]),
                p,
                t + c9 * dt,
            ),
        )
