...
function DiffEqBase.__init(
    ...
    dt = alg isa FunctionMap && isempty(tstops) ? eltype(prob.tspan)(1) :
         eltype(prob.tspan)(0),
    dtmin = nothing,
    ...
    progress &&
        @logmsg(LogLevel(-1), progress_name, _id = _id = :OrdinaryDiffEq, progress = 0)
    ...
    if save_idxs === nothing
        timeseries =
            timeseries_init === () ? uType[] : convert(Vector{uType}, timeseries_init)
    else
        u_initial = u[save_idxs]
        timeseries =
            timeseries_init === () ? typeof(u_initial)[] :
...
function handle_dt!(integrator)
    if iszero(integrator.dt) && integrator.opts.adaptive
        ...
        if sign(integrator.dt) != integrator.tdir &&
           !iszero(integrator.dt) &&
           !isnan(integrator.dt)
            error(
                "Automatic dt setting has the wrong sign. Exiting. Please report this error.",
            )
        end
...
function initialize_saveat(::Type{T}, saveat, tspan) where {T}
    ...
    if typeof(saveat) <: Number
        ...
        for t = (t0+directional_saveat):directional_saveat:tf
