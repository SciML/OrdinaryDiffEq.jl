...
function DiffEqBase.interp_summary(::Type{cacheType}, dense::Bool) where cacheType<:Union{DP5ConstantCache,DP5Cache}
...
function DiffEqBase.interp_summary(::Type{cacheType}, dense::Bool) where cacheType<:Union{Rodas4ConstantCache,Rodas4Cache}
  dense ? "specialized 3rd order \"free\" stiffness-aware interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(::Type{cacheType}, dense::Bool) where cacheType<:Union{Rosenbrock5ConstantCache,Rosenbrock5Cache}
  dense ? "specialized 4rd order \"free\" stiffness-aware interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(::Type{cacheType}, dense::Bool) where cacheType<:Union{SSPRK22,SSPRK22ConstantCache,SSPRK33,SSPRK33ConstantCache,SSPRK43,SSPRK43ConstantCache,SSPRK432,SSPRK432ConstantCache}
  dense ? "2nd order \"free\" SSP interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(::Type{cacheType}, dense::Bool) where cacheType<:Union{OwrenZen3Cache,OwrenZen3ConstantCache}
  dense ? "specialized 3rd order \"free\" interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(::Type{cacheType}, dense::Bool) where cacheType<:Union{OwrenZen4Cache,OwrenZen4ConstantCache}
  dense ? "specialized 4th order \"free\" interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(::Type{cacheType}, dense::Bool) where cacheType<:Union{OwrenZen5Cache,OwrenZen5ConstantCache}
  dense ? "specialized 5th order \"free\" interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(::Type{cacheType}, dense::Bool) where cacheType<:Union{Tsit5Cache,Tsit5ConstantCache}
  dense ? "specialized 4th order \"free\" interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(::Type{cacheType}, dense::Bool) where cacheType<:Union{BS5ConstantCache,BS5Cache}
  dense ? "specialized 5th order lazy interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(::Type{cacheType}, dense::Bool) where cacheType<:Union{Vern6Cache,Vern6ConstantCache}
  dense ? "specialized 6th order lazy interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(::Type{cacheType}, dense::Bool) where cacheType<:Union{Vern7Cache,Vern7ConstantCache}
  dense ? "specialized 7th order lazy interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(::Type{cacheType}, dense::Bool) where cacheType<:Union{Vern8Cache,Vern8ConstantCache}
  dense ? "specialized 8th order lazy interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(::Type{cacheType}, dense::Bool) where cacheType<:Union{Vern9Cache,Vern9ConstantCache}
  dense ? "specialized 9th order lazy interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(::Type{cacheType}, dense::Bool) where cacheType<:Union{DP8ConstantCache,DP8Cache}
...
function DiffEqBase.interp_summary(::Type{cacheType}, dense::Bool) where cacheType<:Union{DPRKN6ConstantCache,DPRKN6Cache}
