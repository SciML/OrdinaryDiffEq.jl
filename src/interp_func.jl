...
function DiffEqBase.interp_summary(
    interp::OrdinaryDiffEqInterpolation{cacheType},
) where {cacheType}
    DiffEqBase.interp_summary(cacheType, interp.dense)
end
DiffEqBase.interp_summary(
    ::Type{cacheType},
    dense::Bool,
) where {cacheType<:FunctionMapConstantCache} = "left-endpoint piecewise constant"
DiffEqBase.interp_summary(
    ::Type{cacheType},
    dense::Bool,
) where {cacheType<:FunctionMapCache} = "left-endpoint piecewise constant"
function DiffEqBase.interp_summary(
    ::Type{cacheType},
    dense::Bool,
) where {cacheType<:Union{DP5ConstantCache,DP5Cache}}
    dense ? "specialized 4th order \"free\" interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(
    ::Type{cacheType},
    dense::Bool,
) where {cacheType<:Union{
...
function DiffEqBase.interp_summary(
    ::Type{cacheType},
    dense::Bool,
) where {cacheType<:Union{Rodas4ConstantCache,Rodas4Cache}}
...
function DiffEqBase.interp_summary(
    ::Type{cacheType},
    dense::Bool,
) where {cacheType<:Union{Rosenbrock5ConstantCache,Rosenbrock5Cache}}
...
function DiffEqBase.interp_summary(
    ::Type{cacheType},
    dense::Bool,
) where {cacheType<:Union{OwrenZen3Cache,OwrenZen3ConstantCache}}
    dense ? "specialized 3rd order \"free\" interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(
    ::Type{cacheType},
    dense::Bool,
) where {cacheType<:Union{OwrenZen4Cache,OwrenZen4ConstantCache}}
    dense ? "specialized 4th order \"free\" interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(
    ::Type{cacheType},
    dense::Bool,
) where {cacheType<:Union{OwrenZen5Cache,OwrenZen5ConstantCache}}
    dense ? "specialized 5th order \"free\" interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(
    ::Type{cacheType},
    dense::Bool,
) where {cacheType<:Union{Tsit5Cache,Tsit5ConstantCache}}
    dense ? "specialized 4th order \"free\" interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(
    ::Type{cacheType},
    dense::Bool,
) where {cacheType<:Union{BS5ConstantCache,BS5Cache}}
    dense ? "specialized 5th order lazy interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(
    ::Type{cacheType},
    dense::Bool,
) where {cacheType<:Union{Vern6Cache,Vern6ConstantCache}}
    dense ? "specialized 6th order lazy interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(
    ::Type{cacheType},
    dense::Bool,
) where {cacheType<:Union{Vern7Cache,Vern7ConstantCache}}
    dense ? "specialized 7th order lazy interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(
    ::Type{cacheType},
    dense::Bool,
) where {cacheType<:Union{Vern8Cache,Vern8ConstantCache}}
    dense ? "specialized 8th order lazy interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(
    ::Type{cacheType},
    dense::Bool,
) where {cacheType<:Union{Vern9Cache,Vern9ConstantCache}}
    dense ? "specialized 9th order lazy interpolation" : "1st order linear"
end
function DiffEqBase.interp_summary(
    ::Type{cacheType},
    dense::Bool,
) where {cacheType<:Union{DP8ConstantCache,DP8Cache}}
...
function DiffEqBase.interp_summary(
    ::Type{cacheType},
    dense::Bool,
) where {cacheType<:Union{DPRKN6ConstantCache,DPRKN6Cache}}
    dense ? "specialized 6th order interpolation" : "1st order linear"
end

(interp::InterpolationData)(tvals, idxs, deriv, p, continuity::Symbol = :left) =
    ode_interpolation(tvals, interp, idxs, deriv, p, continuity)
(interp::CompositeInterpolationData)(tvals, idxs, deriv, p, continuity::Symbol = :left) =
    ode_interpolation(tvals, interp, idxs, deriv, p, continuity)
(interp::InterpolationData)(val, tvals, idxs, deriv, p, continuity::Symbol = :left) =
    ode_interpolation!(val, tvals, interp, idxs, deriv, p, continuity)
(interp::CompositeInterpolationData)(
    val,
    tvals,
    idxs,
    deriv,
...
function InterpolationData(id::InterpolationData, f)
    InterpolationData(f, id.timeseries, id.ts, id.ks, id.dense, id.cache)
end

function CompositeInterpolationData(id::CompositeInterpolationData, f)
    CompositeInterpolationData(
        f,
        id.timeseries,
        id.ts,
        id.ks,
