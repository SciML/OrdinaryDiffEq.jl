...
@muladd function perform_step!(integrator, cache::DABDF2ConstantCache, repeat_step = false)
    ...
    if integrator.opts.adaptive
        tmp =
            integrator.fsallast - (1 + dtₙ / dtₙ₋₁) * integrator.fsalfirst +
...
function initialize!(integrator, cache::DFBDFConstantCache)
    ...
    integrator.fsalfirst =
        integrator.f(integrator.du, integrator.uprev, integrator.p, integrator.t) # Pre-start fsal
    integrator.destats.nf += 1
...
function perform_step!(
    integrator,
    cache::DFBDFConstantCache{max_order},
    repeat_step = false,
) where {max_order}
    @unpack ts,
    u_history,
    ...
    if u isa Number
        for i = 1:k-1
            u_corrector[i] =
                calc_Lagrange_interp(k, weights, equi_ts[i], ts, u_history, u_corrector[i])
        end
    ...
    integrator.u = u
    integrator.fsallast =
        integrator.du = (nlsolver.α * z + nlsolver.tmp) * inv(nlsolver.γ * dt)
end
...
function perform_step!(
    ...
    @.. broadcast = false integrator.fsallast =
        integrator.du = (nlsolver.α * z + nlsolver.tmp) * inv(nlsolver.γ * dt) #TODO Lorenz plot seems not smooth
end
