...
@muladd function perform_step!(integrator,cache::SSPRKMSVS32Cache,repeat_step=false)
  ...
  if cache.step < 3
    ...
    Ω = 2
    @.. broadcast=false u = ((Ω*Ω - 1)/(Ω*Ω))*(uprev + (Ω/(Ω-1))*dt*fsalfirst) + (1/(Ω*Ω))*cache.u_2
    cache.u_2 .= u_1
    cache.u_1 .= uprev
    stage_limiter!(u, integrator, p, t+dt)
    step_limiter!(u, integrator, p, t+dt)
...
@muladd function perform_step!(integrator,cache::SSPRKMSVS43Cache,repeat_step=false)
  ...
  # u
    ...
    @.. broadcast=false u = (16/27)*(uprev + 3*dt*fsalfirst) + (11/27)*(u_3 + (12/11)*dt*k3)
    stage_limiter!(u, integrator, p, t+dt)
    step_limiter!(u, integrator, p, t+dt)
    cache.k3 .= k2
    cache.k2 .= k1
...
@muladd function perform_step!(integrator,cache::SSPRK932Cache,repeat_step=false)
  ...
  # u6*
  @.. broadcast=false thread=thread u = (3*uprev + dt_2*integrator.fsalfirst + 2*u) / 5
  stage_limiter!(u, integrator, p, t+dt_6)
  f( k,  u, p, t+dt_2)
  # u7*
  @.. broadcast=false thread=thread u = u + dt_6*k
...
@muladd function perform_step!(integrator,cache::SSPRK54Cache,repeat_step=false)
  ...
  f( k,  tmp, p, t+c4*dt)
  # u
  @.. broadcast=false thread=thread u = α52 * u₂ + α53 * u₃ + β53 * dt * k₃ + α54 * tmp + β54 * dt * k
  stage_limiter!(u, integrator, p, t+dt)
  step_limiter!(u, integrator, p, t+dt)
  integrator.destats.nf += 5
end
...
@muladd function perform_step!(integrator,cache::SSPRK104Cache,repeat_step=false)
  ...
  f( k,  tmp, p, t+dt)
  @.. broadcast=false thread=thread u = (uprev + 9*(u + dt_6*k₄) + 15*(tmp + dt_6*k)) / 25
  stage_limiter!(u, integrator, p, t+dt)
  step_limiter!(u, integrator, p, t+dt)
  integrator.destats.nf += 10
end
