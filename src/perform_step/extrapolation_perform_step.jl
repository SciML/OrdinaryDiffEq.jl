        ...
        let max_order = max_order,
            ...
            @threaded alg.threading for i = 1:2
                ...
                for index = startIndex:endIndex
                    ...
                    # Solve using Euler method
                    @muladd @.. broadcast = false u_tmps[Threads.threadid()] =
                        uprev + dt_temp * fsalfirst
                    f(
                        k_tmps[Threads.threadid()],
                        u_tmps[Threads.threadid()],
                        p,
                        t + dt_temp,
                    )
                    for j = 2:2^(index-1)
                        @muladd @.. broadcast = false u_tmps[Threads.threadid()] =
                            u_tmps[Threads.threadid()] +
                            dt_temp * k_tmps[Threads.threadid()]
                        f(
                            k_tmps[Threads.threadid()],
                            u_tmps[Threads.threadid()],
                            p,
...
function perform_step!(
    integrator,
    cache::ImplicitEulerExtrapolationCache,
    repeat_step = false,
)
    @unpack t, dt, uprev, u, f, p = integrator
    alg = unwrap_alg(integrator, true)
    ...
    if integrator.opts.adaptive
        ...
        if !(alg.n_min + 1 <= n_curr <= alg.n_max - 1)
            error(
                "Something went wrong while setting up the order window: $n_curr ∉ [$(alg.n_min+1),$(alg.n_max-1)].
        ...
        let n_curr = n_curr,
            ...
            @threaded alg.threading for i = 1:2
                ...
                for index = startIndex:endIndex
                    ...
                    for j = 1:sequence[index]
                        @.. broadcast = false linsolve_tmps[Threads.threadid()] =
                            dt_temp * k_tmps[Threads.threadid()]
                        ...
                        @.. broadcast = false k_tmps[Threads.threadid()] =
                            -k_tmps[Threads.threadid()]
                        @.. broadcast = false u_tmps2[Threads.threadid()] =
                            u_tmps[Threads.threadid()]
                        @.. broadcast = false u_tmps[Threads.threadid()] =
                            u_tmps[Threads.threadid()] + k_tmps[Threads.threadid()]
                        if index <= 2 && j >= 2
                            # Deuflhard Stability check for initial two sequences
                            @.. broadcast = false diff2[Threads.threadid()] =
                                u_tmps[Threads.threadid()] - u_tmps2[Threads.threadid()]
                            @.. broadcast = false diff2[Threads.threadid()] =
                                0.5 *
                                (diff2[Threads.threadid()] - diff1[Threads.threadid()])
                        ...
                        @.. broadcast = false diff1[Threads.threadid()] =
                            u_tmps[Threads.threadid()] - u_tmps2[Threads.threadid()]
                        f(
                            k_tmps[Threads.threadid()],
                            u_tmps[Threads.threadid()],
    ...
    for j = 2:n_curr+1
        for i = j:n_curr+1
            @.. broadcast = false T[i, j] =
                ((sequence[i] / sequence[i-j+1]) * T[i, j-1] - T[i-1, j-1]) /
    ...
    if integrator.opts.adaptive
        ...
        while n_curr <= win_max
            ...
            elseif (n_curr < alg.n_min + 1) ||
                   integrator.EEst <= typeof(integrator.EEst)(
                prod(sequence[n_curr+2:win_max+1] .// sequence[1]^2),
            )
...
function perform_step!(
    ...
    if !isthreaded(alg.threading)
        ...
        if alg.sequence == :romberg
            ...
            # 1 + 2 + 4 + ... + 2^(i-1) = 2^(i) - 1
            let n_curr = n_curr,
                subdividing_sequence = subdividing_sequence,
                uprev = uprev,
                dt = dt,
                u_temp3 = u_temp3,
                u_temp4 = u_temp4,
                ...
                @threaded alg.threading for i = 1:2
                    ...
                    for index = startIndex:endIndex
                        ...
                        @.. broadcast = false u_temp3[Threads.threadid()] =
                            u_temp4[Threads.threadid()] + dt_int_temp * fsalfirst # Euler starting step
                        for j = 2:j_int_temp
                            f(
                                k_tmps[Threads.threadid()],
                            ...
                            @.. broadcast = false T[index+1] =
                                u_temp4[Threads.threadid()] +
                                2 * dt_int_temp * k_tmps[Threads.threadid()] # Explicit Midpoint rule
                            @.. broadcast = false u_temp4[Threads.threadid()] =
            ...
            let n_curr = n_curr,
                ...
                @threaded alg.threading for i = 0:(n_curr÷2)
                    indices = (i, n_curr - i)
    ...
    if integrator.opts.adaptive
        ...
        for i = alg.n_min:n_curr
            ...
            u_temp2 .= false
            for j = 1:(i+1)
                @.. broadcast = false u_temp1 +=
                    cache.T[j] * extrapolation_weights[j, (i+1)]
            end
            for j = 2:i+1
                @.. broadcast = false u_temp2 +=
                    cache.T[j] * extrapolation_weights_2[j-1, i]
            end
        ...
        while n_curr <= win_max
            ...
            elseif integrator.EEst <=
                   tol^(
                stage_number[n_curr-alg.n_min+1] / stage_number[win_max-alg.n_min+1] - 1
            )
                # Reject current approximation order but pass convergence monitor
                # Compute approximation of order (n_curr + 1)
                n_curr = n_curr + 1
                ...
                for j = 1:n_curr+1
                    @.. broadcast = false u_temp1 +=
                        cache.T[j] * extrapolation_weights[j, (n_curr+1)]
                end
                for j = 2:n_curr+1
                    @.. broadcast = false u_temp2 +=
                        cache.T[j] * extrapolation_weights_2[j-1, n_curr]
                end
                @.. broadcast = false integrator.u =
                    extrapolation_scalars[n_curr+1] * u_temp1
                @.. broadcast = false cache.utilde =
                    extrapolation_scalars_2[n_curr] * u_temp2
...
function perform_step!(
    integrator,
    cache::ExtrapolationMidpointDeuflhardConstantCache,
    repeat_step = false,
)
    # Unpack all information needed
    @unpack t, uprev, dt, f, p = integrator
                ...
                @threaded alg.threading for i = 1:2
                    ...
                    for index = startIndex:endIndex
                        ...
                        for j = 2:j_int_temp
                            T[index+1] =
                                u_temp4 +
                                2 * dt_int_temp * f(u_temp3, p, t + (j - 1) * dt_int_temp) # Explicit Midpoint rule
                            u_temp4 = u_temp3
                            u_temp3 = T[index+1]
                        end
                    end
    ...
    if integrator.opts.adaptive
        ...
        for i = alg.n_min:n_curr
            u =
                eltype(uprev).(extrapolation_scalars[i+1]) * sum(
                    broadcast(
                        *,
                        T[1:(i+1)],
            ...
            utilde =
                eltype(uprev).(extrapolation_scalars_2[i]) * sum(
                    broadcast(
                        *,
                        T[2:(i+1)],
        ...
        while n_curr <= win_max
            ...
            elseif integrator.EEst <=
                ...
                # Update u, integrator.EEst and cache.Q
                u =
                    eltype(uprev).(extrapolation_scalars[n_curr+1]) * sum(
                        broadcast(
                            *,
                            T[1:(n_curr+1)],
                            eltype(uprev).(extrapolation_weights[1:(n_curr+1), (n_curr+1)]),
                        ),
                    ) # Approximation of extrapolation order n_curr
                utilde =
                    eltype(uprev).(extrapolation_scalars_2[n_curr]) * sum(
                        broadcast(
                            *,
                            T[2:(n_curr+1)],
                            eltype(uprev).(extrapolation_weights_2[1:n_curr, n_curr]),
                        ),
                    ) # and its internal counterpart
                res = calculate_residuals(
                    u,
...
function perform_step!(
    integrator,
    cache::ImplicitDeuflhardExtrapolationCache,
    repeat_step = false,
)
    # Unpack all information needed
    @unpack t, uprev, dt, f, p = integrator
                ...
                @threaded alg.threading for i = 1:2
                    ...
                    for index = startIndex:endIndex
                        ...
                        @.. broadcast = false k_tmps[Threads.threadid()] =
                            -k_tmps[Threads.threadid()]
                        @.. broadcast = false u_temp3[Threads.threadid()] =
                            u_temp4[Threads.threadid()] + k_tmps[Threads.threadid()] # Euler starting step
                        @.. broadcast = false diff1[Threads.threadid()] =
                            u_temp3[Threads.threadid()] - u_temp4[Threads.threadid()]
                        for j = 2:j_int_temp
                            f(
                                k_tmps[Threads.threadid()],
                                cache.u_temp3[Threads.threadid()],
                            ...
                            @.. broadcast = false linsolve_tmps[Threads.threadid()] =
                                dt_int_temp * k_tmps[Threads.threadid()] -
                                (u_temp3[Threads.threadid()] - u_temp4[Threads.threadid()])
                            ...
                            @.. broadcast = false k_tmps[Threads.threadid()] =
                                -k_tmps[Threads.threadid()]
                            @.. broadcast = false T[index+1] =
                                2 * u_temp3[Threads.threadid()] -
                                u_temp4[Threads.threadid()] + 2 * k_tmps[Threads.threadid()] # Explicit Midpoint rule
                            @.. broadcast = false u_temp4[Threads.threadid()] =
                                u_temp3[Threads.threadid()]
                            @.. broadcast = false u_temp3[Threads.threadid()] = T[index+1]
                            if (index <= 1)
    ...
    if integrator.opts.adaptive
        ...
        while n_curr <= win_max
            tol =
                integrator.opts.internalnorm(cache.utilde - integrator.u, t) /
            ...
            elseif integrator.EEst <=
                ...
                for j = 2:j_int
                    f(k, cache.u_temp1, p, t + (j - 1) * dt_int)
                    integrator.destats.nf += 1
                    @.. broadcast = false linsolve_tmps[1] =
                        dt_int * k - (u_temp1 - u_temp2)
...
function perform_step!(
    integrator,
    cache::ImplicitDeuflhardExtrapolationConstantCache,
    repeat_step = false,
)
    # Unpack all information needed
    @unpack t, uprev, dt, f, p = integrator
    ...
    if !isthreaded(alg.threading)
        for i = 0:n_curr
            ...
            u_temp2 = uprev
            u_temp1 =
                u_temp2 + _reshape(W \ -_vec(dt_int * integrator.fsalfirst), axes(uprev)) # Euler starting step
                        ...
                        for j = 2:j_int
                            T[index+1] =
                                ...
                                2 * _reshape(
                                    W \
                                    -_vec(
                                        dt_int * f(u_temp3, p, t + (j - 1) * dt_int) -
                                        (u_temp3 - u_temp4),
                                    ),
                                    axes(uprev),
                                )
    ...
    if integrator.opts.adaptive
        ...
        while n_curr <= win_max
            ...
            elseif (n_curr < alg.n_min + 1) ||
                   integrator.EEst <= typeof(integrator.EEst)(
                prod(
                    subdividing_sequence[n_curr+2:win_max+1] .// subdividing_sequence[1]^2,
                ),
            )
                # Reject current approximation order but pass convergence monitor
                # Compute approximation of order (n_curr + 1)
...
function perform_step!(
    integrator,
    cache::ImplicitHairerWannerExtrapolationConstantCache,
    repeat_step = false,
)
    # Unpack all information needed
    @unpack t, uprev, dt, f, p = integrator
                ...
                @threaded alg.threading for i = 1:2
                    ...
                    for index = startIndex:endIndex
                        ...
                        for j = 2:j_int_temp+1
                            ...
                            if (j == j_int_temp + 1)
                                @.. broadcast = false T[index+1] =
                                    0.5(T[index+1] + u_temp4[Threads.threadid()])
                            end
                            @.. broadcast = false u_temp4[Threads.threadid()] =
                                u_temp3[Threads.threadid()]
                            @.. broadcast = false u_temp3[Threads.threadid()] = T[index+1]
                ...
                @threaded alg.threading for i = 0:(n_curr÷2)
                    ...
                    for index in indices
                        ...
                        for j = 2:j_int_temp+1
                            ...
                            @.. broadcast = false linsolvetmp =
                                dt_int_temp * ktmp - (u_temp3[tid] - u_temp4[tid])
                            ...
                            @.. broadcast = false ktmp = -ktmp
                            @.. broadcast = false T[index+1] =
                                2 * u_temp3[tid] - u_temp4[tid] + 2 * ktmp # Explicit Midpoint rule
                            if (j == j_int_temp + 1)
                                @.. broadcast = false T[index+1] =
                                    0.5(T[index+1] + u_temp4[tid])
                            end
                            @.. broadcast = false u_temp4[tid] = u_temp3[tid]
                            @.. broadcast = false u_temp3[tid] = T[index+1]
                            if (index <= 1)
                                # Deuflhard Stability check for initial two sequences
                                @.. broadcast = false diff2[tid] =
                                    u_temp3[tid] - u_temp4[tid]
                                @.. broadcast = false diff2[tid] =
                                    0.5 * (diff2[tid] - diff1[tid])
                                if (
                ...
                @threaded alg.threading for i = 1:2
                    ...
                    for index = startIndex:endIndex
                        ...
                        for j = 2:j_int+1
                            T[index+1] =
                                u_temp3 + _reshape(
                                    W \ -_vec(dt_int * f(u_temp3, p, t + (j - 1) * dt_int)),
                                    axes(uprev),
                                )
                            integrator.destats.nf += 1
                            if (j == j_int + 1)
...
function perform_step!(
    integrator,
    cache::ImplicitEulerBarycentricExtrapolationCache,
    repeat_step = false,
)
    # Unpack all information needed
    @unpack t, uprev, dt, f, p = integrator
                    ...
                    for index = startIndex:endIndex
                        ...
                        for j = 2:j_int_temp+1
                            ...
                            @.. broadcast = false k_tmps[Threads.threadid()] =
                                -k_tmps[Threads.threadid()]
                            @.. broadcast = false T[index+1] =
                                u_temp3[Threads.threadid()] + k_tmps[Threads.threadid()] # Explicit Midpoint rule
                            if (j == j_int_temp + 1)
                                @.. broadcast = false T[index+1] =
                                    0.25(
                                        T[index+1] +
        ...
        while n_curr <= win_max
            ...
            elseif (n_curr < alg.n_min + 1) || integrator.EEst <= EEst1
                ...
                for j = 2:j_int+1
                    ...
                    if (j == j_int + 1)
                        @.. broadcast = false T[n_curr+1] =
                            0.25(T[n_curr+1] + 2 * u_temp1 + u_temp2)
                    end
                    @.. broadcast = false u_temp2 = u_temp1
                    @.. broadcast = false u_temp1 = T[n_curr+1]
                end
