...
@muladd function perform_step!(
    integrator,
    cache::Nystrom4VelocityIndependentConstantCache,
    repeat_step = false,
)
    @unpack t, dt, f, p = integrator
    duprev, uprev = integrator.uprev.x
...
@muladd function perform_step!(
    integrator,
    cache::Nystrom4VelocityIndependentCache,
    repeat_step = false,
)
    @unpack t, dt, f, p = integrator
    du, u = integrator.u.x
...
@muladd function perform_step!(integrator, cache::IRKN3ConstantCache, repeat_step = false)
    ...
    if integrator.iter < 2 && !integrator.u_modified
        ...
        du =
            duprev + dt * (b1 * k1cache.x[1] + bbar1 * k1cache.x[1] + b2 * (k₂x1 - k₂.x[1]))
        u =
            uprev +
            bconst1 * dt * duprev +
        ...
        integrator.fsallast =
            ArrayPartition((f.f1(du, u, p, t + dt), f.f2(du, u, p, t + dt)))
        integrator.destats.nf += 3
        integrator.destats.nf2 += 1
        copyto!(k₂.x[1], k₂.x[2])
    ...
    if integrator.iter < 2 && !integrator.u_modified
        ...
        @tight_loop_macros for i in uidx
            @inbounds u[i] =
                uprev[i] +
                bconst1 * dt * duprev[i] +
                dt * (bconst2 * duprev2[i] + dt * bbar2 * (k₂.x[2][i] - k₂.x[1][i]))
            @inbounds du[i] =
                duprev[i] +
                dt * (
                    b1 * k1cache.x[1][i] +
        ...
        @tight_loop_macros for i in uidx
            @inbounds u[i] =
                ...
                dt * (
                    bconst2 * duprev2[i] +
                    dt *
                    (bbar2 * (k₂.x[2][i] - k₂.x[1][i]) + bbar3 * (k₃.x[2][i] - k₃.x[1][i]))
                )
            @inbounds du[i] =
                duprev[i] +
                dt * (
...
@muladd function perform_step!(
    integrator,
    cache::Nystrom5VelocityIndependentConstantCache,
    repeat_step = false,
)
    @unpack t, dt, f, p = integrator
    duprev, uprev = integrator.uprev.x
    @unpack c1, c2, a21, a31, a32, a41, a42, a43, bbar1, bbar2, bbar3, b1, b2, b3, b4 =
        cache
    k₁ = integrator.fsalfirst.x[1]
...
@muladd function perform_step!(
    integrator,
    cache::Nystrom5VelocityIndependentCache,
    repeat_step = false,
)
    @unpack t, dt, f, p = integrator
    du, u = integrator.u.x
    ...
    #end
    @.. broadcast = false u =
        uprev + dt * (duprev + dt * (bbar1 * k₁ + bbar2 * k₂ + bbar3 * k₃))
...
@muladd function perform_step!(integrator, cache::DPRKN6ConstantCache, repeat_step = false)
    ...
    if integrator.opts.adaptive
        ...
        duhat =
            dt *
            (bptilde1 * k1 + bptilde3 * k3 + bptilde4 * k4 + bptilde5 * k5 + bptilde6 * k6)
...
@muladd function perform_step!(integrator, cache::DPRKN6Cache, repeat_step = false)
    ...
    @.. broadcast = false ku =
        uprev + dt * (c3 * duprev + dt * (a41 * k1 + a42 * k2 + a43 * k3))

    f.f1(k4, du, ku, p, t + dt * c3)
    @.. broadcast = false ku =
        uprev + dt * (c4 * duprev + dt * (a51 * k1 + a52 * k2 + a53 * k3 + a54 * k4))

    f.f1(k5, du, ku, p, t + dt * c4)
    @.. broadcast = false ku =
        uprev + dt * (c5 * duprev + dt * (a61 * k1 + a63 * k3 + a64 * k4 + a65 * k5)) # no a62

    f.f1(k6, du, ku, p, t + dt * c5)

    @.. broadcast = false u =
        uprev + dt * (duprev + dt * (b1 * k1 + b3 * k3 + b4 * k4 + b5 * k5)) # b1 -- b5, no b2
    @.. broadcast = false du =
        duprev + dt * (bp1 * k1 + bp3 * k3 + bp4 * k4 + bp5 * k5 + bp6 * k6) # bp1 -- bp6, no bp2
    ...
    if integrator.opts.adaptive
        duhat, uhat = utilde.x
        dtsq = dt^2
        @.. broadcast = false uhat =
            dtsq *
            (btilde1 * k1 + btilde2 * k2 + btilde3 * k3 + btilde4 * k4 + btilde5 * k5)
        @.. broadcast = false duhat =
            dt *
            (bptilde1 * k1 + bptilde3 * k3 + bptilde4 * k4 + bptilde5 * k5 + bptilde6 * k6)
        calculate_residuals!(
            atmp,
...
@muladd function perform_step!(integrator, cache::DPRKN8ConstantCache, repeat_step = false)
    ...
    ku =
        ...
        dt * (
            c7 * duprev +
            dt *
            (a81 * k1 + a82 * k2 + a83 * k3 + a84 * k4 + a85 * k5 + a86 * k6 + a87 * k7)
        )
...
@muladd function perform_step!(integrator, cache::DPRKN8Cache, repeat_step = false)
    ...
    @tight_loop_macros for i in uidx
        @inbounds ku[i] =
            uprev[i] +
            dt *
            (c4 * duprev[i] + dt * (a51 * k1[i] + a52 * k2[i] + a53 * k3[i] + a54 * k4[i]))
    end
...
@muladd function perform_step!(integrator, cache::DPRKN12Cache, repeat_step = false)
    ...
    @tight_loop_macros for i in uidx
        @inbounds ku[i] =
            uprev[i] +
            dt * (c4 * duprev[i] + dt * (a51 * k1[i] + a53 * k3[i] + a54 * k4[i])) # no a52
    end
    ...
    @tight_loop_macros for i in uidx
        @inbounds ku[i] =
            uprev[i] +
            dt *
            (c5 * duprev[i] + dt * (a61 * k1[i] + a63 * k3[i] + a64 * k4[i] + a65 * k5[i])) # no a62
    end
    ...
    @tight_loop_macros for i in uidx
        @inbounds ku[i] =
            ...
            dt * (
                c6 * duprev[i] +
                dt * (a71 * k1[i] + a73 * k3[i] + a74 * k4[i] + a75 * k5[i] + a76 * k6[i])
            ) # no a72
    end
    ...
    @tight_loop_macros for i in uidx
        @inbounds ku[i] =
            ...
            dt * (
                c7 * duprev[i] +
                dt * (a81 * k1[i] + a84 * k4[i] + a85 * k5[i] + a86 * k6[i] + a87 * k7[i])
            ) # no a82, a83
    end
...
@muladd function perform_step!(integrator, cache::ERKN4Cache, repeat_step = false)
    ...
    @tight_loop_macros for i in uidx
        @inbounds u[i] =
            uprev[i] +
            dt * (duprev[i] + dt * (b1 * k1[i] + b2 * k2[i] + b3 * k3[i] + b4 * k4[i]))
        @inbounds du[i] =
            duprev[i] + dt * (bp1 * k1[i] + bp2 * k2[i] + bp3 * k3[i] + bp4 * k4[i])
    end
    ...
    if integrator.opts.adaptive
        ...
        @tight_loop_macros for i in uidx
            @inbounds uhat[i] =
                dtsq *
                (btilde1 * k1[i] + btilde2 * k2[i] + btilde3 * k3[i] + btilde4 * k4[i])
            @inbounds duhat[i] =
                dt *
                (bptilde1 * k1[i] + bptilde2 * k2[i] + bptilde3 * k3[i] + bptilde4 * k4[i])
        end
        calculate_residuals!(
            atmp,
            utilde,
