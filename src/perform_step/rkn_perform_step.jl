...
function initialize!(integrator,cache::NystromCCDefaultInitialization)
 ...
 ku  = integrator.f.f1(uprev,duprev,integrator.p,integrator.t)
 kdu = integrator.f.f2(uprev,duprev,integrator.p,integrator.t)
...
function initialize!(integrator,cache::NystromDefaultInitialization)
  ...
  integrator.f.f1(integrator.k[1].x[1],uprev,duprev,integrator.p,integrator.t)
  integrator.f.f2(integrator.k[1].x[2],uprev,duprev,integrator.p,integrator.t)
end

@muladd function perform_step!(integrator,cache::Nystrom4ConstantCache,repeat_step=false)
  @unpack t,dt,f,p = integrator
  uprev,duprev = integrator.uprev.x
  k₁ = integrator.fsalfirst.x[2]
  halfdt = dt/2
  dtsq = dt^2
  eighth_dtsq = dtsq/8
  half_dtsq = dtsq/2
  ...
  k₄ = f.f2(ku,kdu,p,t+dt)
  u = uprev + (dtsq/6)*(k₁ + k₂ + k₃) + dt*duprev
  du = duprev + (dt/6)*(k₁ + k₄ + 2*(k₂ + k₃))

  integrator.u = ArrayPartition((u,du))
  integrator.fsallast = ArrayPartition((f.f1(u,du,p,t+dt),f.f2(u,du,p,t+dt)))
  integrator.k[1] = integrator.fsalfirst
  integrator.k[2] = integrator.fsallast
end
...
@muladd function perform_step!(integrator,cache::Nystrom4Cache,repeat_step=false)
  ...
  f.f2(k₄,ku,kdu,p,t+dt)
  @. u = uprev + (dtsq/6)*(k₁ + k₂ + k₃) + dt*duprev
  @. du = duprev + (dt/6)*(k₁ + k₄ + 2*(k₂ + k₃))

  f.f1(k.x[1],u,du,p,t+dt)
  f.f2(k.x[2],u,du,p,t+dt)
end
...
@muladd function perform_step!(integrator,cache::Nystrom4VelocityIndependentConstantCache,repeat_step=false)
  ...
  k₂ = f.f2(ku,duprev,p,ttmp)
  ku = uprev + dt*duprev + half_dtsq*k₂

  k₃ = f.f2(ku,duprev,p,t+dt)
...
@muladd function perform_step!(integrator,cache::IRKN3ConstantCache,repeat_step=false)
  ...
  if integrator.iter < 2 && !integrator.u_modified
    ...
    k1cache = ArrayParition((k.x[2],f.f2(uprev,duprev,p,t+c1*dt)))
    kdu = uprev + dt*(c1*duprev + dt*a21*k1cache.x[2])
    k₂.x[2] = f.f2(kdu,duprev,p,t+c1*dt)
  else
    ku  = uprev  + dt*(c1*duprev  + dt*a21*k1cache.x[1])
    kdu = uprev2 + dt*(c1*duprev2 + dt*a21*k1cache.x[2])

    k₂x1 = f.f2(ku, duprev, p, t+c1*dt)
    u  = uprev + bconst1*dt*duprev + dt*(bconst2*duprev2 + dt*bbar2*(k₂x1-k₂.x[2]))
    du = duprev + dt*(b1*k1cache.x[1] + bbar1*k1cache.x[2] + b2*(k₂x1-k₂.x[2]))

    integrator.fsallast = ArrayPartition((f.f1(u,du,p,t+dt),f.f2(u,du,p,t+dt)))
    copy!(k₂.x[2],k₂.x[1])
    k1cache = ArrayPartition((k.x[2],k1cache.x[1]))
  end # end if
end
...
@muladd function perform_step!(integrator,cache::IRKN3Cache,repeat_step=false)
  ...
  if integrator.iter < 2 && !integrator.u_modified
    ...
    f.f2(k1cache.x[2],uprev,duprev,p,t+c1*dt)
    @. kdu= uprev + dt*(c1*duprev + dt*a21*k1cache.x[2])
    f.f2(k₂.x[2],kdu,duprev,p,t+c1*dt)
  else
    @. ku  = uprev  + dt*(c1*duprev  + dt*a21*k1cache.x[1])
    @. kdu = uprev2 + dt*(c1*duprev2 + dt*a21*k1cache.x[2])

    f.f2(k₂.x[1], ku, duprev, p, t+c1*dt)
...
@muladd function perform_step!(integrator,cache::IRKN4Cache,repeat_step=false)
  ...
  if integrator.iter < 2 && !integrator.u_modified
    ...
    f.f2(k₃.x[1], ku, duprev, p, t+c2*dt)
    @tight_loop_macros for i in uidx
...
@muladd function perform_step!(integrator,cache::Nystrom5VelocityIndependentConstantCache,repeat_step=false)
  ...
  k₂ = f.f2(ku,duprev,p,t+c1*dt)
  ku = uprev + dt*(c2*duprev + dt*(a31*k₁ + a32*k₂))

  k₃ = f.f2(ku,duprev,p,t+c2*dt)
...
@muladd function perform_step!(integrator,cache::Nystrom5VelocityIndependentCache,repeat_step=false)
  ...
  f.f2(k₂,ku,du,p,t+c1*dt)
  @. ku = uprev + dt*(c2*duprev + dt*(a31*k₁ + a32*k₂))

  f.f2(k₃,ku,du,p,t+c2*dt)
  @tight_loop_macros for i in uidx
...
@muladd function perform_step!(integrator,cache::DPRKN6ConstantCache,repeat_step=false)
  ...
  integrator.u = ArrayPartition((u,du))
  integrator.fsallast = ArrayPartition((f.f1(u,du,p,t+dt),f.f2(u,du,p,t+dt)))
  integrator.k[1] = ArrayPartition(integrator.fsalfirst.x[2],k2)
  ...
  if integrator.opts.adaptive
    ...
    utilde = ArrayPartition((uhat,duhat))
    atmp = calculate_residuals(utilde, integrator.uprev, integrator.u, integrator.opts.abstol, integrator.opts.reltol,integrator.opts.internalnorm)
...
function initialize!(integrator, cache::DPRKN6Cache)
  ...
  integrator.f.f1(integrator.fsallast.x[1],uprev,duprev,integrator.p,integrator.t)
  integrator.f.f2(integrator.fsallast.x[2],uprev,duprev,integrator.p,integrator.t)
end
