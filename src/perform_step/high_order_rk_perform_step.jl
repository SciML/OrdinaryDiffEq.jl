...
@muladd function perform_step!(integrator, cache::TanYam7Cache, repeat_step = false)
    ...
    @.. broadcast = false tmp =
        uprev +
        dt *
        (a101 * k1 + a103 * k3 + a104 * k4 + a105 * k5 + a106 * k6 + a107 * k7 + a108 * k8)
    f(k10, tmp, p, t + dt)
    @.. broadcast = false u =
...
@muladd function perform_step!(integrator, cache::DP8ConstantCache, repeat_step = false)
    ...
    kupdate =
        b1 * k1 + b6 * k6 + b7 * k7 + b8 * k8 + b9 * k9 + b10 * k10 + b11 * k11 + b12 * k12
    u = uprev + dt * kupdate
    if integrator.opts.adaptive
        utilde =
            dt * (
...
@muladd function perform_step!(integrator, cache::DP8Cache, repeat_step = false)
    ...
    if integrator.opts.calck
        ...
        @.. broadcast = false integrator.k[4] =
            ...
            d415 * k15 +
            d416 * k16
        @.. broadcast = false integrator.k[5] =
            d501 * k1 +
            d506 * k6 +
            d507 * k7 +
            d508 * k8 +
            ...
            d515 * k15 +
            d516 * k16
        @.. broadcast = false integrator.k[6] =
            d601 * k1 +
            d606 * k6 +
            d607 * k7 +
            d608 * k8 +
            ...
            d615 * k15 +
            d616 * k16
        @.. broadcast = false integrator.k[7] =
            d701 * k1 +
            d706 * k6 +
            d707 * k7 +
            d708 * k8 +
...
@muladd function perform_step!(integrator, cache::PFRK87ConstantCache, repeat_step = false)
    ...
    d_ν =
        1 - (0.296457092123567400 * νsq) + (0.0015793885907465726 * (νsq^2)) -
    ...
    k8 = f(
        uprev + dt * (α0801 * k1 + α0804 * k4 + α0805 * k5 + α0806 * k6 + α0807 * k7),
        p,
        t + c8 * dt,
    )
    k9 = f(
        uprev +
        dt * (α0901 * k1 + α0904 * k4 + α0905 * k5 + α0906 * k6 + α0907 * k7 + α0908 * k8),
        p,
        t + c9 * dt,
    )
    k10 = f(
...
@muladd function perform_step!(integrator, cache::PFRK87Cache, repeat_step = false)
    ...
    f(k6, tmp, p, t + c6 * dt)
    @.. broadcast = false tmp =
        uprev + dt * (α0701 * k1 + α0704 * k4 + α0705 * k5 + α0706 * k6)
    f(k7, tmp, p, t + c7 * dt)
    @.. broadcast = false tmp =
        uprev + dt * (α0801 * k1 + α0804 * k4 + α0805 * k5 + α0806 * k6 + α0807 * k7)
    f(k8, tmp, p, t + c8 * dt)
