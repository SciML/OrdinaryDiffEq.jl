...
@muladd function perform_step!(integrator, cache::RadauIIA3ConstantCache)
    ...
    while iter < maxiters
        ...
        if η * ndw < κ && (iter > 1 || iszero(ndw) || !iszero(integrator.success_iter))
            ...
            cache.status =
                η < alg.fast_convergence_cutoff ? DiffEqBase.FastConvergence :
...
@muladd function perform_step!(
    ...
    if integrator.iter == 1 || integrator.u_modified || alg.extrapolant == :constant
        ...
        c1′ = c1 * c3′
        c2′ = c2 * c3′
        z1 = @.. broadcast = false c1′ *
                              (cont1 + (c1′ - c2m1) * (cont2 + (c1′ - c1m1) * cont3))
        z2 = @.. broadcast = false c2′ *
                              (cont1 + (c2′ - c2m1) * (cont2 + (c2′ - c1m1) * cont3))
        z3 = @.. broadcast = false c3′ *
                              (cont1 + (c3′ - c2m1) * (cont2 + (c3′ - c1m1) * cont3))
...
@muladd function perform_step!(integrator, cache::RadauIIA5Cache, repeat_step = false)
    ...
    if integrator.iter == 1 || integrator.u_modified || alg.extrapolant == :constant
        ...
        c2′ = c2 * c3′
        @.. broadcast = false z1 =
            c1′ * (cont1 + (c1′ - c2m1) * (cont2 + (c1′ - c1m1) * cont3))
        @.. broadcast = false z2 =
            c2′ * (cont1 + (c2′ - c2m1) * (cont2 + (c2′ - c1m1) * cont3))
        @.. broadcast = false z3 =
            c3′ * (cont1 + (c3′ - c2m1) * (cont2 + (c3′ - c1m1) * cont3))
    ...
    while iter < maxiters
        ...
        if needfactor
            linres1 =
                dolinsolve(integrator, linsolve1; A = W1, b = _vec(ubuff), linu = _vec(dw1))
        else
            linres1 = dolinsolve(
                integrator,
                linsolve1;
        ...
        @.. broadcast = false cubuff =
            complex(fw2 - αdt * Mw2 + βdt * Mw3, fw3 - βdt * Mw2 - αdt * Mw3)
    ...
    if adaptive
        ...
        if alg.smooth_est
            linres1 =
                dolinsolve(integrator, linres1.cache; b = _vec(ubuff), linu = _vec(utilde))
            cache.linsolve1 = linres1.cache
            integrator.destats.nsolve += 1
        end
