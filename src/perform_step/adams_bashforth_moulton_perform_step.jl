...
function initialize!(
    integrator,
    cache::Union{AB3Cache,AB4Cache,AB5Cache,ABM32Cache,ABM43Cache,ABM54Cache},
)
...
@muladd function perform_step!(integrator, cache::ABM32Cache, repeat_step = false)
    ...
    if cache.step == 1
        ...
        if cnt == 2
            perform_step!(
                integrator,
                AB3Cache(u, uprev, fsalfirst, copy(k2), k3, ralk2, k, tmp, cnt),
...
@muladd function perform_step!(integrator, cache::ABM43Cache, repeat_step = false)
    ...
    if cache.step <= 2
        ...
        perform_step!(
            integrator,
            AB4Cache(u, uprev, fsalfirst, t2, t3, t4, ralk2, k, tmp, t5, t6, t7, cnt),
        )
        k = integrator.fsallast
...
@muladd function perform_step!(integrator, cache::AB5Cache, repeat_step = false)
    ...
    if cache.step <= 4
        ...
        @.. broadcast = false u =
            uprev + (dt / 720) * (1901 * k1 - 2774 * k2 + 2616 * k3 - 1274 * k4 + 251 * k5)
...
@muladd function perform_step!(integrator, cache::ABM54Cache, repeat_step = false)
    ...
    if cache.step <= 3
        ...
        perform_step!(
            integrator,
            AB5Cache(u, uprev, fsalfirst, t2, t3, t4, t5, k, tmp, t6, t7, t8, cnt),
        )
        k = integrator.fsallast
        @.. broadcast = false u =
            uprev + (dt / 720) * (251 * k + 646 * k1 - 264 * k2 + 106 * k3 - 19 * k4)
        cache.k5, cache.k4 = k4, k5
        cache.k4 .= k3
        cache.k3 .= k2
        cache.k2 .= k1
...
@muladd function perform_step!(integrator, cache::VCABM5Cache, repeat_step = false)
    @inbounds begin
        @unpack t, dt, uprev, u, f, p = integrator
        @unpack k4, dts, g, ϕ_n, ϕ_np1, ϕstar_n, ϕstar_nm1, order, atmp, utilde, rk4cache =
            cache
        k1 = integrator.fsalfirst
        if integrator.u_modified
            cache.step = 1
