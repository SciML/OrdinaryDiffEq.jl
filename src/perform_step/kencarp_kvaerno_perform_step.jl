...
@muladd function perform_step!(
    integrator,
    cache::Kvaerno3ConstantCache,
    repeat_step = false,
)
    @unpack t, dt, uprev, u, f, p = integrator
    nlsolver = cache.nlsolver
    @unpack γ, a31, a32, a41, a42, a43, btilde1, btilde2, btilde3, btilde4, c3, α31, α32 =
        cache.tab
...
@muladd function perform_step!(integrator, cache::KenCarp3Cache, repeat_step = false)
    ...
    if typeof(integrator.f) <: SplitFunction
        ...
        integrator.destats.nf2 += 1
        @.. broadcast = false tmp =
            uprev + a41 * z₁ + a42 * z₂ + a43 * z₃ + ea41 * k1 + ea42 * k2 + ea43 * k3
    else
        @unpack α41, α42 = cache.tab
        @.. broadcast = false z₄ = α41 * z₁ + α42 * z₂
        @.. broadcast = false tmp = uprev + a41 * z₁ + a42 * z₂ + a43 * z₃
...
@muladd function perform_step!(integrator, cache::Kvaerno4Cache, repeat_step = false)
    ...
    if integrator.opts.adaptive
        @.. broadcast = false tmp =
            btilde1 * z₁ + btilde2 * z₂ + btilde3 * z₃ + btilde4 * z₄ + btilde5 * z₅
        if isnewton(nlsolver) && alg.smooth_est # From Shampine
            est = nlsolver.cache.dz
...
@muladd function perform_step!(
    integrator,
    cache::KenCarp4ConstantCache,
    repeat_step = false,
)
    @unpack t, dt, uprev, u, p = integrator
    nlsolver = cache.nlsolver
    @unpack γ, a31, a32, a41, a42, a43, a51, a52, a53, a54, a61, a63, a64, a65, c3, c4, c5 =
        cache.tab
...
@muladd function perform_step!(
    ...
    @unpack ea21, ea31, ea32, ea41, ea43, ea51, ea53, ea54, ea61, ea63, ea64, ea65 =
        cache.tab
...
@muladd function perform_step!(integrator, cache::KenCarp5Cache, repeat_step = false)
    ...
    if typeof(integrator.f) <: SplitFunction
        ...
        integrator.destats.nf2 += 1
        @.. broadcast = false tmp =
            uprev + a51 * z₁ + a53 * z₃ + a54 * z₄ + ea51 * k1 + ea53 * k3 + ea54 * k4
    else
    ...
    if typeof(integrator.f) <: SplitFunction
        ...
        @.. broadcast = false tmp =
            uprev + a71 * z₁ + a73 * z₃ + a74 * z₄ + a75 * z₅ + a76 * z₆
    end
    nlsolver.z = z₇
    ...
    if typeof(integrator.f) <: SplitFunction
        ...
        @.. broadcast = false tmp =
            uprev + a81 * z₁ + a84 * z₄ + a85 * z₅ + a86 * z₆ + a87 * z₇
    end
    nlsolver.z = z₈
...
@muladd function perform_step!(
    ...
    c5,
    c6 = cache.tab
    @unpack α31, α32, α41, α42, α43, α51, α52, α61, α62, α63, α71, α72, α73, α74, α75, α76 =
        cache.tab
...
@muladd function perform_step!(integrator, cache::KenCarp47Cache, repeat_step = false)
    ...
    if typeof(integrator.f) <: SplitFunction
        ...
        @.. broadcast = false z₆ = α61 * z₁ + α62 * z₂ + α63 * z₃
        @.. broadcast = false tmp =
            uprev + a61 * z₁ + a62 * z₂ + a63 * z₃ + a64 * z₄ + a65 * z₅
    end
    nlsolver.z = z₆
    ...
    if typeof(integrator.f) <: SplitFunction
        ...
        @.. broadcast = false z₇ =
            α71 * z₁ + α72 * z₂ + α73 * z₃ + α74 * z₄ + α75 * z₅ + α76 * z₆
    ...
    if integrator.opts.adaptive
        ...
        if typeof(integrator.f) <: SplitFunction
            ...
            @.. broadcast = false tmp =
                btilde3 * z₃ + btilde4 * z₄ + btilde5 * z₅ + btilde6 * z₆ + btilde7 * z₇
        end
...
@muladd function perform_step!(
    ...
    ea64,
    ea65 = cache.tab
    @unpack ea71, ea72, ea73, ea74, ea75, ea76, ea81, ea82, ea83, ea84, ea85, ea86, ea87 =
        cache.tab
...
@muladd function perform_step!(integrator, cache::KenCarp58Cache, repeat_step = false)
    ...
    if typeof(integrator.f) <: SplitFunction
        ...
        @.. broadcast = false tmp =
            uprev + a71 * z₁ + a72 * z₂ + a73 * z₃ + a74 * z₄ + a75 * z₅ + a76 * z₆
    end
    nlsolver.z = z₇
    ...
    if typeof(integrator.f) <: SplitFunction
        ...
        @.. broadcast = false z₈ =
            α81 * z₁ + α82 * z₂ + α83 * z₃ + α84 * z₄ + α85 * z₅ + α86 * z₆ + α87 * z₇
        @.. broadcast = false tmp =
            uprev + a83 * z₃ + a84 * z₄ + a85 * z₅ + a86 * z₆ + a87 * z₇
    end
    nlsolver.z = z₈
