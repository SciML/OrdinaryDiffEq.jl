...
function perform_step!(integrator, cache::CG3Cache, repeat_step = false)
    ...
    C = Matrix(deepcopy(L))
    u .=
        (exp(dt * (24 / 17) * C) * exp(dt * (-2 / 3) * B) * exp(dt * (13 / 51) * A)) * uprev
...
function perform_step!(integrator, cache::MagnusNC8Cache, repeat_step = false)
    ...
    Q5 =
        (-6025 * B0 / 4116 + 2875 * B2 / 343) * (B2 * Q1 - Q1 * B2) -
        (B2 * Q1 - Q1 * B2) * (-6025 * B0 / 4116 + 2875 * B2 / 343)
    Q6 =
        B3 * (20 * (Q3 + Q4) / 7 + 820 * dt * Q5 / 189) -
...
function perform_step!(
    integrator,
    cache::MagnusLeapfrogCache,
    repeat_step = false,
    alg_extrapolates = true,
    iter = 1,
)
