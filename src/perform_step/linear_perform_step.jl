...
function perform_step!(integrator, cache::MagnusGL6Cache, repeat_step=false)
  ...
  L2 = deepcopy(integrator.f.f)
  L3 = deepcopy(integrator.f.f)
  update_coefficients!(L1,uprev,p,t-dt*(sqrt(3/20)))
  A1 = Matrix(L1)
  update_coefficients!(L2,uprev,p,t)
  A2 = Matrix(L2)
  update_coefficients!(L3,uprev,p,t+dt*(sqrt(3/20)))
  A3 = Matrix(L3)
  B0 = (1/18)*(5*(A1+A3)+8*A2)
  println("*********")
  println("Intitial B = $B")
  B1 = (sqrt(15)/36)*(A3-A1)
  B2 = (1/24)*(A1+A3)
  Ω1 = dt*B0
  println("Final B = $B")
  println("*********")
  Ω2 = (dt^2)*(B1*(3*B0/2 - 6*B2) - (3*B0/2 - 6*B2)*B1)
  Ω3_4 = (dt^2)*(B0*(B0*(dt*B2/2-Ω2/60)-(dt*B2/2-Ω2/60)*B0) - (B0*(dt*B2/2-Ω2/60)-(dt*B2/2-Ω2/60)*B0)*B0) + (3*dt/5)*(B1*Ω2 - Ω2*B1)
  if integrator.alg.krylov
    u .= expv(1.0,Ω1 + Ω2 + Ω3_4, uprev; m=min(alg.m, size(L1,1)), opnorm=integrator.opts.internalopnorm, iop=alg.iop)
  else
    u .= exp(Ω1 + Ω2 + Ω3_4) * uprev
