...
function initialize!(integrator, cache::ROCK2Cache)
    ...
    alg = unwrap_alg(integrator, true)
    cache.constantcache.max_stage =
        (alg.max_stages < 1 || alg.max_stages > 200) ? 200 : alg.max_stages
    cache.constantcache.min_stage =
        (alg.min_stages > cache.constantcache.max_stage) ? cache.constantcache.max_stage :
        alg.min_stages
...
function initialize!(integrator, cache::ROCK4Cache)
    ...
    alg = unwrap_alg(integrator, true)
    cache.constantcache.max_stage =
        (alg.max_stages < 1 || alg.max_stages > 152) ? 152 : alg.max_stages
    cache.constantcache.min_stage =
        (alg.min_stages > cache.constantcache.max_stage) ? cache.constantcache.max_stage :
        alg.min_stages
...
@muladd function perform_step!(integrator, cache::RKCCache, repeat_step = false)
    ...
    for iter = 2:mdeg
        ...
        integrator.destats.nf += 1
        @.. broadcast = false u =
            μ * gprev + ν * gprev2 + (1 - μ - ν) * uprev + dt * μs * (k - νs * fsalfirst)
        th = μ * th1 + ν * th2 + μs * (1 - νs)
        if (iter < mdeg)
            gprev2 = gprev
            gprev = u
...
function perform_step!(integrator, cache::IRKCCache, repeat_step = false)
    ...
    if isnewton(nlsolver) && integrator.opts.adaptive
        ...
        @.. broadcast = false gprev =
            dt * 0.5 * (du₂ - f2ⱼ₋₁) + dt * (0.5 - μs₁) * (du₁ - f1ⱼ₋₁)
