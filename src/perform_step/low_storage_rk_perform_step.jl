...
@muladd function perform_step!(
    integrator,
    cache::LowStorageRK2NConstantCache,
    repeat_step = false,
)
...
@muladd function perform_step!(
    integrator,
    cache::LowStorageRK2CConstantCache,
    repeat_step = false,
)
    @unpack t, dt, u, f, p = integrator
    @unpack A2end, B1, B2end, c2end = cache
...
@muladd function perform_step!(
    integrator,
    cache::LowStorageRK3SConstantCache,
    repeat_step = false,
)
...
@muladd function perform_step!(integrator, cache::LowStorageRK3SCache, repeat_step = false)
    ...
    for i in eachindex(γ12end)
        ...
        @.. broadcast = false u =
            γ12end[i] * u + γ22end[i] * tmp + γ32end[i] * uprev + β2end[i] * dt * k
    end
...
@muladd function perform_step!(
    integrator,
    cache::LowStorageRK3SpFSALConstantCache,
    repeat_step = false,
)
    @unpack t, dt, uprev, u, f, p = integrator
    @unpack γ12end, γ22end, γ32end, δ2end, β1, β2end, c2end, bhat1, bhat2end, bhatfsal =
        cache
...
@muladd function perform_step!(
    integrator,
    cache::LowStorageRK3SpFSALCache,
    repeat_step = false,
)
    @unpack t, dt, uprev, u, f, p = integrator
    @unpack k, tmp, utilde, atmp, stage_limiter!, step_limiter!, thread = cache
...
@muladd function perform_step!(
    integrator,
    cache::LowStorageRK2RPConstantCache,
    repeat_step = false,
)
...
@muladd function perform_step!(integrator, cache::LowStorageRK2RPCache, repeat_step = false)
    ...
    for i in eachindex(Aᵢ)
        integrator.opts.adaptive &&
            (@.. broadcast = false tmp = tmp + (Bᵢ[i] - B̂ᵢ[i]) * dt * k)
...
@muladd function perform_step!(
    integrator,
    cache::LowStorageRK4RPConstantCache,
    repeat_step = false,
)
    @unpack t, dt, u, uprev, f, fsalfirst, p = integrator
...
@muladd function perform_step!(integrator, cache::LowStorageRK4RPCache, repeat_step = false)
    ...
    for i in eachindex(Aᵢ₁)
        integrator.opts.adaptive &&
            (@.. broadcast = false tmp = tmp + (Bᵢ[i] - B̂ᵢ[i]) * dt * k)
        @.. broadcast = false gprev =
            uᵢ₋₃ + (Aᵢ₁[i] * k + Aᵢ₂[i] * fᵢ₋₂ + Aᵢ₃[i] * fᵢ₋₃) * dt
        @.. broadcast = false u = u + Bᵢ[i] * dt * k
...
@muladd function perform_step!(
    integrator,
    cache::LowStorageRK5RPConstantCache,
    repeat_step = false,
)
    @unpack t, dt, u, uprev, f, fsalfirst, p = integrator
...
@muladd function perform_step!(integrator, cache::LowStorageRK5RPCache, repeat_step = false)
    ...
    for i in eachindex(Aᵢ₁)
        integrator.opts.adaptive &&
            (@.. broadcast = false tmp = tmp + (Bᵢ[i] - B̂ᵢ[i]) * dt * k)
        @.. broadcast = false gprev =
            uᵢ₋₄ + (Aᵢ₁[i] * k + Aᵢ₂[i] * fᵢ₋₂ + Aᵢ₃[i] * fᵢ₋₃ + Aᵢ₄[i] * fᵢ₋₄) * dt
        @.. broadcast = false u = u + Bᵢ[i] * dt * k
        @.. broadcast = false fᵢ₋₄ = fᵢ₋₃
        @.. broadcast = false fᵢ₋₃ = fᵢ₋₂
        @.. broadcast = false fᵢ₋₂ = k
