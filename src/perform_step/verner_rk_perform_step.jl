...
@muladd function perform_step!(integrator, cache::Vern6Cache, repeat_step = false)
    ...
    if integrator.opts.adaptive
        @.. broadcast=false utilde=dt * (btilde1 * k1 + btilde4 * k4 + btilde5 * k5 +
...
@muladd function perform_step!(integrator, cache::Vern7Cache, repeat_step = false)
    ...
    f(k6, tmp, p, t + c6 * dt)
    @.. broadcast=false tmp=uprev +
                            dt * (a071 * k1 + a073 * k3 + a074 * k4 + a075 * k5 + a076 * k6)
    f(k7, tmp, p, t + c7 * dt)
    @.. broadcast=false tmp=uprev +
                            dt *
...
@muladd function perform_step!(integrator, cache::Vern7Cache{<:Array}, repeat_step = false)
    ...
    @inbounds @simd ivdep for i in eachindex(u)
        tmp[i] = uprev[i] +
                 dt *
                 (a071 * k1[i] + a073 * k3[i] + a074 * k4[i] + a075 * k5[i] + a076 * k6[i])
    end
    f(k7, tmp, p, t + c7 * dt)
    ...
    @inbounds @simd ivdep for i in eachindex(u)
        tmp[i] = uprev[i] +
                 dt *
                 (a081 * k1[i] + a083 * k3[i] + a084 * k4[i] + a085 * k5[i] + a086 * k6[i] +
                  a087 * k7[i])
    end
    f(k8, tmp, p, t + c8 * dt)
    ...
    @inbounds @simd ivdep for i in eachindex(u)
        tmp[i] = uprev[i] +
                 dt *
                 (a091 * k1[i] + a093 * k3[i] + a094 * k4[i] + a095 * k5[i] + a096 * k6[i] +
                  a097 * k7[i] + a098 * k8[i])
    end
    f(k9, tmp, p, t + dt)
    ...
    @inbounds @simd ivdep for i in eachindex(u)
        tmp[i] = uprev[i] +
                 dt *
                 (a101 * k1[i] + a103 * k3[i] + a104 * k4[i] + a105 * k5[i] + a106 * k6[i] +
                  a107 * k7[i])
    end
    f(k10, tmp, p, t + dt)
    ...
    @inbounds @simd ivdep for i in eachindex(u)
        u[i] = uprev[i] +
               dt * (b1 * k1[i] + b4 * k4[i] + b5 * k5[i] + b6 * k6[i] + b7 * k7[i] +
                b8 * k8[i] + b9 * k9[i])
    end
    integrator.destats.nf += 10
    ...
    if integrator.opts.adaptive
        @inbounds @simd ivdep for i in eachindex(u)
            utilde[i] = dt * (btilde1 * k1[i] + btilde4 * k4[i] + btilde5 * k5[i] +
                         btilde6 * k6[i] + btilde7 * k7[i] + btilde8 * k8[i] +
    ...
    if !alg.lazy && (integrator.opts.adaptive == false ||
        ...
        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a1101 * k[1][i] + a1104 * k[4][i] + a1105 * k[5][i] +
                      a1106 * k[6][i] + a1107 * k[7][i] + a1108 * k[8][i] + a1109 * k[9][i])
        end
        f(k[11], tmp, p, t + c11 * dt)

        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a1201 * k[1][i] + a1204 * k[4][i] + a1205 * k[5][i] +
                      a1206 * k[6][i] + a1207 * k[7][i] + a1208 * k[8][i] +
                      a1209 * k[9][i] + a1211 * k[11][i])
        end
        f(k[12], tmp, p, t + c12 * dt)

        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a1301 * k[1][i] + a1304 * k[4][i] + a1305 * k[5][i] +
                      a1306 * k[6][i] + a1307 * k[7][i] + a1308 * k[8][i] +
                      a1309 * k[9][i] + a1311 * k[11][i] + a1312 * k[12][i])
        end
        f(k[13], tmp, p, t + c13 * dt)

        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a1401 * k[1][i] + a1404 * k[4][i] + a1405 * k[5][i] +
                      a1406 * k[6][i] + a1407 * k[7][i] + a1408 * k[8][i] +
                      a1409 * k[9][i] + a1411 * k[11][i] + a1412 * k[12][i] +
                      a1413 * k[13][i])
        end
        f(k[14], tmp, p, t + c14 * dt)
        ...
        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a1501 * k[1][i] + a1504 * k[4][i] + a1505 * k[5][i] +
                      a1506 * k[6][i] + a1507 * k[7][i] + a1508 * k[8][i] +
                      a1509 * k[9][i] + a1511 * k[11][i] + a1512 * k[12][i] +
                      a1513 * k[13][i])
        end
        ...
        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a1601 * k[1][i] + a1604 * k[4][i] + a1605 * k[5][i] +
                      a1606 * k[6][i] + a1607 * k[7][i] + a1608 * k[8][i] +
                      a1609 * k[9][i] + a1611 * k[11][i] + a1612 * k[12][i] +
                      a1613 * k[13][i])
        end
        f(k[16], tmp, p, t + c16 * dt)
        integrator.destats.nf += 6
...
@muladd function perform_step!(integrator, cache::Vern8Cache, repeat_step = false)
    ...
    @.. broadcast=false tmp=uprev +
                            dt *
                            (a0801 * k1 + a0804 * k4 + a0805 * k5 + a0806 * k6 + a0807 * k7)
    f(k8, tmp, p, t + c8 * dt)
    @.. broadcast=false tmp=uprev +
                            dt * (a0901 * k1 + a0904 * k4 + a0905 * k5 + a0906 * k6 +
                             a0907 * k7 + a0908 * k8)
    f(k9, tmp, p, t + c9 * dt)
    @.. broadcast=false tmp=uprev +
                            dt * (a1001 * k1 + a1004 * k4 + a1005 * k5 + a1006 * k6 +
    ...
    @.. broadcast=false u=uprev +
                          dt *
                          (a1301 * k1 + a1304 * k4 + a1305 * k5 + a1306 * k6 + a1307 * k7 +
    ...
    @.. broadcast=false u=uprev +
                          dt *
                          (b1 * k1 + b6 * k6 + b7 * k7 + b8 * k8 + b9 * k9 + b10 * k10 +
                           b11 * k11 + b12 * k12)
    if integrator.opts.adaptive
        @.. broadcast=false utilde=dt * (btilde1 * k1 + btilde6 * k6 + btilde7 * k7 +
                                    btilde8 * k8 + btilde9 * k9 + btilde10 * k10 +
...
@muladd function perform_step!(integrator, cache::Vern9Cache, repeat_step = false)
    ...
    @.. broadcast=false tmp=uprev +
                            dt *
                            (a1001 * k1 + a1006 * k6 + a1007 * k7 + a1008 * k8 + a1009 * k9)
    f(k10, tmp, p, t + c9 * dt)
    @.. broadcast=false tmp=uprev +
                            dt * (a1101 * k1 + a1106 * k6 + a1107 * k7 + a1108 * k8 +
                             a1109 * k9 + a1110 * k10)
    f(k11, tmp, p, t + c10 * dt)
    @.. broadcast=false tmp=uprev +
                            dt * (a1201 * k1 + a1206 * k6 + a1207 * k7 + a1208 * k8 +
                             a1209 * k9 + a1210 * k10 + a1211 * k11)
    f(k12, tmp, p, t + c11 * dt)
    @.. broadcast=false tmp=uprev +
                            dt * (a1301 * k1 + a1306 * k6 + a1307 * k7 + a1308 * k8 +
                             a1309 * k9 + a1310 * k10 + a1311 * k11 + a1312 * k12)
    f(k13, tmp, p, t + c12 * dt)
    @.. broadcast=false tmp=uprev +
                            dt * (a1401 * k1 + a1406 * k6 + a1407 * k7 + a1408 * k8 +
                             a1409 * k9 + a1410 * k10 + a1411 * k11 + a1412 * k12 +
                             a1413 * k13)
    f(k14, tmp, p, t + c13 * dt)
    @.. broadcast=false tmp=uprev +
                            dt * (a1501 * k1 + a1506 * k6 + a1507 * k7 + a1508 * k8 +
    ...
    @.. broadcast=false u=uprev +
                          ...
                          (a1601 * k1 + a1606 * k6 + a1607 * k7 + a1608 * k8 + a1609 * k9 +
                           a1610 * k10 + a1611 * k11 + a1612 * k12 + a1613 * k13)
    f(k16, u, p, t + dt)
    integrator.destats.nf += 16
    if integrator.alg isa CompositeAlgorithm
        g16 = u
    ...
    if integrator.opts.adaptive
        @.. broadcast=false utilde=dt * (btilde1 * k1 + btilde8 * k8 + btilde9 * k9 +
...
@muladd function perform_step!(integrator, cache::Vern9Cache{<:Array}, repeat_step = false)
    ...
    @inbounds @simd ivdep for i in eachindex(u)
        tmp[i] = uprev[i] +
                 dt * (a0701 * k1[i] + a0704 * k4[i] + a0705 * k5[i] + a0706 * k6[i])
    end
    f(k7, tmp, p, t + c6 * dt)
    ...
    @inbounds @simd ivdep for i in eachindex(u)
        tmp[i] = uprev[i] +
                 dt * (a0901 * k1[i] + a0906 * k6[i] + a0907 * k7[i] + a0908 * k8[i])
    end
    f(k9, tmp, p, t + c8 * dt)

    @inbounds @simd ivdep for i in eachindex(u)
        tmp[i] = uprev[i] +
                 dt * (a1001 * k1[i] + a1006 * k6[i] + a1007 * k7[i] + a1008 * k8[i] +
                  a1009 * k9[i])
    end
    f(k10, tmp, p, t + c9 * dt)
    ...
    @inbounds @simd ivdep for i in eachindex(u)
        tmp[i] = uprev[i] +
                 dt * (a1101 * k1[i] + a1106 * k6[i] + a1107 * k7[i] + a1108 * k8[i] +
                  a1109 * k9[i] + a1110 * k10[i])
    end
    f(k11, tmp, p, t + c10 * dt)

    @inbounds @simd ivdep for i in eachindex(u)
        tmp[i] = uprev[i] +
                 dt * (a1201 * k1[i] + a1206 * k6[i] + a1207 * k7[i] + a1208 * k8[i] +
                  a1209 * k9[i] + a1210 * k10[i] + a1211 * k11[i])
    end
    f(k12, tmp, p, t + c11 * dt)

    @inbounds @simd ivdep for i in eachindex(u)
        tmp[i] = uprev[i] +
                 dt * (a1301 * k1[i] + a1306 * k6[i] + a1307 * k7[i] + a1308 * k8[i] +
                  a1309 * k9[i] + a1310 * k10[i] + a1311 * k11[i] + a1312 * k12[i])
    end
    f(k13, tmp, p, t + c12 * dt)

    @inbounds @simd ivdep for i in eachindex(u)
        tmp[i] = uprev[i] +
                 dt * (a1401 * k1[i] + a1406 * k6[i] + a1407 * k7[i] + a1408 * k8[i] +
                  a1409 * k9[i] + a1410 * k10[i] + a1411 * k11[i] + a1412 * k12[i] +
                  a1413 * k13[i])
    end
    f(k14, tmp, p, t + c13 * dt)
    ...
    @inbounds @simd ivdep for i in eachindex(u)
        tmp[i] = uprev[i] +
                 dt * (a1501 * k1[i] + a1506 * k6[i] + a1507 * k7[i] + a1508 * k8[i] +
                  a1509 * k9[i] + a1510 * k10[i] + a1511 * k11[i] + a1512 * k12[i] +
    ...
    @inbounds @simd ivdep for i in eachindex(u)
        u[i] = uprev[i] +
               dt * (a1601 * k1[i] + a1606 * k6[i] + a1607 * k7[i] + a1608 * k8[i] +
                a1609 * k9[i] + a1610 * k10[i] + a1611 * k11[i] + a1612 * k12[i] +
                a1613 * k13[i])
    end
    f(k16, u, p, t + dt)
    ...
    @inbounds @simd ivdep for i in eachindex(u)
        u[i] = uprev[i] +
               dt * (b1 * k1[i] + b8 * k8[i] + b9 * k9[i] + b10 * k10[i] + b11 * k11[i] +
                b12 * k12[i] + b13 * k13[i] + b14 * k14[i] + b15 * k15[i])
    end

    if integrator.opts.adaptive
        @inbounds @simd ivdep for i in eachindex(u)
            utilde[i] = dt * (btilde1 * k1[i] + btilde8 * k8[i] + btilde9 * k9[i] +
                         btilde10 * k10[i] + btilde11 * k11[i] + btilde12 * k12[i] +
                         btilde13 * k13[i] + btilde14 * k14[i] + btilde15 * k15[i] +
                         btilde16 * k16[i])
        end
        calculate_residuals!(atmp, utilde, uprev, u, integrator.opts.abstol,
                             integrator.opts.reltol, integrator.opts.internalnorm, t)
    ...
    if !alg.lazy && (integrator.opts.adaptive == false ||
        ...
        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a1701 * k[1][i] + a1708 * k[2][i] + a1709 * k[3][i] +
                      a1710 * k[4][i] + a1711 * k[5][i] + a1712 * k[6][i] +
                      a1713 * k[7][i] + a1714 * k[8][i] + a1715 * k[9][i])
        end
        f(k[11], tmp, p, t + c17 * dt)

        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a1801 * k[1][i] + a1808 * k[2][i] + a1809 * k[3][i] +
                      a1810 * k[4][i] + a1811 * k[5][i] + a1812 * k[6][i] +
                      a1813 * k[7][i] + a1814 * k[8][i] + a1815 * k[9][i] +
                      a1817 * k[11][i])
        end
        f(k[12], tmp, p, t + c18 * dt)
        ...
        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a1901 * k[1][i] + a1908 * k[2][i] + a1909 * k[3][i] +
                      a1910 * k[4][i] + a1911 * k[5][i] + a1912 * k[6][i] +
                      a1913 * k[7][i] + a1914 * k[8][i] + a1915 * k[9][i] +
                      a1917 * k[11][i] + a1918 * k[12][i])
        end
        f(k[13], tmp, p, t + c19 * dt)

        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a2001 * k[1][i] + a2008 * k[2][i] + a2009 * k[3][i] +
                      a2010 * k[4][i] + a2011 * k[5][i] + a2012 * k[6][i] +
                      a2013 * k[7][i] + a2014 * k[8][i] + a2015 * k[9][i] +
                      a2017 * k[11][i] + a2018 * k[12][i] + a2019 * k[13][i])
        end
        f(k[14], tmp, p, t + c20 * dt)

        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a2101 * k[1][i] + a2108 * k[2][i] + a2109 * k[3][i] +
                      a2110 * k[4][i] + a2111 * k[5][i] + a2112 * k[6][i] +
                      a2113 * k[7][i] + a2114 * k[8][i] + a2115 * k[9][i] +
                      a2117 * k[11][i] + a2118 * k[12][i] + a2119 * k[13][i] +
                      a2120 * k[14][i])
        end
        f(k[15], tmp, p, t + c21 * dt)
        ...
        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a2201 * k[1][i] + a2208 * k[2][i] + a2209 * k[3][i] +
                      a2210 * k[4][i] + a2211 * k[5][i] + a2212 * k[6][i] +
                      a2213 * k[7][i] + a2214 * k[8][i] + a2215 * k[9][i] +
                      a2217 * k[11][i] + a2218 * k[12][i] + a2219 * k[13][i] +
                      a2220 * k[14][i] + a2221 * k[15][i])
        end
        f(k[16], tmp, p, t + c22 * dt)

        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a2301 * k[1][i] + a2308 * k[2][i] + a2309 * k[3][i] +
                      a2310 * k[4][i] + a2311 * k[5][i] + a2312 * k[6][i] +
                      a2313 * k[7][i] + a2314 * k[8][i] + a2315 * k[9][i] +
                      a2317 * k[11][i] + a2318 * k[12][i] + a2319 * k[13][i] +
                      a2320 * k[14][i] + a2321 * k[15][i])
        end
        f(k[17], tmp, p, t + c23 * dt)

        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a2401 * k[1][i] + a2408 * k[2][i] + a2409 * k[3][i] +
                      a2410 * k[4][i] + a2411 * k[5][i] + a2412 * k[6][i] +
                      a2413 * k[7][i] + a2414 * k[8][i] + a2415 * k[9][i] +
                      a2417 * k[11][i] + a2418 * k[12][i] + a2419 * k[13][i] +
                      a2420 * k[14][i] + a2421 * k[15][i])
        end
        f(k[18], tmp, p, t + c24 * dt)

        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a2501 * k[1][i] + a2508 * k[2][i] + a2509 * k[3][i] +
                      a2510 * k[4][i] + a2511 * k[5][i] + a2512 * k[6][i] +
                      a2513 * k[7][i] + a2514 * k[8][i] + a2515 * k[9][i] +
                      a2517 * k[11][i] + a2518 * k[12][i] + a2519 * k[13][i] +
                      a2520 * k[14][i] + a2521 * k[15][i])
        end
        f(k[19], tmp, p, t + c25 * dt)

        @inbounds @simd ivdep for i in eachindex(u)
            tmp[i] = uprev[i] +
                     dt * (a2601 * k[1][i] + a2608 * k[2][i] + a2609 * k[3][i] +
                      a2610 * k[4][i] + a2611 * k[5][i] + a2612 * k[6][i] +
                      a2613 * k[7][i] + a2614 * k[8][i] + a2615 * k[9][i] +
                      a2617 * k[11][i] + a2618 * k[12][i] + a2619 * k[13][i] +
                      a2620 * k[14][i] + a2621 * k[15][i])
        end
        integrator.destats.nf += 10
        f(k[20], tmp, p, t + c26 * dt)
    end
