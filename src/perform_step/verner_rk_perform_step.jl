...
@muladd function perform_step!(integrator, cache::Vern7ConstantCache, repeat_step = false)
    ...
    g10 =
        uprev + dt * (a101 * k1 + a103 * k3 + a104 * k4 + a105 * k5 + a106 * k6 + a107 * k7)
    k9 = f(g9, p, t + dt)
    k10 = f(g10, p, t + dt)
...
@muladd function perform_step!(integrator, cache::Vern7Cache, repeat_step = false)
    ...
    f(k7, tmp, p, t + c7 * dt)
    @.. broadcast = false tmp =
        uprev + dt * (a081 * k1 + a083 * k3 + a084 * k4 + a085 * k5 + a086 * k6 + a087 * k7)
    f(k8, tmp, p, t + c8 * dt)
    @.. broadcast = false tmp =
        uprev +
        dt *
    ...
    f(k10, tmp, p, t + dt)
    @.. broadcast = false u =
        uprev + dt * (b1 * k1 + b4 * k4 + b5 * k5 + b6 * k6 + b7 * k7 + b8 * k8 + b9 * k9)
    integrator.destats.nf += 10
    if integrator.alg isa CompositeAlgorithm
        g10 = u
        g9 = tmp
...
@muladd function perform_step!(integrator, cache::Vern7Cache{<:Array}, repeat_step = false)
    ...
    @inbounds @simd ivdep for i in eachindex(u)
        tmp[i] =
            uprev[i] +
            dt * (a071 * k1[i] + a073 * k3[i] + a074 * k4[i] + a075 * k5[i] + a076 * k6[i])
    end
    f(k7, tmp, p, t + c7 * dt)
...
@muladd function perform_step!(integrator, cache::Vern8ConstantCache, repeat_step = false)
    ...
    k9 = f(
        uprev +
        dt * (a0901 * k1 + a0904 * k4 + a0905 * k5 + a0906 * k6 + a0907 * k7 + a0908 * k8),
        p,
        t + c9 * dt,
    )
    k10 = f(
...
@muladd function perform_step!(integrator, cache::Vern9ConstantCache, repeat_step = false)
    ...
    k11 = f(
        uprev +
        dt * (a1101 * k1 + a1106 * k6 + a1107 * k7 + a1108 * k8 + a1109 * k9 + a1110 * k10),
        p,
        t + c10 * dt,
    )
    k12 = f(
...
@muladd function perform_step!(integrator, cache::Vern9Cache{<:Array}, repeat_step = false)
    ...
    @inbounds @simd ivdep for i in eachindex(u)
        tmp[i] =
            uprev[i] + dt * (a0701 * k1[i] + a0704 * k4[i] + a0705 * k5[i] + a0706 * k6[i])
    end
    f(k7, tmp, p, t + c6 * dt)
    ...
    @inbounds @simd ivdep for i in eachindex(u)
        tmp[i] =
            uprev[i] + dt * (a0901 * k1[i] + a0906 * k6[i] + a0907 * k7[i] + a0908 * k8[i])
    end
    f(k9, tmp, p, t + c8 * dt)
    ...
    @inbounds @simd ivdep for i in eachindex(u)
        tmp[i] =
            uprev[i] +
            dt *
            (a1001 * k1[i] + a1006 * k6[i] + a1007 * k7[i] + a1008 * k8[i] + a1009 * k9[i])
    end
    f(k10, tmp, p, t + c9 * dt)
