...
@muladd function perform_step!(integrator, cache::Vern6Cache, repeat_step = false)
    ...
    if integrator.opts.adaptive
        @.. broadcast=false thread=thread utilde=dt * (btilde1 * k1 + btilde4 * k4 + btilde5 * k5 +
...
@muladd function perform_step!(integrator, cache::Vern7Cache, repeat_step = false)
    ...
    f(k5, tmp, p, t + c5 * dt)
    @.. broadcast=false thread=thread tmp=uprev + dt * (a061 * k1 + a063 * k3 + a064 * k4 + a065 * k5)
    f(k6, tmp, p, t + c6 * dt)
    @.. broadcast=false thread=thread tmp=uprev +
...
@muladd function perform_step!(integrator, cache::Vern8Cache, repeat_step = false)
    ...
    f(k4, tmp, p, t + c4 * dt)
    @.. broadcast=false thread=thread tmp=uprev + dt * (a0501 * k1 + a0503 * k3 + a0504 * k4)
    f(k5, tmp, p, t + c5 * dt)
    @.. broadcast=false thread=thread tmp=uprev + dt * (a0601 * k1 + a0604 * k4 + a0605 * k5)
    f(k6, tmp, p, t + c6 * dt)
    @.. broadcast=false thread=thread tmp=uprev + dt * (a0701 * k1 + a0704 * k4 + a0705 * k5 + a0706 * k6)
    f(k7, tmp, p, t + c7 * dt)
    @.. broadcast=false thread=thread tmp=uprev +
                            dt *
...
@muladd function perform_step!(integrator, cache::Vern9Cache, repeat_step = false)
    ...
    @.. broadcast=false thread=thread tmp=uprev + dt * (a0701 * k1 + a0704 * k4 + a0705 * k5 + a0706 * k6)
    f(k7, tmp, p, t + c6 * dt)
    @.. broadcast=false thread=thread tmp=uprev + dt * (a0801 * k1 + a0806 * k6 + a0807 * k7)
    f(k8, tmp, p, t + c7 * dt)
    @.. broadcast=false thread=thread tmp=uprev + dt * (a0901 * k1 + a0906 * k6 + a0907 * k7 + a0908 * k8)
    f(k9, tmp, p, t + c8 * dt)
    @.. broadcast=false thread=thread tmp=uprev +
                            dt *
    ...
    if integrator.opts.adaptive
        @.. broadcast=false thread=thread utilde=dt * (btilde1 * k1 + btilde8 * k8 + btilde9 * k9 +
