...
@muladd function perform_step!(integrator, cache::BS3Cache, repeat_step = false)
    ...
    integrator.destats.nf += 3
    if integrator.opts.adaptive
        @.. broadcast = false thread = thread utilde =
            dt * (btilde1 * k1 + btilde2 * k2 + btilde3 * k3 + btilde4 * k4)
        calculate_residuals!(
            atmp,
            utilde,
            uprev,
...
function initialize!(integrator, cache::OwrenZen3ConstantCache)
    ...
    integrator.fsallast = zero(integrator.fsalfirst)
    integrator.k[1] = integrator.fsalfirst
    @inbounds for i = 2:integrator.kshortsize-1
...
@muladd function perform_step!(integrator, cache::OwrenZen3Cache, repeat_step = false)
    ...
    f(k3, tmp, p, t + c2 * dt)
    @.. broadcast = false u = uprev + dt * (a41 * k1 + a42 * k2 + a43 * k3)
    f(k4, u, p, t + dt)
    integrator.destats.nf += 3
    if integrator.opts.adaptive
        @.. broadcast = false utilde = dt * (btilde1 * k1 + btilde2 * k2 + btilde3 * k3)
        calculate_residuals!(
            atmp,
            utilde,
            uprev,
...
@muladd function perform_step!(integrator, cache::OwrenZen4Cache, repeat_step = false)
    ...
    f(k5, tmp, p, t + c4 * dt)
    @.. broadcast = false u = uprev + dt * (a61 * k1 + a63 * k3 + a64 * k4 + a65 * k5)
    f(k6, u, p, t + dt)
    integrator.destats.nf += 5
    if integrator.opts.adaptive
        @.. broadcast = false utilde =
            dt * (btilde1 * k1 + btilde3 * k3 + btilde4 * k4 + btilde5 * k5)
        calculate_residuals!(
            atmp,
            utilde,
            uprev,
...
@muladd function perform_step!(
    ...
    k5 = f(uprev + dt * (a51 * k1 + a52 * k2 + a53 * k3 + a54 * k4), p, t + c4 * dt)
    k6 = f(
        uprev + dt * (a61 * k1 + a62 * k2 + a63 * k3 + a64 * k4 + a65 * k5),
        p,
        t + c5 * dt,
    )
    k7 = f(
...
@muladd function perform_step!(integrator, cache::BS5Cache, repeat_step = false)
    ...
    if integrator.opts.adaptive
        @.. broadcast = false utilde =
            dt * (bhat1 * k1 + bhat3 * k3 + bhat4 * k4 + bhat5 * k5 + bhat6 * k6)
        calculate_residuals!(
            atmp,
            utilde,
            uprev,
...
@muladd function perform_step!(integrator, cache::Tsit5ConstantCache, repeat_step = false)
    ...
    if typeof(integrator.alg) <: CompositeAlgorithm
        ...
        integrator.eigen_est =
            integrator.opts.internalnorm(k7 - k6, t) /
...
@muladd function perform_step!(integrator, cache::Tsit5Cache, repeat_step = false)
    ...
    f(k3, tmp, p, t + c2 * dt)
    @.. broadcast = false thread = thread tmp =
        uprev + dt * (a41 * k1 + a42 * k2 + a43 * k3)
...
@muladd function perform_step!(
    integrator,
    cache::Tsit5Cache{uType,rateType,uNoUnitsType,TabType,StageLimiter,StepLimiter,Thread},
    repeat_step = false,
) where {uType<:Array,rateType,uNoUnitsType,TabType,StageLimiter,StepLimiter,Thread<:False}
    @unpack t, dt, uprev, u, f, p = integrator
    uidx = eachindex(integrator.uprev)
    @unpack c1,
    c2,
...
@muladd function perform_step!(integrator, cache::DP5Cache, repeat_step = false)
    ...
    @.. broadcast = false u = uprev + dt * update
    f(k7, u, p, t + dt)
    integrator.destats.nf += 6
    if integrator.alg isa CompositeAlgorithm
        g6 = tmp
        g7 = u
        # Hairer II, page 22
        @.. broadcast = false g6 = g7 - g6
        ϱd = integrator.opts.internalnorm(g6, t)
        @.. broadcast = false tmp = k7 - k6
    ...
    if integrator.opts.calck
        #integrator.k[4] == k5
        @.. broadcast = false integrator.k[4] =
            d1 * k1 + d3 * k3 + d4 * k4 + d5 * k5 + d6 * k6 + d7 * k7
        #bspl == k3
        @.. broadcast = false bspl = k1 - update
        # k6 === integrator.k[3] === k2
        @.. broadcast = false integrator.k[3] = update - k7 - bspl
    end
    return nothing
end
...
@muladd function perform_step!(
    integrator,
    cache::KYK2014DGSSPRK_3S2_Cache,
    repeat_step = false,
)
    @unpack t, dt, uprev, u, f, p = integrator
    ...
    @.. broadcast = false u_1 = α_10 * uprev + dt * β_10 * integrator.fsalfirst
    f(kk_1, u_1, p, t + c_1 * dt)
    @.. broadcast = false u_2 = (α_20 * uprev + α_21 * u_1 + dt * β_21 * kk_1)
    f(kk_2, u_2, p, t + c_2 * dt)
    @.. broadcast = false integrator.u =
        (α_30 * uprev + dt * β_30 * integrator.fsalfirst + α_32 * u_2 + dt * β_32 * kk_2)
...
@muladd function perform_step!(integrator, cache::RKO65ConstantCache, repeat_step = false)
    ...
    k5 = f(
        uprev + α51 * dt * k1 + α52 * dt * k2 + α53 * dt * k3 + α54 * dt * k4,
        p,
        t + c5 * dt,
    )
    k6 = f(
        uprev + α62 * dt * k2 + α63 * dt * k3 + α64 * dt * k4 + α65 * dt * k5,
        p,
        t + c6 * dt,
    )
...
@muladd function perform_step!(integrator, cache::RKO65Cache, repeat_step = false)
    ...
    @.. broadcast = false u = uprev + dt * (β2 * k2 + β3 * k3 + β4 * k4 + β5 * k5 + β6 * k6)
...
@muladd function perform_step!(integrator, cache::FRK65ConstantCache, repeat_step = false)
    ...
    νsq = ν^2
    β4 =
        (d1 + νsq * (d2 + νsq * (d3 + νsq * (d4 + νsq * (d5 + νsq * (d6 + +νsq * d7)))))) /
        (1 + νsq * (d8 + νsq * (d9 + νsq * (d10 + νsq * (d11 + νsq * (d12 + +νsq * d13))))))
    β5 =
        (e1 + νsq * (e2 + νsq * (e3 + νsq * (e4 + νsq * (e5 + νsq * e6))))) /
        (1 + νsq * (e8 + νsq * (e9 + νsq * (e10 + νsq * e11))))
    β6 =
        (f1 + νsq * (f2 + νsq * (f3 + νsq * (f4 + νsq * (f5 + νsq * f6))))) /
    ...
    k6 = f(
        uprev + α61 * dt * k1 + α63 * dt * k3 + α64 * dt * k4 + α65 * dt * k5,
        p,
        t + c6 * dt,
    )
    k7 = f(
...
@muladd function perform_step!(integrator, cache::FRK65Cache, repeat_step = false)
    ...
    f(k8, tmp, p, t + c8 * dt)
    @.. broadcast = false u =
        uprev + dt * (β1 * k1 + β4 * k4 + β5 * k5 + β6 * k6 + β7 * k7 + β8 * k8)
    f(k9, u, p, t + dt)
...
@muladd function perform_step!(integrator, cache::RKMCache, repeat_step = false)
    ...
    f(k6, tmp, p, t + c6 * dt)
    @.. broadcast = false u = uprev + dt * (β1 * k1 + β2 * k2 + β3 * k3 + β4 * k4 + β6 * k6)
...
function perform_step!(integrator, cache::MSRK5Cache, repeat_step = false)
    ...
    f(k6, tmp, p, t + c6 * dt)
    @.. broadcast = false tmp =
        uprev + dt * (a71 * k1 + a73 * k3 + a74 * k4 + a75 * k5 + a76 * k6)
    f(k7, tmp, p, t + c7 * dt)
    @.. broadcast = false tmp =
        uprev + dt * (a81 * k1 + a83 * k3 + a84 * k4 + a85 * k5 + a86 * k6 + a87 * k7)
    f(k8, tmp, p, t + c8 * dt)
    @.. broadcast = false u =
        uprev + dt * (b1 * k1 + b4 * k4 + b5 * k5 + b6 * k6 + b7 * k7 + b8 * k8)
    f(k9, u, p, t + dt)
    integrator.destats.nf += 8
    integrator.fsallast = k9
