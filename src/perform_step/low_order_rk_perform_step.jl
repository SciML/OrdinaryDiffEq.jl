...
@muladd function perform_step!(integrator, cache::BS3ConstantCache, repeat_step=false)
  ...
  a1 = dt*a21
  k2 = f(uprev+a1*k1, t+c1*dt, integrator)
  a2 = dt*a32
  k3 = f(uprev+a2*k2, t+c2*dt, integrator)
  u = uprev+dt*(a41*k1+a42*k2+a43*k3)
  k4 = f(u, t+dt, integrator); integrator.fsallast = k4
...
@muladd function perform_step!(integrator, cache::OwrenZen4ConstantCache, repeat_step=false)
  ...
  k4 = f(uprev+dt*(a41*k1+a42*k2+a43*k3), t+c3*dt, integrator)
  k5 = f(uprev+dt*(a51*k1+a52*k2+a53*k3+a54*k4), t+c4*dt, integrator)
  u = uprev+dt*(a61*k1+a63*k3+a64*k4+a65*k5)
  k6 = f(u, t+dt, integrator); integrator.fsallast = k6
...
function initialize!(integrator,cache::OwrenZen4Cache,f=integrator.f)
  ...
  f(integrator.fsalfirst, integrator.uprev, integrator.t, integrator) # Pre-start fsal
  integrator.destats.nf += 1
end
...
@muladd function perform_step!(integrator, cache::OwrenZen5ConstantCache, repeat_step=false)
  ...
  k2 = f(uprev+a*k1, t+c1*dt, integrator)
  k3 = f(uprev+dt*(a31*k1+a32*k2), t+c2*dt, integrator)
  k4 = f(uprev+dt*(a41*k1+a42*k2+k3), t+c3*dt, integrator)
  k5 = f(uprev+dt*(a51*k1+a52*k2+a53*k3+a54*k4), t+c4*dt, integrator)
  k6 = f(uprev+dt*(a61*k1+a62*k2+a63*k3+a64*k4+a65*k5), t+c5*dt, integrator)
  k7 = f(uprev+dt*(a71*k1+a72*k2+a73*k3+a74*k4+a75*k5+a76*k6), t+c6*dt, integrator)
  u = uprev+dt*(a81*k1+a83*k3+a84*k4+a85*k5+a86*k6+a87*k7)
  k8 = f(u, t+dt, integrator); integrator.fsallast = k8
...
@muladd function perform_step!(integrator, cache::BS5ConstantCache, repeat_step=false)
  ...
  k5 = f(uprev+dt*(a51*k1+a52*k2+a53*k3+a54*k4), t+c4*dt, integrator)
  k6 = f(uprev+dt*(a61*k1+a62*k2+a63*k3+a64*k4+a65*k5), t+c5*dt, integrator)
  k7 = f(uprev+dt*(a71*k1+a72*k2+a73*k3+a74*k4+a75*k5+a76*k6), t+dt, integrator)
  integrator.destats.nf += 6
  u = uprev+dt*(a81*k1+a83*k3+a84*k4+a85*k5+a86*k6+a87*k7)
  integrator.fsallast = f(u, t+dt, integrator); k8 = integrator.fsallast
  integrator.destats.nf += 1
  if integrator.opts.adaptive
  ...
  if !alg.lazy && (integrator.opts.adaptive == false || integrator.EEst <= 1.0)
    ...
    k = integrator.k
    k[9] = f(uprev+dt*(a91*k[1]+a92*k[2]+a93*k[3]+a94*k[4]+a95*k[5]+a96*k[6]+a97*k[7]+a98*k[8]), t+c6*dt, integrator)
    k[10] = f(uprev+dt*(a101*k[1]+a102*k[2]+a103*k[3]+a104*k[4]+a105*k[5]+a106*k[6]+a107*k[7]+a108*k[8]+a109*k[9]), t+c7*dt, integrator)
    k[11] = f(uprev+dt*(a111*k[1]+a112*k[2]+a113*k[3]+a114*k[4]+a115*k[5]+a116*k[6]+a117*k[7]+a118*k[8]+a119*k[9]+a1110*k[10]), t+c8*dt, integrator)
    integrator.destats.nf += 3
  end
end
...
@muladd function perform_step!(integrator, cache::Tsit5ConstantCache, repeat_step=false)
  ...
  k6 = f(g6, t+dt, integrator)
  u = uprev+dt*(a71*k1+a72*k2+a73*k3+a74*k4+a75*k5+a76*k6)
  integrator.fsallast = f(u, t+dt, integrator); k7 = integrator.fsallast
  integrator.destats.nf += 6
...
@muladd function perform_step!(integrator,cache::KYK2014DGSSPRK_3S2_ConstantCache,repeat_step=false)
 ...
 u_2 = (
  α_20 * uprev +
  α_21 * u_1 + dt*β_21*f(u_1, t + c_1*dt, integrator)
 )
 integrator.u = (
  α_30 * uprev + dt*β_30*integrator.fsalfirst +
  α_32 * u_2 + dt*β_32*f(u_2, t + c_2*dt, integrator)
 )
 integrator.k[1] = integrator.fsalfirst
 integrator.k[2] = f(integrator.u, t+dt, integrator) # For interpolation, then FSAL'd
 integrator.destats.nf += 3
...
@muladd function perform_step!(integrator,cache::KYK2014DGSSPRK_3S2_Cache,repeat_step=false)
 ...
 @.. u_1 =  α_10 * uprev + dt*β_10*integrator.fsalfirst
 f(kk_1, u_1, t + c_1*dt, integrator)
 @.. u_2 = (
  α_20 * uprev +
  α_21 * u_1 + dt*β_21*kk_1
 )
 f(kk_2, u_2, t + c_2*dt, integrator)
 @.. integrator.u = (
  α_30 * uprev + dt*β_30*integrator.fsalfirst +
  α_32 * u_2 + dt*β_32*kk_2
 )
 f(integrator.k[2], integrator.u, t+dt, integrator) # For interpolation, then FSAL'd
 integrator.destats.nf += 3
 return nothing
end
