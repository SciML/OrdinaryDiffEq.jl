...
function initialize!(integrator,cache::DGLDDRK84_FConstantCache)
  integrator.fsalfirst = integrator.f(integrator.uprev, integrator.p, integrator.t) # Pre-start fsal
  integrator.kshortsize = 1
  integrator.k = typeof(integrator.k)(undef, integrator.kshortsize)
...
@muladd function perform_step!(integrator,cache::DGLDDRK84_FConstantCache,repeat_step=false)
  @unpack t,dt,uprev,u,f,p = integrator
...
function initialize!(integrator,cache::DGLDDRK84_FCache)
  @unpack k,fsalfirst = cache
  integrator.fsalfirst = fsalfirst
  integrator.fsallast = k
  integrator.kshortsize = 1
...
@muladd function perform_step!(integrator,cache::DGLDDRK84_FCache,repeat_step=false)
  @unpack t,dt,uprev,u,f,p = integrator
  @unpack k,fsalfirst,tmp = cache
