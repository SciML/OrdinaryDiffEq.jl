function initialize!(integrator, cache::Union{Rosenbrock23Cache,Rosenbrock32Cache})
...
function initialize!(
    integrator,
    cache::Union{Rosenbrock23ConstantCache,Rosenbrock32ConstantCache},
)
    integrator.kshortsize = 2
...
@muladd function perform_step!(integrator, cache::Rosenbrock23Cache, repeat_step = false)
    ...
    if integrator.opts.adaptive
        ...
        if mass_matrix === I
            @.. broadcast = false linsolve_tmp =
                fsallast - c₃₂ * (k₂ - f₁) - 2(k₁ - fsalfirst) + dt * dT
        else
            @.. broadcast = false du2 = c₃₂ * k₂ + 2k₁
            mul!(_vec(du1), mass_matrix, _vec(du2))
            @.. broadcast = false linsolve_tmp =
                fsallast - du1 + c₃₂ * f₁ + 2fsalfirst + dt * dT
        end
        ...
        if mass_matrix === I
            ...
            veck₃ = _vec(k₃)
            vectmp = _vec(tmp)
            @.. broadcast = false vectmp =
                ifelse(cache.algebraic_vars, dto6 * (veck₁ - 2 * veck₂ + veck₃))
        end
        calculate_residuals!(
            atmp,
            tmp,
...
@muladd function perform_step!(
    ...
    if integrator.opts.adaptive
        ...
        if mass_matrix === I
            @inbounds @simd ivdep for i in eachindex(u)
                linsolve_tmp[i] =
                    fsallast[i] - c₃₂ * (k₂[i] - f₁[i]) - 2(k₁[i] - fsalfirst[i]) +
                    dt * dT[i]
            end
        else
            @inbounds @simd ivdep for i in eachindex(u)
            ...
            @inbounds @simd ivdep for i in eachindex(u)
                linsolve_tmp[i] =
                    fsallast[i] - du1[i] + c₃₂ * f₁[i] + 2fsalfirst[i] + dt * dT[i]
            end
        end
        ...
        if mass_matrix === I
            ...
            veck₃ = _vec(k₃)
            vectmp = _vec(tmp)
            @inbounds @simd ivdep for i = 1:length(vectmp)
                vectmp[i] = ifelse(
                    cache.algebraic_vars[i],
                    false,
...
@muladd function perform_step!(
    integrator,
    cache::Rosenbrock23ConstantCache,
    repeat_step = false,
)
    ...
    if integrator.opts.adaptive
        ...
        if mass_matrix === I
            ...
            linsolve_tmp =
                integrator.fsallast - mass_matrix * (c₃₂ * k₂ + 2 * k₁) +
                c₃₂ * f₁ +
                2 * integrator.fsalfirst +
                dt * dT
...
@muladd function perform_step!(
    integrator,
    cache::Rosenbrock32ConstantCache,
    repeat_step = false,
)
    @unpack t, dt, uprev, u, f, p = integrator
    @unpack c₃₂, d, tf, uf = cache
    ...
    if mass_matrix === I
        k₃ = _reshape(
            W \
            -_vec((
                integrator.fsallast - c₃₂ * (k₂ - f₁) - 2(k₁ - integrator.fsalfirst) +
                dt * dT
            )),
            axes(uprev),
        )
        ...
        linsolve_tmp =
            integrator.fsallast - mass_matrix * (c₃₂ * k₂ + 2k₁) +
            c₃₂ * f₁ +
            2 * integrator.fsalfirst +
            dt * dT
        k₃ = _reshape(W \ -_vec(linsolve_tmp), axes(uprev))
...
function initialize!(
    integrator,
    cache::Union{Rosenbrock33Cache,Rosenbrock34Cache,Rosenbrock4Cache},
)
    integrator.kshortsize = 2
...
@muladd function perform_step!(
    integrator,
    cache::Rosenbrock33ConstantCache,
    repeat_step = false,
)
    @unpack t, dt, uprev, u, f, p = integrator
    @unpack tf, uf = cache
...
@muladd function perform_step!(
    integrator,
    cache::Rosenbrock34ConstantCache,
    repeat_step = false,
)
    @unpack t, dt, uprev, u, f, p = integrator
    @unpack tf, uf = cache
...
@muladd function perform_step!(integrator, cache::Rosenbrock34Cache, repeat_step = false)
    ...
    if mass_matrix === I
        @.. broadcast = false linsolve_tmp =
            du + dtd4 * dT + dtC41 * k1 + dtC42 * k2 + dtC43 * k3
    else
...
@muladd function perform_step!(integrator, cache::Rodas4ConstantCache, repeat_step = false)
    ...
    if mass_matrix === I
        ...
        linsolve_tmp =
            du + mass_matrix * (dtC52 * k2 + dtC54 * k4 + dtC51 * k1 + dtC53 * k3)
    end
    ...
    if mass_matrix === I
        ...
        linsolve_tmp =
            du +
            mass_matrix * (dtC61 * k1 + dtC62 * k2 + dtC65 * k5 + dtC64 * k4 + dtC63 * k3)
    end
...
@muladd function perform_step!(integrator, cache::Rodas4Cache, repeat_step = false)
    ...
    if mass_matrix === I
        @.. broadcast = false linsolve_tmp =
            du + (dtC52 * k2 + dtC54 * k4 + dtC51 * k1 + dtC53 * k3)
    else
    ...
    if mass_matrix === I
        @.. broadcast = false linsolve_tmp =
            du + (dtC61 * k1 + dtC62 * k2 + dtC65 * k5 + dtC64 * k4 + dtC63 * k3)
    else
        @.. broadcast = false du1 =
    ...
    if integrator.opts.calck
        @unpack h21, h22, h23, h24, h25, h31, h32, h33, h34, h35 = cache.tab
        @.. broadcast = false integrator.k[1] =
            h21 * k1 + h22 * k2 + h23 * k3 + h24 * k4 + h25 * k5
        @.. broadcast = false integrator.k[2] =
            h31 * k1 + h32 * k2 + h33 * k3 + h34 * k4 + h35 * k5
    end
    cache.linsolve = linres.cache
end
...
@muladd function perform_step!(integrator, cache::Rodas4Cache{<:Array}, repeat_step = false)
    ...
    if mass_matrix === I
        @inbounds @simd ivdep for i in eachindex(u)
            linsolve_tmp[i] =
                du[i] + dtd4 * dT[i] + (dtC41 * k1[i] + dtC42 * k2[i] + dtC43 * k3[i])
        end
    else
        @inbounds @simd ivdep for i in eachindex(u)
    ...
    if mass_matrix === I
        @inbounds @simd ivdep for i in eachindex(u)
            linsolve_tmp[i] =
                du[i] + (dtC52 * k2[i] + dtC54 * k4[i] + dtC51 * k1[i] + dtC53 * k3[i])
        end
    else
        @inbounds @simd ivdep for i in eachindex(u)
    ...
    if integrator.opts.calck
        ...
        @inbounds @simd ivdep for i in eachindex(u)
            integrator.k[1][i] =
                h21 * k1[i] + h22 * k2[i] + h23 * k3[i] + h24 * k4[i] + h25 * k5[i]
            integrator.k[2][i] =
                h31 * k1[i] + h32 * k2[i] + h33 * k3[i] + h34 * k4[i] + h35 * k5[i]
        end
    end
    cache.linsolve = linres.cache
end
...
@muladd function perform_step!(
    integrator,
    cache::Rosenbrock5ConstantCache,
    repeat_step = false,
)
    @unpack t, dt, uprev, u, f, p = integrator
    @unpack tf, uf = cache
    ...
    if mass_matrix === I
        ...
        linsolve_tmp =
            du +
            mass_matrix * (dtC61 * k1 + dtC62 * k2 + dtC63 * k3 + dtC64 * k4 + dtC65 * k5)
    end
...
@muladd function perform_step!(integrator, cache::Rosenbrock5Cache, repeat_step = false)
    ...
    if mass_matrix === I
        @.. broadcast = false linsolve_tmp =
            du + dtd5 * dT + (dtC52 * k2 + dtC54 * k4 + dtC51 * k1 + dtC53 * k3)
    else
        @.. broadcast = false du1 = dtC52 * k2 + dtC54 * k4 + dtC51 * k1 + dtC53 * k3
        mul!(_vec(du2), mass_matrix, _vec(du1))
    ...
    if mass_matrix === I
        @.. broadcast = false linsolve_tmp =
            du + (dtC61 * k1 + dtC62 * k2 + dtC63 * k3 + dtC64 * k4 + dtC65 * k5)
    else
        @.. broadcast = false du1 =
    ...
    if integrator.opts.calck
        ...
        @.. broadcast = false integrator.k[2] =
            ...
            h37 * k7 +
            h38 * k8
        @.. broadcast = false integrator.k[3] =
            h41 * k1 +
            h42 * k2 +
            h43 * k3 +
            h44 * k4 +
...
@muladd function perform_step!(
    ...
    if mass_matrix === I
        @inbounds @simd ivdep for i in eachindex(u)
            linsolve_tmp[i] =
                du[i] +
                dtd5 * dT[i] +
                (dtC52 * k2[i] + dtC54 * k4[i] + dtC51 * k1[i] + dtC53 * k3[i])
        end
    else
    ...
    @inbounds @simd ivdep for i in eachindex(u)
        u[i] =
            uprev[i] + a61 * k1[i] + a62 * k2[i] + a63 * k3[i] + a64 * k4[i] + a65 * k5[i]
    end
    f(du, u, p, t + dt)
    integrator.destats.nf += 1
