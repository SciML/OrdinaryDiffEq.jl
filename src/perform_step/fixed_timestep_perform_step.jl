...
@muladd function perform_step!(integrator, cache::Anas5Cache, repeat_step = false)
    ...
    @tight_loop_macros for i in uidx
        @inbounds tmp[i] =
            uprev[i] + dt * (a51 * k1[i] + a52 * k2[i] + a53 * k3[i] + a54 * k4[i])
    end
    f(k5, tmp, p, t + c5 * dt)
    @tight_loop_macros for i in uidx
        @inbounds tmp[i] =
    ...
    @tight_loop_macros for i in uidx
        @inbounds u[i] =
            uprev[i] + dt * (b1 * k1[i] + b3 * k3[i] + b4 * k4[i] + b5 * k5[i] + b6 * k6[i])
    end
    f(k7, u, p, t + dt)
    integrator.destats.nf += 6
end
