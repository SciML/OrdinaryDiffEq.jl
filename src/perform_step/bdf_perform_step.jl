...
@muladd function perform_step!(integrator, cache::ABDF2ConstantCache, repeat_step = false)
    ...
    if mass_matrix === I
        nlsolver.tmp =
            @.. broadcast = false ((dtₙ * β₁) * fₙ₋₁ + (α₁ * uₙ₋₁ + α₂ * uₙ₋₂)) / (dtₙ * β₀)
    else
...
function perform_step!(integrator, cache::SBDFConstantCache, repeat_step = false)
    ...
    elseif cnt == 3
        tmp =
            γ * (3 * uprev - 3 // 2 * uprev2 + 1 // 3 * uprev3 + dt * (3 * (du₂ - k₁) + k₂))
    else
        tmp =
            γ * (
                4 * uprev - 3 * uprev2 + 4 // 3 * uprev3 - 1 // 4 * uprev4 +
                dt * (4 * du₂ - 6 * k₁ + 4 * k₂ - k₃)
            )
    end
    nlsolver.tmp = tmp
...
function perform_step!(
    integrator,
    cache::QNDFConstantCache{max_order},
    repeat_step = false,
) where {max_order}
    @unpack t, dt, uprev, u, f, p = integrator
    ...
    if integrator.opts.adaptive
        ...
        if k < max_order
            @views atmpp1 =
                calculate_residuals(D[:, k+2], uprev, u, abstol, reltol, internalnorm, t)
...
function perform_step!(
    ...
    if integrator.opts.adaptive
        ...
        if cache.consfailcnt > 1 && mass_matrix !== I
            ...
            calculate_residuals!(
                atmp,
                mul!(nlsolver.tmp, mass_matrix, dd),
                uprev,
                u,
                abstol,
                reltol,
...
function perform_step!(
    integrator,
    cache::FBDFConstantCache{max_order},
    repeat_step = false,
) where {max_order}
    @unpack ts,
    u_history,
