...
@muladd function perform_step!(
    ...
    if integrator.opts.adaptive && integrator.success_iter > 0
        ...
        tmp =
            r * integrator.opts.internalnorm.((u - uprev) / dt1 - (uprev - uprev2) / dt2, t)
...
@muladd function perform_step!(
    integrator,
    cache::ImplicitMidpointConstantCache,
    repeat_step = false,
)
    @unpack t, dt, uprev, u, f, p = integrator
    nlsolver = cache.nlsolver
...
@muladd function perform_step!(integrator, cache::TrapezoidCache, repeat_step = false)
    ...
    if integrator.opts.adaptive
        if integrator.iter > 2
            ...
            @.. broadcast = false tmp =
                r * integrator.opts.internalnorm(
                    (
...
@muladd function perform_step!(integrator, cache::Hairer4ConstantCache, repeat_step = false)
    ...
    @unpack bhat1, bhat2, bhat3, bhat4, btilde1, btilde2, btilde3, btilde4, btilde5 =
        cache.tab
    nlsolver = cache.nlsolver
    alg = unwrap_alg(integrator, true)
...
@muladd function perform_step!(
    integrator,
    cache::ESDIRK54I8L2SAConstantCache,
    repeat_step = false,
)
    @unpack t, dt, uprev, u, f, p = integrator
    @unpack γ,
