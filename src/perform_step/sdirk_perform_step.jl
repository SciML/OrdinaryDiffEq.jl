...
@muladd function perform_step!(integrator, cache::ImplicitEulerCache, repeat_step=false)
  ...
  nlcache = nlsolve_cache(integrator.alg, cache, z, uprev, 1, 1, new_W)
  z,η,iter,fail_convergence = diffeq_nlsolve!(integrator, nlcache, cache, Val{:newton})
  fail_convergence && return
  @. u = uprev + z
...
@muladd function perform_step!(integrator, cache::ImplicitMidpointConstantCache, repeat_step=false)
  ...
  tmp = uprev
  nlcache = nlsolve_cache(integrator.alg, cache, z, tmp, W, γ, 1//2, true)
  z,η,iter,fail_convergence = diffeq_nlsolve!(integrator, nlcache, cache, Val{:newton})
  fail_convergence && return
  u = tmp + z
...
@muladd function perform_step!(integrator, cache::ImplicitMidpointCache, repeat_step=false)
  ...
  nlcache = nlsolve_cache(integrator.alg, cache, z, uprev, γ, 1//2, new_W)
  z,η,iter,fail_convergence = diffeq_nlsolve!(integrator, nlcache, cache, Val{:newton})
  fail_convergence && return
  @. u = uprev + z
...
@muladd function perform_step!(integrator, cache::TRBDF2ConstantCache, repeat_step=false)
  ...
  tmp = uprev + d*zprev
  nlcache = nlsolve_cache(integrator.alg, cache, zᵧ, tmp, W, d, γ, true)
...
@muladd function perform_step!(integrator, cache::TRBDF2Cache, repeat_step=false)
  ...
  @. tmp = uprev + d*zprev
  nlcache = nlsolve_cache(integrator.alg, cache, zᵧ, tmp, d, γ, new_W)
  zᵧ,η,iter,fail_convergence = diffeq_nlsolve!(integrator, nlcache, cache, Val{:newton})
  fail_convergence && return
  ...
  @. tmp = uprev + ω*zprev + ω*zᵧ
  nlcache = nlsolve_cache(integrator.alg, cache, z, tmp, d, 1, false)
  z,η,iter,fail_convergence = diffeq_nlsolve!(integrator, nlcache, cache, Val{:newton})
  fail_convergence && return
...
@muladd function perform_step!(integrator, cache::SDIRK2Cache, repeat_step=false)
  ...
  z₂ .= zero(u)
  @. tmp = uprev - z₁
  nlcache = nlsolve_cache(integrator.alg, cache, z₂, tmp, 1, 1, false)
  z₂,η,iter,fail_convergence = diffeq_nlsolve!(integrator, nlcache, cache, Val{:newton})
  fail_convergence && return
...
@muladd function perform_step!(integrator, cache::SSPSDIRK2ConstantCache, repeat_step=false)
  ...
  nlcache = nlsolve_cache(integrator.alg, cache, z₁, uprev, W, γ, 1, true)
  z₁,η,iter,fail_convergence = diffeq_nlsolve!(integrator, nlcache, cache, Val{:newton})
  fail_convergence && return
  ...
  tmp = uprev + z₁/2
  nlcache = nlsolve_cache(integrator.alg, cache, z₂, tmp, W, 1//4, 1, true)
  z₂,η,iter,fail_convergence = diffeq_nlsolve!(integrator, nlcache, cache, Val{:newton})
  fail_convergence && return
...
@muladd function perform_step!(integrator, cache::SSPSDIRK2Cache, repeat_step=false)
  ...
  ##### Step 1
  nlcache = nlsolve_cache(integrator.alg, cache, z₁, uprev, γ, 1, new_W)
  z₁,η,iter,fail_convergence = diffeq_nlsolve!(integrator, nlcache, cache, Val{:newton})
  fail_convergence && return
  ...
  @. tmp = uprev + z₁/2
  nlcache = nlsolve_cache(integrator.alg, cache, z₂, tmp, 1//4, 1, false)
  z₂,η,iter,fail_convergence = diffeq_nlsolve!(integrator, nlcache, cache, Val{:newton})
  fail_convergence && return
...
@muladd function perform_step!(integrator, cache::Cash4ConstantCache, repeat_step=false)
  ...
  nlcache = nlsolve_cache(integrator.alg, cache, z₁, uprev, W, γ, γ, true)
  z₁,η,iter,fail_convergence = diffeq_nlsolve!(integrator, nlcache, cache, Val{:newton})
  fail_convergence && return
  ...
  tmp = uprev + a21*z₁
  nlcache = nlsolve_cache(integrator.alg, cache, z₂, tmp, W, γ, c2, true)
  z₂,η,iter,fail_convergence = diffeq_nlsolve!(integrator, nlcache, cache, Val{:newton})
  fail_convergence && return
  ...
  nlcache = nlsolve_cache(integrator.alg, cache, z₃, tmp, W, γ, c3, true)
  z₃,η,iter,fail_convergence = diffeq_nlsolve!(integrator, nlcache, cache, Val{:newton})
  fail_convergence && return
  ...
  nlcache = nlsolve_cache(integrator.alg, cache, z₄, tmp, W, γ, c4, true)
  z₄,η,iter,fail_convergence = diffeq_nlsolve!(integrator, nlcache, cache, Val{:newton})
  fail_convergence && return
...
@muladd function perform_step!(integrator, cache::Cash4Cache, repeat_step=false)
  ...
  nlcache = nlsolve_cache(integrator.alg, cache, z₁, uprev, γ, γ, new_W)
  z₁,η,iter,fail_convergence = diffeq_nlsolve!(integrator, nlcache, cache, Val{:newton})
  fail_convergence && return
  ...
  @. tmp = uprev + a21*z₁
  nlcache = nlsolve_cache(integrator.alg, cache, z₂, tmp, γ, c2, false)
  z₂,η,iter,fail_convergence = diffeq_nlsolve!(integrator, nlcache, cache, Val{:newton})
  fail_convergence && return
  ...
  @. z₃ = z₁
  @. tmp = uprev + a31*z₁ + a32*z₂
  nlcache = nlsolve_cache(integrator.alg, cache, z₃, tmp, γ, c3, false)
  z₃,η,iter,fail_convergence = diffeq_nlsolve!(integrator, nlcache, cache, Val{:newton})
  fail_convergence && return
  ...
  @. tmp = uprev + a41*z₁ + a42*z₂ + a43*z₃
  nlcache = nlsolve_cache(integrator.alg, cache, z₄, tmp, γ, c4, false)
  z₄,η,iter,fail_convergence = diffeq_nlsolve!(integrator, nlcache, cache, Val{:newton})
  fail_convergence && return
  ...
  @. z₅ = b1hat2*z₁ + b2hat2*z₂ + b3hat2*z₃ + b4hat2*z₄
  tmp = uprev + a51*z₁ + a52*z₂ + a53*z₃ + a54*z₄
  nlcache = nlsolve_cache(integrator.alg, cache, z₅, tmp, γ, 1, false)
  z₅,η,iter,fail_convergence = diffeq_nlsolve!(integrator, nlcache, cache, Val{:newton})
  fail_convergence && return
