...
@muladd function compute_step!(nlsolver::NLSolver{<:NonlinearSolveAlg, false}, integrator)
    ...
    # `step!` can *land* in a terminal state as well as start in one. Checking only on entry
    # defers a failed step to the next call, which never comes: the iterate it leaves behind
    # is unmoved, so the outer displacement test accepts the stage first. An inner solver can
    # also give up *because* there is nothing left to correct — a line search collapses on a
    # stage that is already solved — so the residual, not the retcode alone, has the say.
...
@muladd function compute_step!(nlsolver::NLSolver{<:NonlinearSolveAlg, true}, integrator)
    ...
    # Read before `rescale_stale_W_rhs!`, which rewrites `fu` in place: the unscaled residual
    # is the one `step!` will produce again, so it is the like-for-like comparison.
    fnorm_prev = maxabs(get_fu(nlcache))
    γΔt = residual_to_z_scale(nlsolver, nlsolve_f(integrator) isa DAEFunction)
    rescale_stale_W_rhs!(nlcache, nlsolver, integrator, nlstep_data)
    step!(nlcache; recompute_jacobian)
