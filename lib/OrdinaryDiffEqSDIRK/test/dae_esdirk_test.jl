...
# Type parameters were removed from algorithm structs in v7, so full inference
# of the solution type is no longer guaranteed. Test that solve works correctly.
sol = solve(prob_mm, KenCarp47(), reltol = 1.0e-8, abstol = 1.0e-8)
sol = solve(prob_mm_oop, KenCarp47(), reltol = 1.0e-8, abstol = 1.0e-8)
