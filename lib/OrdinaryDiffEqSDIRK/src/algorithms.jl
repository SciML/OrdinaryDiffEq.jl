...
function ImplicitEuler(; chunk_size = Val{0}(), autodiff = Val{true}(),
    ...
    ImplicitEuler{_unwrap_val(chunk_size), _unwrap_val(autodiff), typeof(linsolve),
        ...
        nlsolve, extrapolant, controller, step_limiter!)
end
...
function TRBDF2(; chunk_size = Val{0}(), autodiff = Val{true}(), standardtag = Val{true}(),
    ...
    TRBDF2{_unwrap_val(chunk_size), _unwrap_val(autodiff), typeof(linsolve),
        typeof(nlsolve), diff_type, _unwrap_val(standardtag),
        _unwrap_val(concrete_jac), typeof(step_limiter!)}(linsolve, nlsolve,
...
but where reaction equations are sufficient stiff to justify implicit integration.""",
    ...
    controller = :PI,
    """)
struct SSPSDIRK2{CS, AD, F, F2, FDT, ST, CJ} <:
...
function SSPSDIRK2(; chunk_size = Val{0}(), autodiff = Val{true}(),
    ...
    SSPSDIRK2{_unwrap_val(chunk_size), _unwrap_val(autodiff), typeof(linsolve),
        typeof(nlsolve), diff_type, _unwrap_val(standardtag),
        _unwrap_val(concrete_jac)}(linsolve, nlsolve, smooth_est, extrapolant,
        controller)
end
...
@doc SDIRK_docstring("An A-L stable stiffly-accurate 3rd order ESDIRK method.",
    ...
    step_limiter! = trivial_limiter!,
    """)
struct Kvaerno3{CS, AD, F, F2, FDT, ST, CJ, StepLimiter} <:
       OrdinaryDiffEqNewtonAdaptiveAlgorithm{CS, AD, FDT, ST, CJ}
    linsolve::F
    nlsolve::F2
    smooth_est::Bool
...
@doc SDIRK_docstring("Third order method.",
    ...
    extrapolant = :linear,
    """)
struct CFNLIRK3{CS, AD, F, F2, FDT, ST, CJ} <:
       OrdinaryDiffEqNewtonAlgorithm{CS, AD, FDT, ST, CJ}
    linsolve::F
    nlsolve::F2
    extrapolant::Symbol
...
@doc SDIRK_docstring("An A-L stable 4th order SDIRK method.",
    ...
    embedding = 3,
    """)
struct Cash4{CS, AD, F, F2, FDT, ST, CJ} <:
       OrdinaryDiffEqNewtonAdaptiveAlgorithm{CS, AD, FDT, ST, CJ}
    linsolve::F
    nlsolve::F2
    smooth_est::Bool
...
@doc SDIRK_docstring("Method of order 4.",
    ...
    extrapolant = :linear,
    """)
struct SFSDIRK4{CS, AD, F, F2, FDT, ST, CJ} <:
       OrdinaryDiffEqNewtonAlgorithm{CS, AD, FDT, ST, CJ}
    linsolve::F
    nlsolve::F2
    extrapolant::Symbol
...
@doc SDIRK_docstring("Method of order 5.",
    ...
    extrapolant = :linear,
    """)
struct SFSDIRK5{CS, AD, F, F2, FDT, ST, CJ} <:
       OrdinaryDiffEqNewtonAlgorithm{CS, AD, FDT, ST, CJ}
    linsolve::F
    nlsolve::F2
    extrapolant::Symbol
...
@doc SDIRK_docstring("Method of order 6.",
    ...
    extrapolant = :linear,
    """)
struct SFSDIRK6{CS, AD, F, F2, FDT, ST, CJ} <:
       OrdinaryDiffEqNewtonAlgorithm{CS, AD, FDT, ST, CJ}
    linsolve::F
    nlsolve::F2
    extrapolant::Symbol
...
@doc SDIRK_docstring("Method of order 7.",
    ...
    extrapolant = :linear,
    """)
struct SFSDIRK7{CS, AD, F, F2, FDT, ST, CJ} <:
       OrdinaryDiffEqNewtonAlgorithm{CS, AD, FDT, ST, CJ}
    linsolve::F
    nlsolve::F2
    extrapolant::Symbol
...
@doc SDIRK_docstring("Method of order 8.",
    ...
    extrapolant = :linear,
    """)
struct SFSDIRK8{CS, AD, F, F2, FDT, ST, CJ} <:
       OrdinaryDiffEqNewtonAlgorithm{CS, AD, FDT, ST, CJ}
    linsolve::F
    nlsolve::F2
    extrapolant::Symbol
...
@doc SDIRK_docstring("An A-L stable 4th order SDIRK method.",
    ...
    controller = :PI,
    """)
struct Hairer4{CS, AD, F, F2, FDT, ST, CJ} <:
       OrdinaryDiffEqNewtonAdaptiveAlgorithm{CS, AD, FDT, ST, CJ}
    linsolve::F
    nlsolve::F2
    smooth_est::Bool
...
@doc SDIRK_docstring("An A-L stable 4th order SDIRK method.",
    ...
    controller = :PI,
    """)
struct Hairer42{CS, AD, F, F2, FDT, ST, CJ} <:
       OrdinaryDiffEqNewtonAdaptiveAlgorithm{CS, AD, FDT, ST, CJ}
    linsolve::F
    nlsolve::F2
    smooth_est::Bool
...
@doc SDIRK_docstring(
    ...
    controller = :PI,
    """)
struct KenCarp47{CS, AD, F, F2, FDT, ST, CJ} <:
       OrdinaryDiffEqNewtonAdaptiveAlgorithm{CS, AD, FDT, ST, CJ}
    linsolve::F
    nlsolve::F2
    smooth_est::Bool
...
@doc SDIRK_docstring(
    ...
    controller = :PI,
    """)
struct KenCarp58{CS, AD, F, F2, FDT, ST, CJ} <:
       OrdinaryDiffEqNewtonAdaptiveAlgorithm{CS, AD, FDT, ST, CJ}
    linsolve::F
    nlsolve::F2
    smooth_est::Bool
...
but are still being fully evaluated in context.",
    ...
    controller = :PI,
    """)
struct ESDIRK54I8L2SA{CS, AD, F, F2, FDT, ST, CJ} <:
       OrdinaryDiffEqNewtonAdaptiveAlgorithm{CS, AD, FDT, ST, CJ}
    linsolve::F
    nlsolve::F2
    extrapolant::Symbol
...
function ESDIRK54I8L2SA(; chunk_size = Val{0}(), autodiff = Val{true}(),
    ...
    ESDIRK54I8L2SA{_unwrap_val(chunk_size), _unwrap_val(autodiff), typeof(linsolve),
        typeof(nlsolve), diff_type, _unwrap_val(standardtag),
        _unwrap_val(concrete_jac)}(linsolve, nlsolve, extrapolant,
        controller)
end
...
but are still being fully evaluated in context.",
    ...
    controller = :PI,
    """)
struct ESDIRK436L2SA2{CS, AD, F, F2, FDT, ST, CJ} <:
       OrdinaryDiffEqNewtonAdaptiveAlgorithm{CS, AD, FDT, ST, CJ}
    linsolve::F
    nlsolve::F2
    extrapolant::Symbol
...
but are still being fully evaluated in context.",
    ...
    controller = :PI,
    """)
struct ESDIRK437L2SA{CS, AD, F, F2, FDT, ST, CJ} <:
       OrdinaryDiffEqNewtonAdaptiveAlgorithm{CS, AD, FDT, ST, CJ}
    linsolve::F
    nlsolve::F2
    extrapolant::Symbol
...
but are still being fully evaluated in context.",
    ...
    controller = :PI,
    """)
struct ESDIRK547L2SA2{CS, AD, F, F2, FDT, ST, CJ} <:
       OrdinaryDiffEqNewtonAdaptiveAlgorithm{CS, AD, FDT, ST, CJ}
    linsolve::F
    nlsolve::F2
    extrapolant::Symbol
...
Check issue https://github.com/SciML/OrdinaryDiffEq.jl/issues/1933 for more details.",
    ...
    controller = :PI,
    """)
struct ESDIRK659L2SA{CS, AD, F, F2, FDT, ST, CJ} <:
       OrdinaryDiffEqNewtonAdaptiveAlgorithm{CS, AD, FDT, ST, CJ}
    linsolve::F
    nlsolve::F2
    extrapolant::Symbol
