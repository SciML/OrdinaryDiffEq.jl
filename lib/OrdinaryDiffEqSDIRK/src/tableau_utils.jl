...
@inline function compute_sdirk_stage!(
        integrator, cache, nlsolver, stage::Int,
...
@inline function compute_stage_constantcache!(
        integrator, cache, stage::Int,
...
@inline function compute_stage_mutablecache!(
        integrator, cache, stage::Int,
        prev_z, coeffs, c_val, base_tmp
    )
    nlsolver = cache.nlsolver
...
@inline function compute_embedded_error!(integrator, cache, btilde_coeffs, z_stages)
    return if integrator.opts.adaptive
        ...
        if hasfield(typeof(cache), :atmp)
            calculate_residuals!(
                cache.atmp, est, integrator.uprev, integrator.u,
            ...
            atmp = calculate_residuals(
                est, integrator.uprev, integrator.u,
                integrator.opts.abstol, integrator.opts.reltol,
                integrator.opts.internalnorm, integrator.t
            )
