...
# `k` holds the node rates only while they describe the step being interpolated.
...
# A recompute request means a callback shortened the step, so the node rates no longer apply.
