...
@testset "SDC dense output is the collocation polynomial" begin
    ...
    # A split problem's node rates are the sum of both parts.
    split = SplitODEProblem(
        (du, u, p, t) -> (du[1] = -u[2]; du[2] = 0; nothing),
        (du, u, p, t) -> (du[1] = 0; du[2] = u[1]; nothing), [1.0, 0.0], (0.0, 4.0)
    )
    sol = solve(split, SDC(num_nodes = 3, num_sweeps = 6); dt = 0.2, adaptive = false)
    @test maximum(maximum(abs.(sol(t) .- rotation_exact(t, Val{0}))) for t in 0:0.01:4) < 1.0e-5
end
...
@testset "SDC dense output falls back when the node rates do not describe the step" begin
    ...
    # The event shortens the step it fires in, so its node rates no longer apply.
    ...
    @test maximum(
        maximum(abs.(last_node(prevfloat(last_node.t[i]), Val{1}) .- [-last_node.u[i][2], last_node.u[i][1]]))
            for i in 2:length(last_node.t)
    ) < 1.0e-10
end
...
@testset "SDC hands the interpolant derivatives" begin
    ...
    # Without `calck` nothing is stored and the interpolant computes its own derivatives.
    integrator = init(
        prob, SDC(num_nodes = 3, num_sweeps = 3);
        dt = 0.2, adaptive = false, dense = false, save_everystep = false
    )
    foreach(_ -> step!(integrator), 1:3)
    @test SciMLBase.get_du(integrator) ≈ [-integrator.u[2], integrator.u[1]] rtol = 1.0e-12
end
