...
# SDE initial timestep determination (stochastic Hairer-Wanner with diffusion terms).
# Differs from ode_determine_initdt in:
# - Jump shortcut (P !== nothing → dtmax/1e6)
# - Diffusion evaluation: g₀ = 3g(u0,p,t), included in d₁ as max(f₀+g₀, f₀-g₀)
# - Second diffusion eval with ΔgMax in d₂
# - Order exponent: order + 1//2 instead of order
# - No mass matrix handling

function sde_determine_initdt(
        u0, t, tdir, dtmax, abstol, reltol,
        internalnorm, prob::SciMLBase.AbstractRODEProblem{uType, tType, true},
        order, integrator
    ) where {tType, uType}
    _tType = eltype(tType)
    ...
    if max(d₁, d₂) <= T1(1 // Int64(10)^(15))
        ...
        dt₁ = _tType(10^(-(2 + log10(max(d₁, d₂))) / (order + 1 // 2)))
    end
...
function sde_determine_initdt(
        u0, t, tdir, dtmax, abstol, reltol,
        internalnorm, prob::SciMLBase.AbstractRODEProblem{uType, tType, false},
        order, integrator
    ) where {tType, uType}
    _tType = eltype(tType)
    if _get_P(integrator) !== nothing
