...
function IController(alg; kwargs...)
    IController(Float64, alg; kwargs...)
end
...
function step_reject_controller!(integrator, cache::PIControllerCache, alg)
    ...
    integrator.dt *= max(qmin, q11 * gamma)
end
...
function LegacyPIDController(beta1, beta2, beta3 = zero(beta1);
        limiter = default_dt_factor_limiter,
        accept_safety = 0.81)
...
function Base.show(io::IO, controller::LegacyPIDController)
    print(io, "LegacyPIDController(beta=", controller.beta,
...
function PIDController(alg; kwargs...)
    PIDController(Float64, alg; kwargs...)
end
