...
@time begin
    ...
    if !is_APPVEYOR && TEST_GROUP == "WeakConvergence4"
    ...
    if !is_APPVEYOR && TEST_GROUP == "WeakConvergence5"
        @time @safetestset "Weak Stratonovich Tests" begin
            include("weak_convergence/weak_strat.jl")
        end
    end

    if !is_APPVEYOR && TEST_GROUP == "WeakConvergence6"
