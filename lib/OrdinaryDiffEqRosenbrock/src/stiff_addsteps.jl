...
function _ode_addsteps!(k, t, uprev, u, dt, f, p, cache::RosenbrockCache,
    ...
    if length(k) < 2 || always_calc_begin
        ...
        @.. broadcast=false veck1=-vecu
        @.. broadcast=false tmp=uprev + a21 * ks[1]
        f(du, tmp, p, t + c2 * dt)

        if mass_matrix === I
            @.. broadcast=false linsolve_tmp=du + dtd2 * dT + dtC21 * ks[1]
        else
            @.. broadcast=false du1=dtC21 * ks[1]
            mul!(du2, mass_matrix, du1)
            @.. broadcast=false linsolve_tmp=du + dtd2 * dT + du2
        end
        ...
        veck2 = _vec(k2)
        @.. broadcast=false veck2=-vecu
        @.. broadcast=false tmp=uprev + a31 * ks[1] + a32 * ks[2]
        f(du, tmp, p, t + c3 * dt)

        if mass_matrix === I
            @.. broadcast=false linsolve_tmp=du + dtd3 * dT + (dtC31 * ks[1] + dtC32 * ks[2])
        else
            @.. broadcast=false du1=dtC31 * ks[1] + dtC32 * ks[2]
            mul!(du2, mass_matrix, du1)
        ...
        veck3 = _vec(k3)
        @.. broadcast=false veck3=-vecu
        @.. broadcast=false tmp=uprev + a41 * ks[1] + a42 * ks[2] + a43 * ks[3]
        f(du, tmp, p, t + c4 * dt)
        ...
        veck4 = _vec(k4)
        @.. broadcast=false veck4=-vecu
        @.. broadcast=false tmp=uprev + a51 * ks[1] + a52 * ks[2] + a53 * ks[3] + a54 * ks[4]
        f(du, tmp, p, t + dt)

        if mass_matrix === I
            @.. broadcast=false linsolve_tmp=du + (dtC52 * ks[2] + dtC54 * ks[4] + dtC51 * ks[1] +
                                              dtC53 * ks[3])
        else
