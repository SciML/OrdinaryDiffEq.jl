...
@muladd function perform_step!(integrator, cache::RosenbrockConstantCache, repeat_step = false)
    ...
    if mass_matrix === I
        linsolve_tmp = du + dtd[2] * dT + dtC[2][1] * k1
    else
        linsolve_tmp = du + dtd[2] * dT + mass_matrix * (dtC[2][1] * k1)
    end
    ...
    if mass_matrix === I
        linsolve_tmp = du + dtd[3] * dT + (dtC[3][1] * k1 + dtC[3][2] * k2)
    else
        linsolve_tmp = du + dtd[3] * dT + mass_matrix * (dtC[3][1] * k1 + dtC[3][2] * k2)
    end
    ...
    if mass_matrix === I
        linsolve_tmp = du + dtd[4] * dT + (dtC[4][1] * k1 + dtC[4][2] * k2 + dtC[4][3] * k3)
    else
        linsolve_tmp = du + dtd[4] * dT + mass_matrix * (dtC[4][1] * k1 + dtC[4][2] * k2 + dtC[4][3] * k3)
    end

    k4 = _reshape(W \ -_vec(linsolve_tmp), axes(uprev))
    integrator.stats.nsolve += 1
    u = uprev + a[5][1] * k1 + a[5][2] * k2 + a[5][3] * k3 + a[5][4] * k4
    du = f(u, p, t + dt)
    integrator.stats.nf += 1

    if mass_matrix === I
        linsolve_tmp = du + (dtC[5][2] * k2 + dtC[5][4] * k4 + dtC[5][1] * k1 + dtC[5][3] * k3)
    else
        linsolve_tmp = du +
                       mass_matrix * (dtC[5][2] * k2 + dtC[5][4] * k4 + dtC[5][1] * k1 + dtC[5][3] * k3)
    end
    ...
    if mass_matrix === I
        linsolve_tmp = du + (dtC[6][1] * k1 + dtC[6][2] * k2 + dtC[6][5] * k5 + dtC[6][4] * k4 + dtC[6][3] * k3)
    else
        linsolve_tmp = du +
                       mass_matrix *
                       (dtC[6][1] * k1 + dtC[6][2] * k2 + dtC[6][5] * k5 + dtC[6][4] * k4 + dtC[6][3] * k3)
    end
...
@muladd function perform_step!(integrator, cache::RosenbrockCache, repeat_step = false)
    ...
    if mass_matrix === I
        @.. broadcast=false linsolve_tmp=dus[1] + dtd[2] * dT + dtC[2][1] * ks[1]
    else
        @.. broadcast=false dus[2]=dtC[2][1] * ks[1]
        mul!(_vec(dus[3]), mass_matrix, _vec(dus[2]))
        @.. broadcast=false linsolve_tmp=dus[1] + dtd[2] * dT + dus[3]
    end

    linres = dolinsolve(integrator, linres.cache; b = _vec(linsolve_tmp))
    @.. broadcast=false $(_vec(ks[2]))=-linres.u
    integrator.stats.nsolve += 1
    ...
    if mass_matrix === I
        @.. broadcast=false linsolve_tmp=dus[1] + dtd[3] * dT + (dtC[3][1] * ks[1] + dtC[3][2] * ks[2])
    else
        @.. broadcast=false dus[2]=dtC[3][1] * ks[1] + dtC[3][2] * ks[2]
        mul!(_vec(dus[3]), mass_matrix, _vec(dus[2]))
        @.. broadcast=false linsolve_tmp=dus[1] + dtd[3] * dT + dus[3]
    end

    linres = dolinsolve(integrator, linres.cache; b = _vec(linsolve_tmp))
    @.. broadcast=false $(_vec(ks[3]))=-linres.u
    integrator.stats.nsolve += 1
    ...
    if mass_matrix === I
        @.. broadcast=false linsolve_tmp=dus[1] + dtd[4] * dT +
                                         (dtC[4][1] * ks[1] + dtC[4][2] * ks[2] + dtC[4][3] * ks[3])
    else
        @.. broadcast=false dus[2]=dtC[4][1] * ks[1] + dtC[4][2] * ks[2] + dtC[4][3] * ks[3]
        mul!(_vec(dus[3]), mass_matrix, _vec(dus[2]))
        @.. broadcast=false linsolve_tmp=dus[1] + dtd[4] * dT + dus[2]
    end

    linres = dolinsolve(integrator, linres.cache; b = _vec(linsolve_tmp))
    @.. broadcast=false $(_vec(ks[4]))=-linres.u
    integrator.stats.nsolve += 1
    ...
    if mass_matrix === I
        @.. broadcast=false linsolve_tmp=dus[1] +
                                         (dtC[5][2] * ks[2] + dtC[5][4] * ks[4] + dtC[5][1] * ks[1] + dtC[5][3] * ks[3])
    else
        @.. broadcast=false dus[2]=dtC[5][2] * ks[2] + dtC[5][4] * ks[4] + dtC[5][1] * ks[1] + dtC[5][3] * ks[3]
        mul!(_vec(dus[3]), mass_matrix, _vec(dus[2]))
        @.. broadcast=false linsolve_tmp=dus[1] + dus[3]
    end

    linres = dolinsolve(integrator, linres.cache; b = _vec(linsolve_tmp))
    @.. broadcast=false $(_vec(ks[5]))=-linres.u
    integrator.stats.nsolve += 1
    ...
    if mass_matrix === I
        @.. broadcast=false linsolve_tmp=dus[1] + (dtC[6][1] * ks[1] + dtC[6][2] * ks[2] + dtC[6][5] * ks[5] +
                                          dtC[6][4] * ks[4] + dtC[6][3] * ks[3])
    else
        @.. broadcast=false dus[2]=dtC[6][1] * ks[1] + dtC[6][2] * ks[2] + dtC[6][5] * ks[5] + dtC[6][4] * ks[4] +
                                dtC[6][3] * ks[3]
        mul!(_vec(dus[2]), mass_matrix, _vec(dus[1]))
        @.. broadcast=false linsolve_tmp=dus[1] + dus[2]
    end
...
@muladd function perform_step!(integrator, cache::Rosenbrock5ConstantCache,
    ...
    if mass_matrix === I
        linsolve_tmp = du + dtd[5] * dT + (dtC[5][2] * k2 + dtC[5][4] * k4 + dtC[5][1] * k1 + dtC[5][3] * k3)
    else
        linsolve_tmp = du + dtd[5] * dT +
                       mass_matrix * (dtC[5][2] * k2 + dtC[5][4] * k4 + dtC[5][1] * k1 + dtC[5][3] * k3)
    end

    k5 = _reshape(W \ -_vec(linsolve_tmp), axes(uprev))
    integrator.stats.nsolve += 1
    u = uprev + a[6][1] * k1 + a[6][2] * k2 + a[6][3] * k3 + a[6][4] * k4 + a[6][5] * k5
    du = f(u, p, t + dt)
    integrator.stats.nf += 1

    if mass_matrix === I
        linsolve_tmp = du + (dtC[6][1] * k1 + dtC[6][2] * k2 + dtC[6][3] * k3 + dtC[6][4] * k4 + dtC[6][5] * k5)
    else
        linsolve_tmp = du +
                       mass_matrix *
                       (dtC[6][1] * k1 + dtC[6][2] * k2 + dtC[6][3] * k3 + dtC[6][4] * k4 + dtC[6][5] * k5)
    end
    ...
    if mass_matrix === I
        linsolve_tmp = du +
                       (dtC[7][1] * k1 + dtC[7][2] * k2 + dtC[7][3] * k3 + dtC[7][4] * k4 + dtC[7][5] * k5 +
                        dtC[7][6] * k6)
    else
        linsolve_tmp = du +
                       mass_matrix *
    ...
    if mass_matrix === I
        linsolve_tmp = du +
                       (dtC[8][1] * k1 + dtC[8][2] * k2 + dtC[8][3] * k3 + dtC[8][4] * k4 + dtC[8][5] * k5 +
                        dtC[8][6] * k6 + dtC[8][7] * k7)
    else
        linsolve_tmp = du +
                       mass_matrix *
                       (dtC[8][1] * k1 + dtC[8][2] * k2 + dtC[8][3] * k3 + dtC[8][4] * k4 + dtC[8][5] * k5 +
...
function initialize!(integrator, cache::RosenbrockCache)
    ...
    integrator.k[1] = dense[1]
    integrator.k[2] = dense[2]
    integrator.k[3] = dense[3]
end

@muladd function perform_step!(integrator, cache::RosenbrockCache, repeat_step = false)
    @unpack t, dt, uprev, u, f, p = integrator
    @unpack dus, ks, dT, J, W, uf, tf, linsolve_tmp, jac_config, atmp, weight, stage_limiter!, step_limiter! = cache
    @unpack a, C, gamma, d, c = cache.tab
    ...
    if mass_matrix === I
        ...
        @.. broadcast=false dus[2]=dtC[3][1] * ks[1] + dtC[3][2] * ks[2]
        mul!(_vec(du2), mass_matrix, _vec(dus[2]))
        @.. broadcast=false linsolve_tmp=dus[1] + dtd[3] * dT + dus[3]
    end
    ...
    if mass_matrix === I
        ...
        @.. broadcast=false dus[2]=dtC[4][1] * ks[1] + dtC[4][2] * ks[2] + dtC[4][3] * ks[3]
        mul!(_vec(du2), mass_matrix, _vec(dus[2]))
        @.. broadcast=false linsolve_tmp=dus[1] + dtd[4] * dT + dus[3]
    end
    ...
    if mass_matrix === I
        @.. broadcast=false linsolve_tmp=dus[1] + dtd[5] * dT +
                                         (dtC[5][2] * ks[2] + dtC[5][4] * ks[4] + dtC[5][1] * ks[1] + dtC[5][3] * ks[3])
    else
        @.. broadcast=false du1=dtC[5][2] * ks[2] + dtC[5][4] * ks[4] + dtC[5][1] * ks[1] + dtC[5][3] * ks[3]
        mul!(_vec(dus[3]), mass_matrix, _vec(dus[2]))
        @.. broadcast=false linsolve_tmp=dus[1] + dtd[5] * dT + dus[3]
    end
    ...
    @.. broadcast=false u=uprev + a[6][1] * ks[1] + a[6][2] * ks[2] + a[6][3] * ks[3] + a[6][4] * ks[4] + a[6][5] * ks[5]
    stage_limiter!(u, integrator, p, t + dt)
    f(du, u, p, t + dt)
    integrator.stats.nf += 1

    if mass_matrix === I
        @.. broadcast=false linsolve_tmp=du + (dtC[6][1] * ks[1] + dtC[6][2] * ks[2] + dtC[6][3] * ks[3] +
                                          dtC[6][4] * ks[4] + dtC[6][5] * ks[5])
    else
        @.. broadcast=false du1=dtC[6][1] * ks[1] + dtC[6][2] * ks[2] + dtC[6][3] * ks[3] + dtC[6][4] * ks[4] +
                                dtC[6][5] * ks[5]
        mul!(_vec(dus[3]), mass_matrix, _vec(dus[2]))
        @.. broadcast=false linsolve_tmp=du + dus[3]
    end
    ...
    if mass_matrix === I
        @.. broadcast=false linsolve_tmp=dus[1] + (dtC[7][1] * ks[1] + dtC[7][2] * ks[2] + dtC[7][3] * ks[3] +
                                          dtC[7][4] * ks[4] + dtC[7][5] * ks[5] + dtC[7][6] * ks[6])
    else
        @.. broadcast=false du1=dtC[7][1] * ks[1] + dtC[7][2] * ks[2] + dtC[7][3] * ks[3] + dtC[7][4] * ks[4] +
                                dtC[7][5] * ks[5] + dtC[7][6] * ks[6]
        mul!(_vec(dus[3]), mass_matrix, _vec(dus[2]))
        @.. broadcast=false linsolve_tmp=du + dus[3]
    end
    ...
    if mass_matrix === I
        @.. broadcast=false linsolve_tmp=dus[1] + (dtC[8][1] * ks[1] + dtC[8][2] * ks[2] + dtC[8][3] * ks[3] +
                                          dtC[8][4] * ks[4] + dtC[8][5] * ks[5] + dtC[8][6] * ks[6] + dtC[8][7] * ks[7])
    else
        @.. broadcast=false du1=dtC[8][1] * ks[1] + dtC[8][2] * ks[2] + dtC[8][3] * ks[3] + dtC[8][4] * ks[4] +
                                dtC[8][5] * ks[5] + dtC[8][6] * ks[6] + dtC[8][7] * ks[7]
        mul!(_vec(dus[3]), mass_matrix, _vec(dus[2]))
        @.. broadcast=false linsolve_tmp=dus[1] + dus[3]
    end
    ...
    if integrator.opts.calck
        @unpack h21, h22, h23, h24, h25, h26, h27, h28, h31, h32, h33, h34, h35, h36, h37, h38, h41, h42, h43, h44, h45, h46, h47, h48 = cache.tab
        @.. broadcast=false integrator.k[1]=h21 * ks[1] + h22 * ks[2] + h23 * ks[3] + h24 * ks[4] +
                                            h25 * ks[5] + h26 * ks[6] + h27 * ks[7] + h28 * ks[8]
        @.. broadcast=false integrator.k[2]=h31 * ks[1] + h32 * ks[2] + h33 * ks[3] + h34 * ks[4] +
                                            h35 * ks[5] + h36 * ks[6] + h37 * ks[7] + h38 * ks[8]
        @.. broadcast=false integrator.k[3]=h41 * ks[1] + h42 * ks[2] + h43 * ks[3] + h44 * ks[4] +
                                            h45 * ks[5] + h46 * ks[6] + h47 * ks[7] + h48 * ks[8]
        if (integrator.alg isa Rodas5Pr) && integrator.opts.adaptive &&
           (integrator.EEst < 1.0)
            k2 = 0.5 * (uprev + u +
                  0.5 * (integrator.k[1] + 0.5 * (integrator.k[2] + 0.5 * integrator.k[3])))
            ...
            EEst = norm(dus[3]) / norm(integrator.opts.abstol .+ integrator.opts.reltol .* k2)
            integrator.EEst = max(EEst, integrator.EEst)
        end
    end
    cache.linsolve = linres.cache
