...
@muladd function perform_step!(integrator, cache::Union{Rosenbrock23Cache, Rosenbrock32Cache, Rosenbrock33Cache, 
    Rosenbrock34Cache, Rosenbrock23ConstantCache, Rosenbrock32ConstantCache, 
    Rosenbrock33ConstantCache, Rosenbrock34ConstantCache, Rodas23WConstantCache, Rodas3PConstantCache,
    Rodas4ConstantCache}, repeat_step = false)
    ...
    if cache isa Rodas23WConstantCache || Rodas3PConstantCache || Rodas4ConstantCache
        @unpack t, dt, uprev, u, f, p = integrator
        @unpack tf, uf = cache
        @unpack a21, a31, a32, a41, a42, a43, a51, a53, a54, a52, C21, C31, C32, C41, C42, C43, C51, C52, C53, C54, C61, C62, C63, C64, C65, gamma, c2, c3, c4, d1, d2, d3, d4 = cache.tab
        ...
        du = f(uprev, p, t)
        integrator.stats.nf += 1
        if cache isa Rodas23WConstantCache || Rodas3PConstantCache
        ...
        if cache isa Rodas4ConstantCache
        ...
        integrator.stats.nsolve += 1
        if cache isa Rosenbrock4ConstantCache
            u = u + k5
            du = f(u, p, t + dt)
            integrator.stats.nf += 1
        end
        ...
        if cache isa Rodas23WConstantCache || cache isa Rodas3PConstantCache
            EEst = 0.0
    ...
    if cache isa Rosenbrock33Cache || cache isa Rosenbrock34Cache
        @unpack t, dt, uprev, u, f, p = integrator
    ...
    if cache isa Rosenbrock34ConstantCache
        @unpack t, dt, uprev, u, f, p = integrator
        @unpack tf, uf = cache
        @unpack a21, a31, a32, a41, a42, a43, C21, C31, C32, C41, C42, C43, b1, b2, b3, b4, btilde1, btilde2, btilde3, btilde4, gamma, c2, c3, d1, d2, d3, d4 = cache.tab
    ...
    if cache isa Rosenbrock33Cache || cache isa Rosenbrock34Cache || cache isa Rosenbrock23ConstantCache || cache isa Rosenbrock32ConstantCache || cache isa Rosenbrock33ConstantCache || cache isa Rosenbrock34ConstantCache
...
function perform_step!(integrator, cache::Union{Rodas23WCache, Rodas3PCache}, repeat_step = false)
    @unpack t, dt, uprev, u, f, p = integrator
    @unpack du, du1, du2, dT, J, W, uf, tf, k1, k2, k3, k4, k5, linsolve_tmp, jac_config, atmp, weight, stage_limiter!, step_limiter! = cache
    @unpack a, C, h, h2, gamma, c2, d = cache.tab
    ...
    A = repeat_step ? nothing : W
    linres = dolinsolve(integrator, cache.linsolve; A = A, b = _vec(linsolve_tmp),
    ...
    for i in 1:4
        @.. broadcast=false u=uprev + sum(a[i, j] * k[j] for j in 1:i)
        stage_limiter!(u, integrator, p, t + c2 * dt)
        f(du, u, p, t + dt * c2)
        integrator.stats.nf += 1

        if mass_matrix === I
            @.. broadcast=false linsolve_tmp=du + sum(dtC[i, j] * k[j] for j in 1:i) + dtd[i] * dT
        else
            @.. broadcast=false du1=sum(dtC[i, j] * k[j] for j in 1:i)
            mul!(_vec(du2), mass_matrix, _vec(du1))
            @.. broadcast=false linsolve_tmp=du + du2 + dtd[i] * dT
        end

        linres = dolinsolve(integrator, linres.cache; b = _vec(linsolve_tmp))
        @.. broadcast=false $(_vec(k[i + 1]))=-linres.u
        integrator.stats.nsolve += 1
    end
    ...
    if integrator.opts.calck
        for i in 1:3
            @.. broadcast=false integrator.k[i]=sum(h[i, j] * k[j] for j in 1:5)
            @.. broadcast=false integrator.k[3]=sum(h2[i, j] * k[j] for j in 1:5)
        end
        if integrator.opts.adaptive
            ...
            calculate_residuals!(atmp, du2, uprev, du1, integrator.opts.abstol,
                integrator.opts.reltol, integrator.opts.internalnorm, t)
            EEst = max(EEst, integrator.opts.internalnorm(atmp, t))
        end
    end
...
@muladd function perform_step!(integrator, cache::Union{Rodas4Cache, Rosenbrock5ConstantCache, Rosenbrock4Cache, Rosenbrock5Cache}, repeat_step = false)
    @unpack t, dt, uprev, u, f, p = integrator
    if cache isa Rosenbrock5ConstantCache
        @unpack tf, uf = cache
    end
    @unpack du, du1, du2, dT, J, W, uf, tf, k1, k2, k3, k4, k5, k6, k7, k8 linsolve_tmp, jac_config, atmp, weight, stage_limiter!, step_limiter! = cache
    if cache isa Rosenbrock5ConstantCache
    ...
    @unpack a21, a31, a32, a41, a42, a43, a51, a52, a53, a54, C21, C31, C32, C41, C42, C43, C51, C52, C53, C54, C61, C62, C63, C64, C65, C71, C72, C73, C74, C75, C76, C81, C82, C83, C84, C85, C86, C87, gamma, c2, c3, c4, c5, d1, d2, d3, d4 = cache.tab

    # Assignments
    sizeu = size(u)
    uidx = eachindex(integrator.uprev)

    if cache isa Rosenbrock5Cache || cache isa Rosenbrock4Cache
        # Precalculations
        dtC21 = C21 / dt
        dtC31 = C31 / dt
        dtC32 = C32 / dt
    ...
    if cache isa Rosenbrock4Cache || cache isa Rosenbrock5Cache
        @.. broadcast=false u=uprev + a51 * k1 + a52 * k2 + a53 * k3 + a54 * k4
