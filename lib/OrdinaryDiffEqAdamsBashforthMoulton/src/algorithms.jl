...
- `thread`: determines whether internal broadcasting on appropriate CPU arrays should be serial (`thread = OrdinaryDiffEq.Serial()`) or use multiple threads (`thread = Threaded()`) when Julia is started with multiple threads.
"""

keyword_default = """
thread = OrdinaryDiffEq.Serial(),
"""
