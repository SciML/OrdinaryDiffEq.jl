...
@cache mutable struct MREEFCache{uType,rateType,uNoUnitsType} <: OrdinaryDiffEqMutableCache
    ...
    k_slow::rateType
    k_fast::rateType
    T::Array{uType,1}   # extrapolation tableau column, T[j] = base result with ns[j] macro steps
    fsalfirst::rateType
    k::rateType
end
...
function alg_cache(
    ...
    atmp = similar(u, uEltypeNoUnits)
    recursivefill!(atmp, false)
    T = Array{typeof(u),1}(undef, alg.order)
    for j = 1:alg.order
        T[j] = zero(u)
    end
    return MREEFCache(
...
struct MREEFConstantCache <: OrdinaryDiffEqConstantCache end
