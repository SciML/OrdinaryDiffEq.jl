...
@testset "QPRK Allocation Tests" begin
    ...
    @testset "QPRK98 perform_step! Static Analysis" begin
        integrator = init(
            prob, QPRK98(), dt = 0.1, save_everystep = false,
        ...
        if length(allocs) > 0
            println(
                "AllocCheck found $(length(allocs)) allocation sites in QPRK98 perform_step!"
            )
        else
            println("QPRK98 perform_step! appears allocation-free with AllocCheck")
        end
    end
end
