...
#
# Skip widening when the user function lives behind a
# `FunctionWrappersWrapper` (`AutoSpecialize`). DiffEqBase precompiles the
# `(nested_u, outer_p, t)` FW signature directly; passing a widened `p` would
# not match any compiled slot and trip `AllowNonIsBits` into
