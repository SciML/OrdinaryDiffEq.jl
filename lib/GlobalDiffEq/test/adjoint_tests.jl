...
@testset "Adjoint error estimation and control" begin
    ...
    # the controlled solution surfaces the endpoint estimate through the SciMLBase
    # global-error interface
    @test SciMLBase.has_global_error(GlobalAdjoint(Tsit5()))
    @test length(controlled_sol.global_error) == length(controlled_sol.t)
    @test controlled_sol.global_error[end] > 0
    @test controlled_sol.global_error[end] <= gtol
    ...
    # callbacks are no longer rejected: the adjoint system supports them, so a
    # benign (non-state-changing) callback estimates without error
    callback = DiscreteCallback((u, t, integrator) -> false, integrator -> nothing)
    callback_prob = ODEProblem(linear!, [1.0], tspan, rate; callback)
    callback_estimate = adjoint_error_estimate(
        callback_prob, GlobalAdjoint(Tsit5(); samples = 1, rng = Xoshiro(789));
        abstol = 1.0e-4, reltol = 1.0e-4
    )
