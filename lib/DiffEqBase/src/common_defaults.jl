...
Replace `nothing` absolute/relative tolerances with the concrete defaults ODE/DAE
solvers would compute from `u` (`1 // 10^6` / `1 // 10^3` in the state eltype,
or `false` for discrete problems). User-supplied values are passed through
`real.(...)`. Used so `solve(prob, alg)` and `solve(prob, alg; abstol, reltol)`
enter the specialized init body with matching keyword types.
"""
function resolve_ode_tolerances(prob, u, abstol, reltol)
    uBottomEltype = RecursiveArrayTools.recursive_bottom_eltype(u)
    uBottomEltypeNoUnits = RecursiveArrayTools.recursive_unitless_bottom_eltype(u)
    scalar_type_tol =
        uBottomEltypeNoUnits == uBottomEltype &&
        uBottomEltype <: Union{Real, Complex}

    if prob isa AbstractDiscreteProblem && abstol === nothing
        abstol_out = false
    ...
    if prob isa AbstractDiscreteProblem && reltol === nothing
        reltol_out = false
...
function with_resolved_ode_tolerances(prob, u0, kwargs)
    if !(prob isa Union{AbstractODEProblem, AbstractDAEProblem, AbstractDiscreteProblem})
        return kwargs
    end
    u = u0 === nothing ? Float64[] : u0
    abstol = get(kwargs, :abstol, nothing)
    reltol = get(kwargs, :reltol, nothing)
    if abstol === nothing && has_kwargs(prob)
        abstol = get(prob.kwargs, :abstol, nothing)
    end
    if reltol === nothing && has_kwargs(prob)
        reltol = get(prob.kwargs, :reltol, nothing)
    end
    abstol, reltol = resolve_ode_tolerances(prob, u, abstol, reltol)
    kwargs = Base.structdiff((; kwargs...), NamedTuple{(:abstol, :reltol)})
    return (; abstol, reltol, kwargs...)
end
