...
function promote_f(
    ...
    # Opaque-p path: when p is an isbits non-NullParameters payload, route the
    # wrapped Void through OpaqueVoid so the wrapper's signature has
    # `OpaqueParams` in the p slot. The packed p is returned alongside f and
    # is plumbed into prob.p by `get_concrete_problem`.
    opaque = should_opaque_p(p)
    P = typeof(p)
    ...
    if f.tgrad !== nothing && !(f.tgrad isa FunctionWrappersWrappers.FunctionWrappersWrapper)
        if opaque
            ...
            f = @set f.tgrad = wrap_void_opaque(f.tgrad, P, (tgrad_sig,))
        else
    ...
    if f.jac !== nothing && !(f.jac isa FunctionWrappersWrappers.FunctionWrappersWrapper)
        if f.jac_prototype !== nothing && !(f.jac_prototype isa AbstractSciMLOperator)
            ...
            f = if opaque
                @set f.jac = wrap_void_opaque(f.jac, P, (sig,))
            else
        ...
        elseif isdefined(f, :sparsity) && f.sparsity isa AbstractMatrix &&
            ...
            f = if opaque
                @set f.jac = wrap_void_opaque(f.jac, P, (dense_sig, sparse_sig))
            else
                @set f.jac = FunctionWrappersWrappers.FunctionWrappersWrapper(
                    Void(f.jac),
    ...
    p_out = opaque ? RespecializeParams.pack(p) : p
...
function promote_f(
    ...
    if should_opaque_p(p)
        P = typeof(p)
        wrapped = wrapfun_iip_opaque(f.f, P, (u0, u0, p, t))
        return (unwrapped_f(f, wrapped), RespecializeParams.pack(p))
    end
