...
struct DiagonalFourierOperator{T,uType<:AbstractVector{T},fftType,ifftType} <: AbstractSciMLOperator{T}
    diag::uType
    plan_fft!::fftType
    plan_ifft!::ifftType
    tmp::uType
...
function DiagonalFourierOperator(diag, plan_fft!_prototype, plan_ifft!_prototype)
    tmp = similar(diag)
    return DiagonalFourierOperator(diag, plan_fft!_prototype(tmp), plan_ifft!_prototype(tmp), tmp)
end

DiagonalFourierOperator(diag::Vector{T}) where {T<:Complex} = DiagonalFourierOperator(diag, FFTW.plan_fft!, FFTW.plan_ifft!)
DiagonalFourierOperator(diag::CuArray{T,1}) where {T<:Complex} = DiagonalFourierOperator(diag, CUDA.CUFFT.plan_fft!, CUDA.CUFFT.plan_ifft!)
...
(D::DiagonalFourierOperator)(du, u, _, _, _) = apply_kernel!(du, u, D.tmp, D.diag, D.plan_fft!, D.plan_ifft!)
...
create_nlse(nb_pts::Int, dτ::T, integration_limit::T, u0, use_gpu) where {T<:Real} = create_semilinear_problem(nb_pts, dτ, integration_limit, u0, ω -> -0.5im * ω^2,
    (du, u, t) -> nlse_non_linpart(du, u), use_gpu)

create_lle(nb_pts::Int, dτ::T, integration_limit::T, u0, use_gpu, Δ::T, S::T) where {T<:Real} = create_semilinear_problem(nb_pts, dτ, integration_limit, u0, ω -> -1 - 1im * ω^2,
    (du, u, t) -> lle_non_linpart!(du, u, Δ, S), use_gpu)

create_lle_scan(nb_pts::Int, dτ::T, scan_time::T, u0, use_gpu, Δ_min::T, Δ_max::T, S::T) where {T<:Real} = create_semilinear_problem(nb_pts, dτ, scan_time, u0, ω -> -1 - 1im * ω^2,
    (du, u, t) -> lle_non_linpart!(du, u, Δ_min + (Δ_max - Δ_min) * t / scan_time, S), use_gpu)

"""
Test function of the NLSE on a conservative soliton
"""
function nlse_test(::Type{T}, use_gpu; B=5.0, nb_pts=512, dτ=5e-3, propag_dist=10.0, save_everystep=false, reltol=1e-6) where {T<:Real}
    B = T(B)
...
nlse_test(type::Type{T}; kwargs...) where {T<:Real} = nlse_test(type, Val(false); kwargs...)
...
"""
function lle_scan_test(::Type{T}, use_gpu; nb_pts=1024, dτ=5e-2, save_everystep=false, reltol=1e-6, S=3, Δ_min=-2.0, Δ_max=12.0, t_max=10) where {T<:Real}
...
lle_scan_test(type::Type{T}; kwargs...) where {T<:Real} = lle_scan_test(type, Val(false); kwargs...)
