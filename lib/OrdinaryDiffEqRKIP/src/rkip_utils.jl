...
"""
@inline expmv_rkip_mip(cache::RKIPCache{expOpType,cacheType,tType,opType,uType,iip}, v::uType, h::tType) where {expOpType,cacheType,tType,opType,uType,iip} = expmv_rkip_mip(cache, v, h, lastindex(cache.c_mapping)) # If i is not precised, this is the final step in the RKIP -> h = dt

@inline function expmv_rkip_mip(cache::RKIPCache{expOpType,cacheType,tType,opType,uType,iip}, v::uType, h::tType, stage_index::Int) where {expOpType,cacheType,tType,opType,uType,iip}
...
@inline function _expmv_rkip_mip(cache::ExpCacheNoLdiv{expOpType}, tmp::vType, v::vType, stage_index::Integer, positive, iip) where {expOpType<:AbstractSciMLOperator,vType}
    op = get_op_for_this_step(cache, positive, stage_index)
    v = matvec_prod_mip(tmp, op, v, iip)
    return v
end

@inline function _expmv_rkip_mip(cache::ExpCache{expOpType}, tmp::vType, v::vType, stage_index::Integer, positive, ::Val{true}) where {expOpType<:AbstractSciMLOperator,vType}
...
"""
@inline function matvec_prod_mip(tmp::V, mat::M, v::V, ::Val{true}) where {V,M<:AbstractSciMLOperator}
    mat(tmp, v, nothing, nothing, zero(eltype(v)))
    copyto!(v, tmp)
    return v
end

@inline function matvec_prod_mip(_::V, mat::M, v::V, ::Val{false}) where {V,M<:AbstractSciMLOperator}
