...
@testset "callback jump enters the history" begin
    # The history is the ODE integrator's solution, so a jump has to be recorded there
    # whatever `save_positions` asks of the returned solution: a later lookback into the
    # step that crosses the jump reads the history, not the output.
    ...
    # the jump itself is only in the returned solution when it was asked for
    @test solve(
        bolus_prob, alg; common..., callback = bolus((false, true))
    )(12.0 + 1.0e-8)[1] > 19.9

    # the returned solution still carries only what `save_positions` asked for
