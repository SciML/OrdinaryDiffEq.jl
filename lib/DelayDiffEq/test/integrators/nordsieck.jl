...
@testset "NordsieckBDF trial-step history" begin
    ...
    # trial interpolation data before the acceptance controller runs, so
    # publishing k only on acceptance feeds the preceding step's polynomial
    # into the RHS.
    h(p, t) = exp(-t)
    hvec(p, t; idxs = nothing) = idxs === nothing ? [exp(-t)] : exp(-t)
    scalar(u) = u isa Number ? u : u[1]
    # lag > dt keeps the delayed argument inside the saved history;
    # lag < dt lands it in the uncommitted step and drives the fixed-point
    # iteration, which re-invokes perform_step! on the predicted state.
    for lag in (0.2, 0.02), iip in (false, true), with_callback in (false, true)
        f(u, h, p, t) = -exp(-lag) * h(p, t - lag)
        function f!(du, u, h, p, t)
            du[1] = -exp(-lag) * h(p, t - lag; idxs = 1)
            return nothing
        end
        @testset "lag=$lag inplace=$iip callback=$with_callback" begin
            prob = iip ?
