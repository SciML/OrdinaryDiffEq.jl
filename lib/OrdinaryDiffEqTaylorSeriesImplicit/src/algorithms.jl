...
@doc SDIRK_docstring(
    "A 1st order implicit solver. A-B-L-stable.
...
struct ImplicitTaylor{P, T, CS, AD, F, F2, Prec, FDT, ST, CJ, Tol, StepLimiter} <:
...
function get_order_param(order::Val{P}) where {P}
    return P
end

function ImplicitTaylor(;
        order = Val(1), μ = 1.0, chunk_size = Val{0}(), autodiff = AutoForwardDiff(),
    ...
    return ImplicitTaylor{
        get_order_param(order), typeof(μ), _unwrap_val(chunk_size), typeof(AD_choice), typeof(linsolve),
        typeof(nlsolve), typeof(precs), diff_type, _unwrap_val(standardtag),
        _unwrap_val(concrete_jac), typeof(κ), typeof(step_limiter!),
    }(
        order, μ, linsolve,
        nlsolve, precs, extrapolant, controller, step_limiter!, AD_choice, κ, maxiters
    )
end
