...
function RadauIIATableau{T1, T2}(tab::RadauIIATableau{BigFloat, BigFloat}) where {T1, T2}
    RadauIIATableau{T1, T2}(tab.T, tab.TI, tab.c, tab.γ,tab.α, tab.β, tab.e)
end
function RadauIIATableau(T1, T2, num_stages::Int)
    tab = get(RadauIIATableauCache, num_stages) do
        tab = generateRadauTableau(BigFloat, BigFloat, num_stages)
        RadauIIATableauCache[num_stages] = tab
        tab
    end
...
function generateRadauTableau(T1, T2, num_stages::Int)
    ...
    α = [real(vals[i]) for i in 1:2:num_stages-1]
    β = [imag(vals[i]) for i in 1:2:num_stages-1]
