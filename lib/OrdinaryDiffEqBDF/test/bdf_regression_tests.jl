...
@testset "MOOSE234 ROBER stiff system" begin
    ...
    @test sol.t[end] == 1e5
    @test isapprox(sum(sol[end]), 1.0, atol = 0.05)
end

@testset "FBDF stab_filter basic solve (out-of-place)" begin
    prob = ODEProblem((u, p, t) -> -u, 1.0, (0.0, 1.0))
    exact = exp(-1.0)

    sol_def = solve(prob, FBDF(), abstol = 1e-8, reltol = 1e-8)
    sol_stab = solve(prob, FBDF(stab_filter = true), abstol = 1e-8, reltol = 1e-8)

    @test sol_def.retcode == ReturnCode.Success
    @test sol_stab.retcode == ReturnCode.Success
    @test isapprox(sol_stab[end], exact, rtol = 1e-6)
end

@testset "FBDF stab_filter basic solve (in-place)" begin
    fiip = (du, u, p, t) -> du .= -u
    prob = ODEProblem(fiip, [1.0, 2.0], (0.0, 1.0))
    exact = exp(-1.0) .* [1.0, 2.0]
    ...
    @test sol_def.retcode == ReturnCode.Success
    @test sol_stab.retcode == ReturnCode.Success
    @test isapprox(sol_stab.u[end], exact, rtol = 1e-4)
end

@testset "FBDF stab_filter Van der Pol (stiff)" begin
    function vdp!(du, u, p, t)
        μ = p[1]
        du[1] = u[2]
        du[2] = μ * (1 - u[1]^2) * u[2] - u[1]
    ...
    prob = ODEProblem(vdp!, [2.0, 0.0], (0.0, 6.3), [1000.0])
    sol_def = solve(prob, FBDF(), abstol = 1e-6, reltol = 1e-6)
    sol_stab = solve(prob, FBDF(stab_filter = true), abstol = 1e-6, reltol = 1e-6)

    @test sol_def.retcode == ReturnCode.Success
    @test sol_stab.retcode == ReturnCode.Success
    @test sol_stab.t[end] == 6.3
    @test isapprox(sol_stab.u[end], sol_def.u[end], rtol = 0.1)
end

@testset "FBDF stab_filter ROBER (stiff)" begin
    function rober!(du, u, p, t)
        y1, y2, y3 = u
        du[1] = -0.04 * y1 + 1e4 * y2 * y3
        du[2] = 0.04 * y1 - 1e4 * y2 * y3 - 3e7 * y2^2
    ...
    sol_stab = solve(prob, FBDF(stab_filter = true), abstol = 1e-8, reltol = 1e-8)
    @test sol_stab.retcode == ReturnCode.Success
    @test sol_stab.t[end] == 1e5
    @test isapprox(sum(sol_stab[end]), 1.0, atol = 0.05)
end
