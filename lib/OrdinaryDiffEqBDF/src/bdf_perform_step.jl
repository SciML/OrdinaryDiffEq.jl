...
function perform_step!(integrator, cache::SBDFCache, repeat_step = false)
    ...
    elseif cnt == 2
        @.. broadcast=false tmp=γ * (2 * uprev - Int64(1) // 2 * uprev2 + dt * (2 * du₂ - k₁))
    elseif cnt == 3
        @.. broadcast=false tmp=γ * (3 * uprev - Int64(3) // 2 * uprev2 + Int64(1) // 3 * uprev3 +
                                 dt * (3 * (du₂ - k₁) + k₂))
    else
        @.. broadcast=false tmp=γ * (4 * uprev - 3 * uprev2 + Int64(4) // 3 * uprev3 -
                                 Int64(1) // 4 * uprev4 + dt * (4 * du₂ - 6 * k₁ + 4 * k₂ - k₃))
    end
    # Implicit part
    # precalculations
    γdt = γ * dt
