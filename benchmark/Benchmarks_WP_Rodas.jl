using DifferentialEquations,
    Plots, LaTeXStrings, LinearAlgebra, DiffEqDevTools, SparseArrays
gr()
...
function ode_1(du, u, p, t)
    x, b1, J = p
    du[:] =
        J * u .+ b1 / (1 + t) .- (x .+ 1 / 2) .* (3 / 2 .- x) ./ (t + 1) .^ 2 .- 2 / (1 + t)
    nothing
end
...
function jac_3(J, y, p, t)
    if p == 1
        J[:] = [
            ...
            0 0 0 1 0
            0 0 y[5] 0 y[3]
            2*y[5]*y[1] 2*y[2] 2*y[5]*y[3]-9.81 2*y[4] y[1]^2+y[3]^2
        ] #-- Index-1
    else
        J[:] =
            [0 1 0 0 0; y[5] 0 0 0 y[1]; 0 0 0 1 0; 0 0 y[5] 0 y[3]; y[2] y[1] y[4] y[3] 0] #-- Index-2
    end
    nothing
end
...
function ode_7(du, u, p, t)
    ...
    for n = 1:nedges
        ...
        r = abs(phi[n] * d / (nu * a))
        rghres[n] =
            1.0 / rtla - 1.74 + 2.0 * log10(2.0 * k / d + 18.7 / (max(r, rcrit) * rtla))
