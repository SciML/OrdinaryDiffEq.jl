...
callback_single =
    VectorContinuousCallback(condition_single, affect!, affect_neg!, 1, interp_points = 100)
...
@test maximum(
    maximum.(map((i) -> sol.k[9][i] - sol_bounced.k[2][i], 1:length(sol.k[9]))),
) == 0
...
vcustom_retcode_callback =
    VectorContinuousCallback(vcondition, (x, idx) -> vaffect!(x, idx, :Custom), 1)
...
vterminate_callback2 =
    VectorContinuousCallback(vcondition, nothing, vaffect2!, 1, interp_points = 100)
    ...
    nothingf =
        affect_neg! = function (integrator)
            integrator.u = ArrayPartition(
                SVector{1}(integrator.u[1]),
                SVector{1}(-integrator.u[2]),
            )
        end
