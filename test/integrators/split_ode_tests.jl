...
@views function capillary_leveling(du, u, dr, nodes, boundaries)
    ...
    @. tmp[2:(end - 1)] -= 1.0e-3 / h[2:(end - 1)]^3
...
dr = 4 / N
r = @. dr / 2 + dr * (0:(N - 1))
h0 = map(x -> droplet(x, 1.0e-4), r)
r = [r[2]; r[1]; r; r[end]; r[end]]
boundaries = @. (r[1:(end - 1)] + r[2:end]) / 2

jac_proto = spdiagm(
    0 => ones(N), -1 => ones(N - 1), 1 => ones(N - 1), -2 => ones(N - 2),
    2 => ones(N - 2)
)

fun = ODEFunction(
    (dh, h, p, t) -> capillary_leveling(dh, h, dr, r, boundaries),
...
@test_broken solve(sprob, CFNLIRK3(), reltol = 1.0e-8, dt = 1.0e-5).retcode ==
    ReturnCode.Success

for Alg in (KenCarp3, KenCarp4, KenCarp5, KenCarp47, KenCarp58)
    print(Alg)
    sol = solve(prob, Alg(), reltol = 1.0e-8)
