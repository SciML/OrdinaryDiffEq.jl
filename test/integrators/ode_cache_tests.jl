...
NON_IMPLICIT_ALGS = filter(
    (x) -> isconcretetype(x) && !OrdinaryDiffEq.isimplicit(x()),
    union(
        subtypes(OrdinaryDiffEq.OrdinaryDiffEqAlgorithm),
...
sol = solve(
    prob,
    TRBDF2(linsolve = LinearSolve.IterativeSolversJL_GMRES()),
    callback = callback,
)
@test length(sol[end]) > 1
...
callback_adapt =
    DiscreteCallback(condition_adapt, affect!_adapt, save_positions = (false, false))
