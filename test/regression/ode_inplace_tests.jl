...
import ODEProblemLibrary:
    prob_ode_2Dlinear, prob_ode_large2Dlinear, prob_ode_linear, prob_ode_2Dlinear_notinplace
...
sol = solve(prob_ode_large2Dlinear, Euler(), dt = 1 // 2^(6), save_everystep = true)
alloc1 = @allocated sol =
    solve(prob_ode_large2Dlinear, Euler(), dt = 1 // 2^(8), save_everystep = true)
