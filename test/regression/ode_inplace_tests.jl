...
sol2 = solve(prob_ode_linear, Euler(), sol[:], sol.t, sol.k; dt = 1 // 2^(8),
    save_everystep = true)

sol = solve(prob_ode_large2Dlinear, Euler(), dt = 1 // 2^(6), save_everystep = true)
sol2 = solve(prob_ode_large2Dlinear, Euler(), sol[:], sol.t, sol.k; dt = 1 // 2^(8),
    save_everystep = true)

sol = solve(prob_ode_large2Dlinear, Euler(), dt = 1 // 2^(6), save_everystep = true)
alloc1 = @allocated sol = solve(prob_ode_large2Dlinear, Euler(), dt = 1 // 2^(8),
    save_everystep = true)
alloc2 = @allocated sol2 = solve(prob_ode_large2Dlinear, Euler(), sol[:], sol.t, sol.k;
    dt = 1 // 2^(8), save_everystep = true)
