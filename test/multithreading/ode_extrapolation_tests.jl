...
@testset "Testing extrapolation methods" begin
    ...
    @testset "Testing ImplicitEulerBarycentricExtrapolation" begin
        for prob in problem_array, seq in sequence_array
            ...
            for j = 1:4
                ...
                sim = test_convergence(dts, prob, alg)
                @test sim.𝒪est[:final] ≈ alg.n_init + 0.5 atol = newTol #Superconvergence
                algp = ImplicitEulerBarycentricExtrapolation(
                    min_order = j,
                    init_order = j,
                    max_order = j,
                    sequence = seq,
    ...
    @testset "Regression Test Float32 and Float64 Fallbacks" begin
        prob_ode_2Dlinear = ODEProblem(
            ODEFunction(f_2dlinear, analytic = f_2dlinear_analytic),
            Float64.(prob_ode_bigfloat2Dlinear.u0),
            (0.0, 1.0),
            1.01,
        )
        ...
        @test all(all(s1[i] - s2[i] .< 5e-14) for i = 1:length(s1))
        ...
        s1 = solve(prob_ode_bigfloat2Dlinear, ExtrapolationMidpointDeuflhard())
        s2 = solve(prob_ode_2Dlinear, ExtrapolationMidpointDeuflhard())
        @test all(all(s1[i] - s2[i] .< 5e-6) for i = 1:length(s1))
    end
