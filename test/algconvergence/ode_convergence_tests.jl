...
@testset "Explicit Solver Convergence Tests ($(["out-of-place", "in-place"][i]))" for i =
                                                                                      1:2
...
@testset "Implicit Solver Convergence Tests ($(["out-of-place", "in-place"][i]))" for i =
                                                                                      1:2
    prob = (ODEProblemLibrary.prob_ode_linear, ODEProblemLibrary.prob_ode_2Dlinear)[i]
    dts = 1 .// 2 .^ (9:-1:5)
    ...
    sim112 =
        test_convergence(dts, prob, ImplicitEuler(nlsolve = NLFunctional()), reltol = 1e-2)
    @test sim112.𝒪est[:final] ≈ 1 atol = testTol

    sim113 =
        test_convergence(dts, prob, ImplicitEuler(nlsolve = NLAnderson()), reltol = 1e-2)
