...
position_error =
    :final => [mean(sim[i].u[2].x[1] - sim[i].u_analytic[2].x[1]) for i = 1:length(sim)]
...
ff_harmonic_nip =
    DynamicalODEFunction(f1_harmonic_nip, f2_harmonic_nip; analytic = harmonic_analytic)
...
# Methods need BigFloat to test convergence rate
dts = big"1.0" ./ big"2.0" .^ (5:-1:1)
prob_big = DynamicalODEProblem(ff_harmonic_nip, big"1.0", big"0.0", (big"0.", big"70."))
sim = test_convergence(dts, prob_big, DPRKN6(), dense_errors = true)
@test sim.𝒪est[:l2] ≈ 6 rtol = 1e-1
@test sim.𝒪est[:L2] ≈ 6 rtol = 1e-1
sim = test_convergence(dts, prob_big, DPRKN8(), dense_errors = true)
