...
prob_ode_sin_inplace =
    ODEProblem(ODEFunction(f; analytic = (u0, p, t) -> [sin(t)]), [0.0], (0.0, 1.0))

f = (u, p, t) -> sin(u)
prob_ode_nonlinear = ODEProblem(
    ODEFunction(f; analytic = (u0, p, t) -> 2 * acot(exp(-t) * cot(0.5))),
    1.0,
    (0.0, 0.5),
)
...
@testset "ORK256" begin
    ...
    new_prob_ode_nonlinear_inplace =
        ODEProblem(prob_ode_nonlinear_inplace.f, [1.0], (0.0, 0.5))
    sol_old = solve(
        prob_ode_nonlinear_inplace,
        alg,
        dt = 1.e-4,
        save_everystep = false,
        save_start = false,
...
prob_nonlinear_A = ODEProblem(
    ODEFunction(f; analytic = (u0, p, t) -> 2 * acot(exp(-t) * cot(BigFloat(0.5)))),
    BigFloat(1.0),
