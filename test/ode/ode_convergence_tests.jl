...
for i = 1:2
  ...
  sim12 = test_convergence(dts,prob,GenericImplicitEuler(nlsolve=NLSOLVEJL_SETUP(autodiff=true)))
  @test abs(sim12.𝒪est[:final]-1) < testTol
  sim122 = test_convergence(dts,prob,GenericImplicitEuler(nlsolve=NLSOLVEJL_SETUP(autodiff=false)))
  @test abs(sim122.𝒪est[:final]-1) < testTol
  sim13 = test_convergence(dts,prob,GenericTrapezoid(nlsolve=NLSOLVEJL_SETUP(autodiff=true)))
