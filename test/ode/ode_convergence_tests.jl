...
for i = 1:2
  prob = probArr[i]
  println("Special RKs")
  sim = test_convergence(dts,prob,alg=:Euler)
  bool1 = abs(sim.𝒪est[:final]-1) < testTol
  sim2 = test_convergence(dts,prob,alg=:Midpoint)
  bool2 = abs(sim2.𝒪est[:l∞]-2) < testTol
  sim3 = test_convergence(dts,prob,alg=:RK4)
  bool3 = abs(sim3.𝒪est[:l∞]-4) < testTol
  sim4 = test_convergence(dts,prob,alg=:BS3)
  ...
  sim12 = test_convergence(dts,prob,alg=:ImplicitEuler,autodiff=true)
  sim122 = test_convergence(dts,prob,alg=:ImplicitEuler,autodiff=false)
  bool12 = (abs(sim12.𝒪est[:final]-1) < testTol) && (abs(sim122.𝒪est[:final]-1) < testTol)
  sim13 = test_convergence(dts,prob,alg=:Trapezoid,autodiff=true)
  sim132 = test_convergence(dts,prob,alg=:Trapezoid,autodiff=false)
