...
sol = solve(prob,ImplicitEuler(nlsolve=DiffEqDiffTools.NLSOLVEJL_SETUP(chunk_size=1)),callback=callback,dt=1/10)

sol = solve(prob,Trapezoid(nlsolve=DiffEqDiffTools.NLSOLVEJL_SETUP(chunk_size=1)),callback=callback,dt=1/10)
