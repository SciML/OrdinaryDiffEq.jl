...
sol3 =solve(prob::ODEProblem,Δt=1/2^(6),save_timeseries=true,alg=:RK4,abstol=1,reltol=0)

prob = ODEProblem(f,BigInt(1)//BigInt(2),analytic=analytic)

sol =solve(prob::ODEProblem,Δt=BigInt(1)//BigInt(2)^(6),save_timeseries=true,alg=:RK4,abstol=1,reltol=0)
sol2 =solve(prob::ODEProblem,Δt=BigInt(1)/BigInt(2)^(6),save_timeseries=true,alg=:RK4,abstol=1,reltol=0)
...
sol4 =solve(prob::ODEProblem,Δt=BigInt(1)//BigInt(2)^(3),save_timeseries=true,alg=:DP5,adaptive=false)

bool9 = typeof(sol4.u) == Rational{BigInt}

tab = constructDormandPrince8_64bit(Rational{BigInt})
sol5 =solve(prob::ODEProblem,Δt=BigInt(1)//BigInt(2)^(3),save_timeseries=true,alg=:ExplicitRK,abstol=1,reltol=0,tableau=tab,adaptive=false)
