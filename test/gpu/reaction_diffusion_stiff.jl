...
const γ₃ = 0.1
const N = 256
const X = reshape([i for i = 1:N for j = 1:N], N, N)
const Y = reshape([j for i = 1:N for j = 1:N], N, N)
const α₁ = 1.0 .* (X .>= 4 * N / 5)

const Mx = Tridiagonal([1.0 for i = 1:N-1], [-2.0 for i = 1:N], [1.0 for i = 1:N-1])
const My = copy(Mx)
Mx[2, 1] = 2.0
Mx[end-1, end] = 2.0
My[1, 2] = 2.0
...
println("ROCK2")
@time sol =
    solve(prob, ROCK2(), progress = true, save_everystep = false, save_start = false)
...
println("ROCK2")
@time sol =
    solve(prob2, ROCK2(), progress = true, save_everystep = false, save_start = false)
