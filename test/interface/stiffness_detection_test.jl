...
for (i, prob) in enumerate(probArr)
    println(i)
    sol =
        @test_nowarn solve(prob, AutoTsit5(Rosenbrock23(autodiff = false)), maxiters = 1000)
    ...
    sol = solve(
        prob,
        AutoVern6(Kvaerno3(); maxstiffstep = 4, maxnonstiffstep = 2),
        maxiters = 1000,
    )
    ...
    sol = solve(
        prob,
        AutoVern7(Hairer42(); maxstiffstep = 4, maxnonstiffstep = 2),
        maxiters = 1000,
    )
    ...
    sol = solve(
        prob,
        AutoVern8(Rosenbrock23(); maxstiffstep = 4, maxnonstiffstep = 4),
        maxiters = 1000,
    )
    ...
    sol = solve(
        prob,
        AutoVern9(KenCarp3(); maxstiffstep = 4, maxnonstiffstep = 1),
        maxiters = 1000,
    )
    ...
    sol = solve(
        prob,
        AutoVern9(KenCarp3(autodiff = false); maxstiffstep = 4, maxnonstiffstep = 1),
        maxiters = 1000,
    )
    @test length(sol.t) < 570
    @test is_switching_fb(sol)
