...
@testset "Tstops Tests on the Interval [0, 1]" begin
    ...
    # TODO
    integrator =
        init(prob, RK4(), tstops = [1 / 5, 1 / 4, 1 / 3, 1 / 2, 3 / 4], adaptive = false)
    ...
    sol = solve(
        prob,
        RK4(),
        tstops = [0, 1 / 5, 1 / 4, 1 / 3, 1 / 2, 3 / 4, 1],
        adaptive = false,
    )
    @test sol.t == [0, 1 / 5, 1 / 4, 1 / 3, 1 / 2, 3 / 4, 1]
...
@testset "Integrator Tstops Tests on the Interval $(["[-1, 0]", "[0, 1]"][i])" for (
    i,
    tdir,
) in enumerate(
    [-1.0; 1.0],
