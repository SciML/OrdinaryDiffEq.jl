...
sol2 =
    solve(prob, DP5(), dt = 1 // 2^(2), save_everystep = true, saveat = [0.0, 1 / 2, 1.0])
...
sol =
    solve(prob, DP5(), saveat = 0.2, save_on = false, save_start = false, save_end = false)
@test isempty(sol.t) && isempty(sol.u)
...
integ = init(
    ODEProblem((u, p, t) -> u, 0.0, (0.0, 1.0)),
    Tsit5(),
    saveat = _saveat,
    save_end = true,
)
