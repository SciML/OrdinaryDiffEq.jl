...
for prob in [prob_forward, prob_reverse]
    ...
    sol2 = solve(
        prob, DP5(), dt = 1 // 2^(2), save_everystep = false,
        saveat = [0.0, 1 / 2, 1.0]
    )
    ...
    sol3 = solve(
        prob, DP5(), dt = 1 // 2^(2), save_everystep = false, saveat = [1 / 2],
        tstops = [1 / 2]
    )
    ...
    @show tdir > 0
    @test tdir > 0 ? sol3.t == collect(0.0:0.1:1.0) : sol3.t == collect(1.0:-0.1:0.0)
end

sol = solve(prob_forward, RK4(), dt = 1 / 2^(2), save_everystep = true, adaptive = false)
sol2 = solve(
    prob_forward, RK4(), dt = 1 / 2^(2), save_everystep = true, adaptive = false,
    saveat = [0.125, 0.6, 0.61, 0.8]
)
@test sol.retcode == sol2.retcode == ReturnCode.Success
@test symdiff(sol.t, sol2.t) == [0.125, 0.6, 0.61, 0.8]

sol = solve(prob_forward, Rosenbrock32(), dt = 1 / 2^(2), save_everystep = true)
sol2 = solve(
    prob_forward, Rosenbrock32(), dt = 1 / 2^(2), save_everystep = true,
    saveat = [0.125, 0.6, 0.61, 0.8]
)
...
sol2 = solve(
    prob_forward, Trapezoid(), dt = 1 / 2^(2), save_everystep = true,
    saveat = [0.125, 0.6, 0.61, 0.8]
)
...
sol2 = solve(
    prob_reverse, RK4(), dt = 1 / 2^(2), save_everystep = true, adaptive = false,
    saveat = [0.8, 0.61, 0.6, 0.125]
)
...
sol2 = solve(
    prob_reverse, Rosenbrock32(), dt = 1 / 2^(2), save_everystep = true,
    saveat = [0.8, 0.61, 0.6, 0.125]
)
...
sol2 = solve(
    prob_reverse, Trapezoid(), dt = 1 / 2^(2), save_everystep = true,
    saveat = [0.8, 0.61, 0.6, 0.125]
)
...
sol2 = solve(
    prob, DP5(), dt = 1 // 2^(2), save_everystep = true,
    saveat = [0.0, 1 / 2, 1.0]
)
...
sol2 = solve(
    prob, RK4(), dt = 1 / 2^(2), save_everystep = true, adaptive = false,
    saveat = [0.0, 0.125, 0.6, 0.61, 0.8, 1.0]
)
...
sol2 = solve(
    prob, Rosenbrock32(), dt = 1 / 2^(2), save_everystep = true,
    saveat = [0.125, 0.6, 0.61, 0.8]
)
...
sol2 = solve(prob, Trapezoid(), dt = 1 / 2^(2), saveat = [0.125, 0.6, 0.61, 0.8])

@test sol.retcode == sol2.retcode == ReturnCode.Success
@test sort!(symdiff(sol.t, sol2.t)) == [0.0, 0.125, 0.6, 0.61, 0.8, 1.0]

sol = solve(
    prob, Trapezoid(), dt = 1 / 2^(2), save_everystep = true, dense = false,
    saveat = [0, 0.125, 0.6, 0.61, 0.8]
)
...
sol2 = solve(
    prob, DP5(), dt = 1 // 2^(2), save_everystep = false, dense = false,
    saveat = 0:(1 // 100):1
)
...
sol2 = solve(
    prob, DP5(), dt = 1 // 2^(2), save_everystep = false, dense = false,
    saveat = range(0, stop = 1, length = 100)
)
...
sol2 = solve(prob2, DP5(), dt = 1 // 2^(2), saveat = 0.1, save_idxs = 1:2:5)
...
sol2 = solve(
    prob2, DP5(), dt = 1 // 2^(2), saveat = 0.1, save_idxs = 1:2:5,
    save_everystep = true
)
...
sol = solve(
    prob, DP5(), saveat = 0.2, save_on = false, save_start = false,
    save_end = false
)
@test isempty(sol.t) && isempty(sol.u)
...
sol = solve(prob, Tsit5(), saveat = 4.0)
...
integ = init(
    ODEProblem((u, p, t) -> u, 0.0, (0.0, 1.0)), Tsit5(), saveat = _saveat,
    save_end = true
)
add_tstop!(integ, 2.0)
solve!(integ)
...
@testset "Proper save_start and save_end behavior" begin
    ...
    @test solve(prob, Tsit5(); saveat = 0:0.1:0.4).t == [0.0; 0.1; 0.2; 0.3; 0.4]
    @test solve(prob, Tsit5(); saveat = 0:0.1:0.4, save_start = true, save_end = true).t ==
        [0.0; 0.1; 0.2; 0.3; 0.4]
    @test solve(
        prob, Tsit5(); saveat = 0:0.1:0.4, save_start = false, save_end = false
    ).t ==
        [0.1; 0.2; 0.3]
    ...
    @test solve(prob, Tsit5(); saveat = [0.2]).t == [0.2]
    @test solve(prob, Tsit5(); saveat = [0.2], save_start = true, save_end = true).t ==
        [0.0; 0.2; 0.4]
    @test solve(prob, Tsit5(); saveat = [0.2], save_start = false, save_end = false).t ==
        [0.2]
end
