...
@testset "Mass Matrix Accuracy Tests" for mm in (almost_I, mm_A)
    ...
    for iip in (false, true)
        ...
        @test _norm_dsol(ImplicitMidpoint(extrapolant = :constant), prob, prob2) ≈ 0 atol =
            1e-10
        ...
        @test _norm_dsol(FBDF(), prob, prob2) ≈ 0 atol = 1e-12
...
@testset "Dependent Mass Matrix Tests" for mm in (dependent_M1, dependent_M2)
    ...
    for iip in (false, true)
        ...
        @test _norm_dsol2(
            ImplicitEuler(nlsolve = NLNewton(κ = 1e-10)),
            prob,
            prob2,
            reltol = 1e-10,
        ) ≈ 0 atol = 5e-4
        @test_skip _norm_dsol2(
            ImplicitMidpoint(nlsolve = NLNewton(κ = 1e-10)),
            prob,
            prob2,
            tstops = eulersol.t,
        ) ≈ 0 atol = 1e-6
