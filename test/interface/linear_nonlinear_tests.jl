...
solref = @test_nowarn solve(
    prob,
    TRBDF2(autodiff = false, linsolve = IterativeSolversJL_GMRES(), smooth_est = false),
);
@test length(sol.t) < 20
sol = @test_nowarn solve(
    prob,
...
sol = @test_nowarn solve(
    prob,
    QNDF(autodiff = false, linsolve = IterativeSolversJL_GMRES(), concrete_jac = true),
);
...
sol = @test_nowarn solve(
    prob,
    TRBDF2(autodiff = false, linsolve = KrylovJL_GMRES(), smooth_est = false),
);
@test length(sol.t) < 20
sol = @test_nowarn solve(
    prob,
...
sol = @test_nowarn solve(
    prob,
    QNDF(autodiff = false, linsolve = KrylovJL_GMRES(), concrete_jac = true),
);
@test length(sol.t) < 25
sol = @test_nowarn solve(
    prob,
