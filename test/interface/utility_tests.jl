...
@testset "calc_W and calc_W!" begin
    ...
    # Out-of-place
    fun = ODEFunction((u, p, t) -> A * u; mass_matrix = mm, jac = (u, p, t) -> A)
    integrator =
        init(ODEProblem(fun, u0, tspan), ImplicitEuler(); adaptive = false, dt = dt)
    ...
    integrator =
        init(ODEProblem(fun, u0, tspan), ImplicitEuler(); adaptive = false, dt = dt)
    calc_W!(
        integrator.cache.nlsolver.cache.W,
        integrator,
...
@testset "Implicit solver with lazy W" begin
    ...
    fun2_ip = ODEFunction(
        _f_ip;
        mass_matrix = mm,
        jac_prototype = DiffEqArrayOperator(
            similar(A);
